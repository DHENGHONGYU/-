...
| 技术债管理文档 | `docs/TECH-DEBT.md` | 已完成 | 建立技术债登记与清理计划 |
| 预审查脚本 v2.1 | `scripts/pre-review-check.ts` | 已完成 | 修复 ESLint 输出过大、临时文件捕获、错误判断 |
...
| News 收藏状态 IndexedDB 持久化 | `src/store/newsStore.ts`、`src/data/db.ts` | 已完成 | 移除 localStorage 同步读写，支持自动迁移 |
...
| 数据流引擎 SSE + 轮询回退 | `docs/DATAFLOW_DATA_DEFINITION.md` | 规格已完成 | 实时推送优先，降级轮询兜底 |

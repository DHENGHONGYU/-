# Yuandian Law Kimi Plugin

This plugin bundles a Kimi skill and helper script for querying the Yuandian Law
datasource through `agent-gw`.

## Contents

- `kimi.plugin.json`: catalog manifest for the Kimi plugin.
- `skills/yuandian_law/SKILL.md`: model instructions for selecting and calling
  Yuandian Law datasource APIs.
- `scripts/yuandian_law_tool.py`: command-line wrapper around
...
python3 scripts/yuandian_law_tool.py call --api-name "<api name>" --params-json '{"query":"民法典"}'
```

...
summary: "**Version**: v1.0.0 | **生成日期**: 2026-07-13 | **来源**: 12 份审计报告 + 3 份历史教训文件 + 本次对话关键决策 **覆盖范围**: MC..."
...
> **Version**: v1.0.0 | **生成日期**: 2026-07-13 | **来源**: 12 份审计报告 + 3 份历史教训文件 + 本次对话关键决策
...
> **Source**: `../audit/mcp-server-governance-retrospective.md`（2026-07-20）
...
> **Source**: `../../reference/agent-audit-report.md`（2026-06-27）
...
- **What**：`src/showcase/index.ts` 中通过 `new AgentRuntime()` 创建了独立实例，与 `src/agents/index.ts` 导出的全局单例不是同一对象。两个 AgentRuntime 的注册信息和任务互相不可见。
...
> **Source**: `../audit/code-quality-audit-report.md`（2026-06-30）、`../audit/v9-code-quality-kanban-20260629.md`
...
- **How**：① 建立 `src/config/thresholds.ts` 等配置层，所有业务参数从配置注入；② 启用 ESLint `no-magic-numbers` 规则；③ 3 位以上数字必须提取为 const 或 config；④ 引擎层阈值、权重、公式参数必须 100% 配置化。
- **严重级**: ?? P0
...
> **Source**: `../audit/debug-output-cabin-not-rendering.md`（2026-07-04）、`../audit/output-cabin-remediation-report.md`（2026-07-04）
...
> **Source**: `../audit/code-quality-audit-report.md`、`../audit/代码质量合规审查报告_2026-07-08.md`
...
- **Where**：`src/components/atoms/List.tsx`、`src/config/routes.ts`、`src/services/trading/tradingService.ts`、`src/services/analysis`。
...
> **Source**: `../../explanation/v9-架构缺陷与整改行动清单.md`（2026-06-27）、`../../explanation/design/audit-summary-report.md`（2026-06-27）
...
> **Source**: `../audit/文档-代码双向一致性检测报告_2026-07-08.md`、`../audit/代码质量整改与回归测试报告_2026-07-08.md`

### 教训 19：RBAC 文档断层（代码超前、文档未同步）

- **What**：v24 引入了完整的 RBAC 权限系统（6 表 + 7 个 ENVELOPE_ACTION），但 `../../explanation/03-architecture-standards.md` 和 `../../reference/data-definition.md` 完全无 RBAC 记录。`../../../AGENTS.md` 仅在 migration 段提及，顶部服务层清单未列 `rbac` 子域。新人从核心文档无法得知 RBAC 存在。
- **Why**：功能开发完成后未同步更新核心架构文档。文档更新未纳入功能交付的 DoD（Definition of Done）。
- **Where**：`../../../AGENTS.md`（:16, :669）、`../../explanation/03-architecture-standards.md`、`../../reference/data-definition.md`。
- **When**：2026-07-08 文档-代码双向一致性检测。
- **Who**：RBAC 功能开发者 + 文档维护者。
- **How**：① 将「文档同步」纳入功能交付的 DoD（完成标准）；② 建立 `../../explanation/03-architecture-standards.md` 和 `../../reference/data-definition.md` 为必须更新的文档；③ 使用自动化脚本（`doc-code-consistency.cjs`）定期检测文档滞后。
- **严重级**: ?? P1
...
- **Who**：文档维护者 + 团队知识管理。
- **How**：① 核心文档（AGENTS.md/architecture.md/data-definition.md）使用「引用 + 脚本生成」而非硬编码数字；② 定期清理过期文档，定唯一真相源（如加 `deprecated` 红标）；③ Glob 扫描时务必排除 `node_modules`。
- **严重级**: ?? P2
...
> **Source**: `../audit/mcp-server-governance-retrospective.md`、`../audit/output-cabin-remediation-report.md`、`../audit/代码质量合规审查报告_2026-07-08.md`
...
> **Source**: `../audit/debug-output-cabin-not-rendering.md`（2026-07-04）
...
> **Source**: `../../reference/agent-audit-report.md`（2026-06-27，§9）
...
- **Where**：`src/services/analysis` vs `../../../.agents/skills/feature-window-context-doc/SKILL.md`。
...
- **What**：`RESEARCH_STATUS`、`ResearchStatus`、`DEFAULT_POOL_GROUP` 同时在 `src/config/dbConfig.ts` 和 `src/constants/pool.constants.ts` 定义。20+ 生产文件和 4 个测试文件从 `src/config/dbConfig.ts` 导入这些业务常量，违反 AGENTS.md「`config/` 禁止被业务常量污染」的分层意图。
- **Why**：早期实现将股票池业务常量放入配置层，后续为消除 services→config 跨层违规在 constants 层重建定义，但未清理旧定义和旧导入路径，形成「双真相源」。
- **Where**：`src/config/dbConfig.ts`（旧定义） vs `src/constants/pool.constants.ts`（新真相源），影响 `poolStore.ts`、`poolTransitionEngine.ts`、`dataLayerStockStores.ts`、`PoolBoard.tsx` 等 20+ 文件及 `poolStore.test.ts` 等测试。
- **When**：2026-07-13 股票池统一存储清理中确认并修复。
- **Who**：常量迁移执行者 + 代码评审（未在定义重复阶段拦截）。
- **How**：
  1. 明确权威位置：`src/constants/` 承载业务语义常量，`src/config/` 仅保留基础设施配置；
  2. 删除 `src/config/dbConfig.ts` 中的重复定义，批量将生产代码 import 路径迁移到 `src/constants/pool.constants.ts`；
...
| # | 检查项 | 检查方法 |
|---|-------|---------|
| B1 | 文档与代码同步更新（AGENTS.md/architecture.md/data-definition.md） | 人工核对文件变更列表 |
...
> **文档归档**: 本文档已纳入 `./lessons-learned.md`
...
> **Source**: `outputs/audit-db-references-report.txt`（2026-07-13）
> 相关 SKILL: `../../../.agents/skills/feature-window-context-doc/SKILL.md`
...
- **Where**：`scripts/other/validate-data-consistency.ts`、`src/data/db-schema.ts`、`src/data/migrations/rbacMigrationV24.ts`。
...
- **How**：
  ...
  3. 新增 SKILL `../../../.agents/skills/feature-window-context-doc/SKILL.md` 固化 SOP。
- **严重级**: ?? P1

...
> **Date**: 2026-07-09 | **版本**: v1.0 | **作?*: AI Agent
> **范围**: 5 ?E2E 测试文件修复 + PortalShell 日志增强 + 回归单元测试
...
| 维度 | 修复?| 修复?|
|------|--------|--------|
| E2E 测试通过?| 部分失败（选择?路由错误?| **93/93 全部通过** |
| 回归单元测试 | 40 ?| **65 ?*（新?25 个） |
| 核心日志埋点 | 3 ?| **6 ?*（新?3 处） |
| 已知跳过缺口 | 5 ?| 5 个（已生成修复报告） |

---

## 二、E2E 测试修复详情

### 2.1 accessibility.spec.ts ?键盘导航与焦点管?
| 修复?| 修复?| 修复?| 类别 |
|--------|--------|--------|------|
| 路由补全 | `page.goto('/input/hub')` | `page.goto('/#/input/hub')` | 路由 |
| 焦点检?| `page.locator(':focus')` | `document.activeElement` 评估 | 选择?|
| 页面加载 | ?| `waitForLoadState('networkidle')` | 稳定?|

**核心日志**（`[A11Y-Test]` 前缀）：
```typescript
console.log(`${LOG_PREFIX} [P1-FIX] Tab键导航：已导航到 /#/input/hub，准备检?document.activeElement`)
...
### 2.2 data-migration.spec.ts ?面包屑导?
| 修复?| 修复?| 修复?| 类别 |
|--------|--------|--------|------|
| 标题文本 | `'总控中心'` | `'总控?` | 文本 |
| 多元素匹?| `getByRole('heading', { name: '总控? })` | `.first()` 解决 strict mode | 选择?|
| 导航策略 | 直接检?Hub 页面 | 导航到子页面验证面包?| 逻辑 |

**核心日志**（`[DataMigration-Test]` 前缀）：
```typescript
console.log(`${LOG_PREFIX} [P1-FIX] beforeEach：导航到 /#/command/hub，标?总控舱（修复前错误使?'总控中心'），使用 .first() 解决 strict mode ?h1 冲突`)
```
...
| 修复?| 修复?| 修复?| 类别 |
|--------|--------|--------|------|
| 路由路径 | `/input/collection-test`?04?| `/input/collect-tasks` | 路由 |
| 统计卡片 | `getByText('采集?)` 多元素匹?| `.first()` 精确定位 | 选择?|

**核心日志**（`[InputCollection-Test]` 前缀）：
```typescript
console.log(`${LOG_PREFIX} [P1-FIX] 统计卡片：验?任务总数/采集?已完?失败 四个卡片?first() 解决 strict mode 多元素匹配）`)
```
...
| 修复?| 修复?| 修复?| 类别 |
|--------|--------|--------|------|
| 按钮文本 | `'输出舱首?` | `'研报复盘'`、`'仪表?` | 文本 |
| 导航逻辑 | ?Hub 点击按钮 | 从研究页面验证按钮存?| 逻辑 |
...
| 修复?| 修复?| 修复?| 类别 |
|--------|--------|--------|------|
| 定位方式 | `getByRole('heading', ...)` | `getByRole('button', ...)` | 选择?|
| 按钮文本 | `'V6 个股智能评分'` | `'V6 智能评分'` | 文本 |
| Hub 标题 | ?| 验证 Hub 页面标题 | 完整?|

**核心日志**（`[StockScore-Test]` 前缀）：
```typescript
console.log(`${LOG_PREFIX} [P1-FIX] 智能评分导航：侧边栏按钮实际文本?'V6 智能评分'（修复前错误使用 'V6 个股智能评分'）`)
```
...
[PortalShell.tsx](../../../src/portal/PortalShell.tsx) 中新?3 处关键日志：
...
useEffect(() => {
  if (location.pathname.endsWith('/hub') && activeCabin !== 'command') {
    ...
    logger.info('[PortalShell] Hub页面重定?, {
...
**排查价?*：当用户访问 `/input/hub` 被重定向?`/input` 时，日志明确记录触发原因，避免困?为什?Hub 页面没有渲染"?
### 4.2 侧边栏导航日志（L367-377?
```typescript
onClick={() => {
  logger.info('[PortalShell] 侧边栏导?, {
    cabin: activeCabin,
    item: item.key,
    label: item.label,
    path: item.path,
...
**排查价?*：当用户点击侧边栏按钮后页面 404 时，日志会记?`label` ?`path`，可快速定位是侧边栏配置错误还是路由注册缺失?
...
**排查价?*：当页面渲染空白或错误组件时，日志明确记?`pathname ?cabin ?app` 的映射决策链，可快速定位是路由分发逻辑问题?
### 4.4 已有日志（本轮未修改?
| 日志 | 位置 | 作用 |
|------|------|------|
| `[PortalShell] 路径匹配` | L220 | 记录路径匹配到哪个舱?|
| `[PortalShell] 路径未匹配` | L222 | 警告无法匹配的路?|
...
| 测试套件 | 用例?| 覆盖内容 |
|----------|--------|---------|
| 路由注册完整?| 10 | 关键路由是否?ROUTE_REGISTRY 中注?|
| 侧边栏按钮标签验?| 5 | 按钮文本是否与预期一致（防回归） |
| 侧边栏路径与路由一致?| 17 | 侧边栏按钮路径是否在路由表中注册 |
| 关键页面标题正确?| 2 | 舱室标题是否??结尾 |
| 路由分类正确?| 4 | 各舱室子路由 category 是否正确 |
| **isActivePath 路径匹配**（新增） | 5 | 精确匹配、子路径匹配、`/command` 特殊规则 |
| **PANEL_ITEMS 结构完整?*（新增） | 7 | key 唯一性、path 唯一性、标签黑名单 |
| **CABINS 舱室定义**（新增） | 6 | 5 舱室结构、标题规范、id/path 一致?|
| **路由到舱室映?*（新增） | 7 | category 一致性、Hub 重定向逻辑 |
...
当开发团队修复某个缺口后，从 `KNOWN_GAPS` 中移除对应路径，对应?skipped 测试将自动转为执行?
---
...
| 多元素匹?| `.first()` 或更精确?role | 忽略 strict mode 错误 |
| 焦点检?| `document.activeElement` 评估 | `page.locator(':focus')` |

### 6.2 路由防护策略

```
用户点击侧边??路径白名单检??路由注册表匹??App 分发??页面渲染
                                    ?未匹?                                 404 页面
```

**防护措施**?1. `ROUTE_REGISTRY` 作为唯一路由真相?2. 回归测试定期检查侧边栏路径与路由表一致?3. `KNOWN_GAPS` 机制标记已知缺口，防止误?
### 6.3 日志分层
...
| 优先?| 任务 | 关联 |
|--------|------|------|
| P0 | 修复 `/output/reports` 路径不一致（改侧边栏 path ?`/output/research`?| 报告缺口 4 |
| P1 | 修复 `/trading/execution` 路径（指?`/trading/execution-plans`?| 报告缺口 2 |

...
|----------|----------|------------|------|----------|
| `../../reference/03-architecture-standards.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
| `../../reference/06-routing-specs.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
| `../../reference/08-implementation-plan.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
| `../../reference/09-quality-gates.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
| `../../reference/10-glossary.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
...
| `../../reference/v9-system-blueprint.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
| `../../reference/input-cabin-spec.md` | 修改 | Doc-Sync | ? | lint ? / build ? |
...
2. **`../../reference/06-routing-specs.md`**：补全 5 条遗漏路由；修正第 8 节入口路由组件映射为 `PortalShell`。
3. **`../../explanation/design/08-implementation-plan.md`**：统一测试基线为 44 files / 291 tests；数据流引擎、板块轮动、Widget 框架等任务状态更新。
4. **`../../reference/09-quality-gates.md`**：跨层调用 0 违规 / 2 警告；硬编码 389 处；死代码 11 处。
...
2. **基线数据错误**：`../../reference/v9-system-blueprint.md` 中 E2E/死代码基线与实测不符。
3. **模块实现状态描述不精确**：DataFlow 缓存、Widget 子目录、CockpitShell 动态性等。
4. **章节编号错乱**：`../../explanation/design/08-implementation-plan.md` 出现 10 后接 9。

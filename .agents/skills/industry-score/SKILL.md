...
本 Skill 以“第四次工业革命稀缺核心资源”为检索目标，基于 V6 项目 `sectorSkillData` 的 SKILL 量化评分（SKILL-C 四维加权 / SKILL-A 双维度量表 / SKILL-N 六维板块），结合 F:\投资赛道分析\.trae\skills 中的 `sector-analysis-framework` 与 `industry-score-mapping` 进行动态修正与可解释评分。

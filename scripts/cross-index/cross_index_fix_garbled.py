...
def fix_links_in_file(filepath, filename_index):
    ...
    def replace_link(match):
        ...
        has_garbled = any(ord(c) > 127 for c in raw_link) and not all(
            '\u4e00' <= c <= '\u9fff' for c in raw_link if ord(c) > 127
        )
    ...
    for md_file in DOCS_DIR.rglob("*.md"):
        try:
            with open(md_file, "r", encoding="utf-8", errors='ignore') as f:
                content = f.read()
                has_garbled = any(ord(c) > 127 for c in content) and not all(
                    '\u4e00' <= c <= '\u9fff' or '\u3000' <= c <= '\u303f' 
                    for c in content if ord(c) > 127
                )
                if has_garbled:

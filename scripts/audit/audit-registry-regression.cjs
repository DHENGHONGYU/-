...
const AUDIT_REGISTRY_TS = path.join(ROOT, 'scripts', 'audit', 'audit-registry.ts');
const COMPONENT_REGISTRY_TS = path.join(ROOT, 'src', 'components', 'componentRegistry.ts');
const SERVICE_REGISTRY_TS = path.join(ROOT, 'src', 'services', 'serviceRegistry.ts');
const ATOM_REGISTRY_TS = path.join(ROOT, 'src', 'components', 'registry', 'atomRegistry.ts');
...
function runFullAudit() {
  // 用 shell 模式调用 npx tsx（绕过 tsx package exports 的内部入口不确定性）
...
function staticThinEntryCheck() {
  ...
  const warnings = [];
  try {
    const thinEntry = fs.readFileSync(COMPONENT_REGISTRY_TS, 'utf8');
...
function historicalRegressionCheck() {
  const errors = [];
  const warnings = [];
  // ---- [Dedup 1] Service 注册表：PortfolioService vs TradingPortfolioService id 唯一性 ----
  // 本轮修复前问题：两条 id 都写成 PortfolioService → 重名；修复后 id 分别为 PortfolioService / TradingPortfolioService
  try {
    const srv = fs.readFileSync(SERVICE_REGISTRY_TS, 'utf8');
  ...
  // ---- [Dedup 2 + DeprecationMeta] Component 注册表：Atom 的 SkeletonLegacy(deprecated) vs Molecule 的 Skeleton(active) ----
  // 本轮修复前问题：两条 name 都写成 Skeleton → 重名；修复后把 Atom 版改名为 SkeletonLegacy，并补全 deprecationMeta.supersededBy + deprecatedSince
  const MOLECULE_REGISTRY_TS = path.join(ROOT, 'src', 'components', 'registry', 'moleculeRegistry.ts');
  try {
    const atom = fs.readFileSync(ATOM_REGISTRY_TS, 'utf8');
    const mol = fs.readFileSync(MOLECULE_REGISTRY_TS, 'utf8');
    ...
    // Dedup 2 断言：Atom 层仅出现 SkeletonLegacy，Molecule 层仅出现 Skeleton
    ...
    // DeprecationMeta 断言：SkeletonLegacy 必须有 supersededBy + deprecatedSince + removalTarget 完整声明
...
function main() {
  ...
  if (CLI.json) {
    console.log(JSON.stringify({
      tool: 'audit-registry-regression', version: '1.0',
      generatedAt: new Date().toISOString(),

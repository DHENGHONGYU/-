...
> **总警告数**: 192 个（mapping-integrity: 100 + split-quality: 92），typography 已修复
...
1. [概述](#概述)
2. [mapping-integrity 审计警告（100 个）](#mapping-integrity-审计警告-100-个)
   - [路由注册完整性](#路由注册完整性)
   - [App分发器路由覆盖](#app分发器路由覆盖-55个)
   - [Action→Store映射唯一性](#actionstore映射唯一性-5个)
   - [冗余Store检测](#冗余store检测-8个)
   - [EventBus事件订阅完整性](#eventbus事件订阅完整性-32个)
3. [split-quality 审计警告（92 个）](#split-quality-审计警告-92-个)
   - [文件行数超限（AP-001）](#文件行数超限-ap-001)
   - [圈复杂度超限（AP-002）](#圈复杂度超限-ap-002)
   - [Core层依赖违规（AP-005）](#core层依赖违规-ap-005)
   - [函数重复定义（AP-007）](#函数重复定义-ap-007)
4. [typography 审计警告（2 个）](#typography-审计警告-2个)
5. [修复优先级建议](#修复优先级建议)
6. [趋势监控](#趋势监控)
...
| 审计脚本 | 警告数 | 严重级别 | 状态 |
|---------|--------|---------|------|
| mapping-integrity | 100 | 中/低 | 待修复 |
| split-quality | 92 | 中/高 | 待修复 |
| typography | 0 | 中 | ✅ 已修复 |
| **合计** | **192** | - | - |

---

## mapping-integrity 审计警告（100 个）
...
|---|---------|------|---------|
| 1 | `/` | 首页 | PortalShell |
| 2 | `/cockpit` | 驾驶舱 Dashboard | PortalShell |
| 3 | `/input/bulk-import` | 输入舱 - 批量导入 | Input |
| 4 | `/input/hot-sectors` | 输入舱 - 热门板块 | Input |
| 5 | `/input/data-test` | 输入舱 - 采集测试 | Input |
| 6 | `/trading/flow` | 交易舱 - 交易流程 | Trading |
| 7 | `/output/research` | 输出舱 - 研究报告 | Output |
| 8 | `/output/review` | 输出舱 - 交易复盘 | Output |
| 9 | `/output/export` | 输出舱 - 数据导出 | Output |
| 10 | `/output/dashboard` | 输出舱 - 仪表盘 | Output |
| 11 | `/command/agents` | 智能体总控台 | Command |
| 12 | `/command/agents/registry` | 智能体注册表 | Command |
| 13 | `/command/agents/registry/:agentId` | 智能体详情 | Command |
| 14 | `/command/agents/trigger` | 智能体任务触发 | Command |
| 15 | `/command/agents/tasks` | 智能体任务列表 | Command |
...
| 19 | `/command/agents/dag-scheduler` | DAG 调度器 | Command |
| 20 | `/command/agents/feedback` | 反馈控制台 | Command |
...
| 28 | `/command/showcase` | 组件示例库 | Command |
...
| 34 | `/analysis/sector` | 行业与板块分析 | Analysis |
...
| 38 | `/analysis/score-docs` | 评分文档版本库 | Analysis |
...
| 42 | `/analysis/value-pit` | 价值洼地策略选股（五维评分 + 轮动信号） | Analysis |
| 43 | `/analysis/multi-factor` | 多因子筛选（条件组增删 / 因子编辑 / 模板持久化） | Analysis |
| 44 | `/analysis/stock-pool` | 股票池看板 | Analysis |
| 45 | `/trading/strategy-snapshots` | 策略快照 | Trading |
| 46 | `/trading/holdings` | 交易持仓管理 | Trading |
| 47 | `/trading/execution-plans` | 执行计划管理 | Trading |
| 48 | `/trading/execution` | 执行管理（别名 → execution-plans） | Trading |
| 49 | `/trading/portfolio` | 投资组合管理 | Trading |
| 50 | `/trading/risk` | 风险控制管理 | Trading |
| 51 | `/input/local-knowledge` | 本地知识库 | Input |
| 52 | `/input/seven-dim` | 七维采集策略配置 | Input |
| 53 | `/input/fetcher-config` | 抓取引擎配置 | Input |
| 54 | `/input/collect-tasks` | 采集任务监控 | Input |
| 55 | `/mock-test` | V9 模块 Mock 验证页 | Mock |

**修复建议**: 在对应的 `src/apps/{cabin}/{Cabin}App.tsx` 文件中添加路由匹配分支
...
| # | Action名称 | 修复建议 |
|---|-----------|---------|
| 1 | `feedbackIssuesDetected` | 在 `src/core/databridge.ts` 的 ACTION_TO_STORE_MAP 中添加映射 |
| 2 | `deleteRbacAuditLog` | 在 `src/core/databridge.ts` 的 ACTION_TO_STORE_MAP 中添加映射 |
| 3 | `queryGet` | 在 `src/core/databridge.ts` 的 ACTION_TO_STORE_MAP 中添加映射 |
| 4 | `queryList` | 在 `src/core/databridge.ts` 的 ACTION_TO_STORE_MAP 中添加映射 |
| 5 | `queryByIndex` | 在 `src/core/databridge.ts` 的 ACTION_TO_STORE_MAP 中添加映射 |

### 冗余Store检测（8个）

> **问题描述**: 以下 Store 未被任何 UI 层代码直接引用，也未通过传递可达性被标记为 used

| # | Store名称 | Hook名称 | 状态 |
|---|-----------|---------|------|
| 1 | `analysisHubStore` | - | 未使用 |
| 2 | `chatStore` | - | 未使用 |
| 3 | `databridgeStore` | `useDataBridgeStore` | 未使用 |
| 4 | `dataflowStore` | - | 未使用 |
| 5 | `hybridProofreadStore` | - | 未使用 |
| 6 | `rotationSignalStore` | - | 未使用 |
| 7 | `tradingHubStore` | - | 未使用 |
| 8 | `widgetStore` | - | 未使用 |
...
| Store名称 | 依赖来源 | 说明 |
|-----------|---------|------|
| `signalAdviceStore` | `tradingStore` | 仅通过 Facade 传递可达 |
| `watchlistStore` | `tradingStore` | 仅通过 Facade 传递可达 |

**修复建议**: 
- 确认这些 Store 是否仍有业务价值
- 若无价值，考虑删除或标记为 `@deprecated`
- 若有价值，检查为何没有 UI 层直接消费者
...
## split-quality 审计警告（92个）

### 文件行数超限（AP-001）

> **阈值**: services 层 500 行，其他层 800 行

| # | 文件路径 | 行数 | 阈值 | 严重级别 |
|---|---------|------|------|---------|
| 1 | `src/data/dataLayer.test.ts` | 1684 | 800 | Major |
| 2 | `src/data/sectorSkillData.ts` | 879 | 800 | Major |
| 3 | `src/pages/command/agent/LlmManagementPage.tsx` | 1044 | 800 | Major |
| 4 | `src/pages/input/CollectTaskPage.tsx` | 896 | 800 | Major |
...
| 11 | `src/pages/command/agent/LlmManagementPage.tsx` | 89 | Major |
| 12 | `src/pages/input/CollectTaskPage.tsx` | 62 | Major |
...
| 56 | `src/store/poolStore.ts` | 45 | Major |
...
| 60 | `src/utils/dataValidation.ts` | 46 | Major |
...
> **问题描述**: core 层禁止直接 import services 层，违反依赖注入解耦原则
...
**修复建议**: 通过接口注入方式解耦，由 services 层在 `main.tsx` 启动时注册实现
...
| # | 函数名 | 重复位置 | 修复建议 |
|---|-------|---------|---------|
| 1 | `generateId` | `src/data/db-utils.ts`, `src/utils/a11y.ts` | 合并到单一位置 |
| 2 | `now` | `src/data/db-utils.ts`, `src/utils/timeUtils.ts` | 合并到单一位置 |
...
| 类别 | 数量 | 说明 | 状态 |
|------|------|------|------|
| 高圈复杂度（CC > 100） | 4 | 影响可维护性和测试 | 待修复 |
| 函数重复定义 | 7 | 违反 DRY 原则 | 待修复 |
| 硬编码字体 | 0 | 违反设计系统规范 | ✅ 已修复 |
...
|------|------|------|
| 文件行数超限 | 24 | 影响代码可读性 |
| 中等圈复杂度（CC 40-100） | 54 | 渐进式重构 |
| 冗余Store | 8 | 需确认业务价值 |
| EventBus事件不匹配 | 32 | 需确认架构设计 |
| App分发器路由覆盖 | 55 | 需确认路由设计 |

---

## 趋势监控

后续将通过自动化脚本 `scripts/audit-trend-monitor.ts` 定期扫描并记录这些警告项的变化趋势。监控指标包括：

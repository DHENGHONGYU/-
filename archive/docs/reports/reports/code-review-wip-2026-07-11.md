...
| 颜色令牌 | `lint:colors` | ⚠️ 未跑（建议提交前补跑） |
| 单元测试 | `npm test` | ⚠️ 未跑（建议提交前补跑，尤其 dataLayer.test 3362 行重写） |

> 注：`src/utils/` 与 store→store 的 Facade 依赖均被 `audit:layers` 容忍（0 警告），不阻断提交。

---

## 二、`src/utils/dataValidation.ts`（新增，安全相关）
...
`src/utils/` 含 `a11y / precision / timeUtils / xssSanitizer / dataValidation`（本次新增）。虽不在 `AGENTS.md` 标准分层（库函数应在 `src/lib/`），但属既有目录且 `audit:layers` 0 警告容忍。

**Suggestion**：二选一 ——
1. 在 `AGENTS.md` §一显式列出 `src/utils/` 作为库函数层（与 `src/lib/` 并存或合并）；
...
- [ ] （可选）`AGENTS.md` 补充 `src/utils/` 与 store→store Facade 约定
...
**未处理（原 💭 nit，本次保留）**：Zustand subscribe 未用 selector、loadStocks 冗余 set、`src/utils/` 未入 AGENTS.md 分层文档。

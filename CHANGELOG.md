...
- **环境配置净化 — 修正 .env.local.example 违反硬约束**（TD-015 关联）：
  - 修正 `VITE_LLM_MODEL=deepseek-v4-flash` → `deepseek-chat`（违反 project_memory 硬约束"LLM 模型名必须统一为 deepseek-chat，无效模型名会导致 API 错误"）
  - 修正 `VITE_AKSHARE_BASE_URL=http://localhost:8000` → `/api/akshare`（违反 project_memory 硬约束"VITE_AKSHARE_BASE_URL 必须为 /api/akshare，经 Vite 同源代理转发至 :8000"）
  - 补充硬约束注释说明，防止后续误改回错误值
...
- **directDataAPI.ts 重复副本消除（TD-012 阶段 2 完成）**：
  - `src/services/data-collector/directDataAPI.ts` 从 496 行独立副本改为 242 行薄适配层（精简 51%），从 `src/services/fetcher/directDataAPI.ts` 导入 canonical 实现并做签名映射
  - 修复 3 处高风险 bug：新浪 volume/amount 字段索引 [8]/[9] → [29]/[30]、批量行情 matchAll+idx 顺序敏感 → split 逐段按内容解析、腾讯 K 线 qfqday 兜底缺失 → qfqday ?? day 复权优先
  - 保留 10 项 export 向后兼容（RealtimeQuote/SourceInfo + 6 项行情/K线函数 + 2 项适配函数），dataSourceOrchestrator.ts 零破坏
  - `tushareAdapter.ts` + `collectionPipeline.ts` import 路径改为直接指向 `../fetcher/directDataAPI`
  - 新增 `scripts/audit-directDataAPI-consistency.mjs` 自动化一致性检查脚本（适配层模式 + 完整性校验，exit code=0）

- **TD-015 P2-P3 治理 — 跨配置源引用治理**：
  - `tushareProvider.ts` 提取 `resolveTushareUrl()` 函数，环境检测逻辑显式化（浏览器走 Vite 代理，Node 直连，通过 `typeof window` 检测自动选择配置源，无需拆分 browser/node 版本）
  - 删除死代码 `MOCK_TENCENT_BASE_URL` / `MOCK_SINA_BASE_URL` / `MOCK_NETEASE_BASE_URL`（经 Grep 确认无消费者，FetcherConfigPage 使用 `dataSourceRegistry` 的 `DATA_SOURCE_ENDPOINTS`）
  - `MOCK_AKSHARE_BASE_URL` 重命名为 `AKSHARE_LOCAL_BASE_URL`（运行时直连本地 Python 采集服务 :8000，非展示常量）
  - 更新 `multiSourceFetcher.ts` import 和使用处
  - 3 文件 GetDiagnostics 全 0 错误，一致性脚本退出码 = 0

- **环境配置净化 — 配置源分工说明 + 跨源引用警示**（TD-015 关联）：
  - `src/config/dataSourceUrls.ts` + `src/config/marketDataEndpoints.ts` 顶部添加配置源分工说明：前者为绝对 URL（Node/Electron 主进程直连），后者为 Vite proxy 路径（浏览器专用），两者不可混用
  - `dataSourceUrls.ts` 添加 MOCK_* 命名说明（MOCK_TENCENT_BASE_URL 等实际值是真实地址，仅用于 FetcherConfigPage 展示）
  - `src/services/data-collector/tushareProvider.ts` + `multiSourceFetcher.ts` 添加跨配置源引用警示注释 + 长期治理方案（拆分 browser/node 版本 / 展示常量迁移至 uiConstants.ts）
  - 新增自动化检查脚本 `scripts/audit-directDataAPI-consistency.mjs` 可作为 CI/pre-commit hook 守护 TD-012 不回归
...
### Metrics — TD-015 跨配置源引用治理（2026-08-09）

| 指标 | 治理前 | 治理后 | 变化 |
|------|--------|--------|------|
| dataSourceUrls.ts 常量数 | 16 | 13 | -3（删除死代码） |
| 死代码常量数 | 3 | 0 | -3（100% 清除） |
| MOCK_* 误导性命名 | 4 | 2 | -2（50% 消除） |
| 跨源引用隐式三元表达式 | 1 | 0 | -1（100% 显式化） |
| GetDiagnostics 错误数 | - | 0 | 3 文件全 0 错误 |
| 一致性脚本退出码 | - | 0 | exit code = 0 |

...
> **版本**: v1.0.3 | **日期**: 2026-07-05
> **培训对象**: V9 项目所有代码审查者
> **培训目标**: 统一审查标准、提高审查效率、保证代码质量
...
**检查项**:
- [ ] 无跨层调用违规（`npm run audit:layers` → 0 violations）
- [ ] 依赖方向正确（符合 AGENTS.md §1 依赖方向规则）
- [ ] 新模块按四步集成（类型定义 → Store → Service → UI）
- [ ] 路由注册完整（routes.ts + App 分发器 + docs/06-routing-specs.md）
...
**检查项**:
- [ ] 无 `any` 类型（ESLint `@typescript-eslint/no-explicit-any: error`）
- [ ] 无 `@ts-ignore`（使用 `@ts-expect-error` + 注释）
- [ ] 所有数据结构有 TypeScript Interface
- [ ] 复杂泛型有 `Expect<Equals>` 类型测试

**常见违规案例**:

```typescript
// ❌ 错误：使用 any
function processData(data: any) {  // 违规！
  return data.map((item: any) => item.value);  // 双重违规！
}
...
// ✅ 正确：使用 @ts-expect-error 并说明原因
// @ts-expect-error 第三方库类型定义不完整，已知问题：https://github.com/xxx/issues/123
...
**检查项**:
- [ ] 颜色使用令牌（通过 `npm run audit:hardcode` → 0 violations）
- [ ] 魔法数字已提取为 const 或 config
- [ ] 引擎层参数从 `config.ts` 注入
...
<span className="text-red-500">涨</span>  // 违规！
...
function formatPrice(price: number) {
  return price.toFixed(2);  // 如果 price 是 NaN 会报错
}
    ...
    <div>
      {users.map(user => (
        <UserItem
          ...
          onClick={() => console.log(user.id)}  // 每次渲染都创建新函数！
        />
      ))}
    </div>
  );
...
**审查技巧**:
- 先看整体（架构、文件结构），再看细节（代码实现）
- 重点关注核心模块（services/store/core）
- 使用搜索快速定位关键模式（如 `any`、`db.`、`color:`）
- 运行代码并手动测试（涉及 UI 变更的 PR）
...
1. **[架构违规]** `src/services/xxx.ts:45`
   - 问题: 直接调用了 `db`，应通过 `DataBridge.forward()`
   - 修复建议: 
     ```typescript
     // 修改前
...
3. **[代码质量]** `src/services/xxx.ts:120-180`
   - 问题: 函数过长（60 行）
...
**回复审查意见**:
```markdown
@reviewer 感谢审查！我已修复所有 P0 问题：

1. **[架构违规]** 已改用 `DataBridge.forward()`
2. **[类型安全]** 已定义 `interface XXXProps`

P1 问题 3 我计划在下个 PR 中修复，本 PR 先聚焦核心功能。
```
...
**PR 特征**:
- 涉及多个模块（pages + store + services + types）
- 代码量大（200-500 行）
- 需要完整审查
...
2. 类型定义完整性（Interface）
3. 单元测试覆盖率（> 80%）
4. 文档同步（ARCHITECTURE.md / docs/06-routing-specs.md）
...
// Step 1: 类型定义（src/types/modules/fund-flow.ts）
...
// Step 2: Store（src/store/fundFlowStore.ts）
...
// Step 3: Service（src/services/fundFlowService.ts）
import { DataBridge } from '@/core/DataBridge';
...
// Step 4: UI（src/pages/FundFlowPage.tsx）
import { useFundFlowStore } from '@/store/fundFlowStore';
...
useEffect(() => {
  fetchData(stockCode);
}, [stockCode]);  // 当 stockCode 变化时重新获取数据
```
...
test('当 stockCode 变化时，重新获取数据', async () => {
...
**重构目标**: 降低评分引擎复杂度（从 1200 行拆分为 5 个独立模块）

**重构策略**:
1. 提取 `validateInput()` - 输入验证
2. 提取 `calculateScore()` - 评分计算
3. 提取 `normalizeScore()` - 分数归一化
4. 提取 `formatOutput()` - 输出格式化
5. 提取 `config.ts` - 配置注入
...
**根因**: API 返回数据格式变更，缺少容错处理

**修复**: 添加数据容错处理

**后续计划**:
- [ ] 补充单元测试（覆盖 API 数据异常场景）
- [ ] 添加 E2E 测试（首页加载流程）
...
**在线审查**:
1. 打开 PR 页面（`https://github.com/xxx/xxx/pull/123`）
2. 点击 "Files changed" 标签页
3. 鼠标悬停在代码行号上，点击 "+" 号添加评论
4. 填写审查意见（使用模板）
...
- **GitHub Pull Requests**: 在 VS Code 中审查 PR
- **Error Lens**: 实时显示 ESLint/TypeScript 错误

**使用 GitLens 审查**:
1. 安装 GitLens 插件
2. 打开 PR 分支（`git checkout pr-branch-name`）
3. 查看代码变更（Source Control 面板）
4. 使用 GitLens 查看代码历史（Code Lens → "3 authors"）
5. 在 VS Code 中添加评论（右键代码 → "Add Comment"）
...
| 等级 | 要求 | 权限 |
|------|------|------|
| **L1: 初级审查者** | 通过培训 + 完成 5 次审查 | 可审查 UI 组件 |
| **L2: 中级审查者** | 完成 20 次审查 + 无重大遗漏 | 可审查服务层代码 |
| **L3: 高级审查者** | 完成 50 次审查 + 架构师推荐 | 可审查核心模块 + 架构变更 |

**认证流程**:
1. 完成培训（阅读本文档 + 通过测验）
2. 跟随资深审查者学习（3 次 Pair Review）
3. 独立审查（5 次，由资深审查者复核）
4. 获得认证（更新 CODEOWNERS）
...
- P0 问题发现率（目标: 100%）
- 审查意见质量（建设性、有修复建议）
- 审查覆盖率（审查的 PR 数 / 团队 PR 总数）

**团队指标**:
- PR 合并周期（目标: < 3 天）
- 返工率（Request Changes 次数 / 总 PR 数，目标: < 20%）
- P0 问题逃逸率（合并后发现 P0 问题，目标: < 5%）
- 审查参与度（团队成员审查覆盖率，目标: 100%）
...
A: 
1. 先沟通（评论区或线下讨论）
2. 查文档（以 AGENTS.md 和 CODE-REVIEW.md 为准）
3. 升级（无法达成一致时，请架构师仲裁）
...
A:
1. 提前协调（作者提交 PR 前，先与审查者沟通）
2. 轮换审查者（避免单点依赖）
3. 升级（超过 24 小时未响应，请求其他审查者或维护者介入）

**Q3: 如何平衡审查严格度与开发速度？**

A:
- **P0 问题零容忍**（必须修复，否则拒绝合并）
- **P1 问题灵活处理**（可与作者讨论，决定是否本 PR 修复或后续迭代）
- **P2 问题记录跟进**（不阻塞合并，但记录到 Issue 后续修复）
- **紧急修复例外**（Hotfix 可先合并，但需 24 小时内补充审查）
...
1. **代码审查目标**: 发现缺陷、保证质量、知识共享、遵守规范
2. **三级审查体系**: L1 自动化 → L2 同级 → L3 架构
3. **P0 问题**: 架构合规、类型安全、零硬编码、功能正确（必须修复）
4. **审查流程**: 自检 → 审查 → 修改 → 批准合并
5. **审查者职责**: 及时响应、全面检查、建设性反馈、区分优先级

### 下一步行动

- [ ] 阅读 `docs/CODE-REVIEW.md`（完整审查标准）
- [ ] 阅读 `AGENTS.md`（项目规范）
- [ ] 完成培训测验（验证学习效果）
- [ ] 跟随资深审查者学习（3 次 Pair Review）
- [ ] 独立审查（5 次，获得 L1 认证）

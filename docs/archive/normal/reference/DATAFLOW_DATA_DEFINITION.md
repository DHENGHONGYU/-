---title: "数据流引擎（DataFlow Engine）数据字典"
domain: ref
status: active
...
change_log:
  - version: 1.2.3
    changes: "基准校对(2026-08-23)：A类双轨(fm v1.2.2 / 正文 v1.2.0) → 取真值 max=1.2.2 → PATCH++ 对齐 frontmatter/正文/change_log 三轨"
    date: 2026-08-23
  - version: v1.2.2
    changes: "基准日校对(2026-08-22)：R1取真值(P1 change_log 最新条目=v1.2.1) → R2 PATCH++(v1.2.2) / last_updated 刷新 / change_log 闭环"
    date: 2026-08-22
- version: v1.2.1
...
> 关联任务：`../guides/08-implementation-plan.md` 2.1.8
...
1. **魔法数字未常量化**：`refreshInterval`、`maxReconnectAttempts`、`maxReconnectDelay`、性能阈值等仍为硬编码，后续应迁移到 `src/constants/dataflow.constants.ts`。
...
- `../explanation/03-architecture-standards.md` 3.1.2：数据流引擎设计
- `../guides/08-implementation-plan.md` 2.1.8：数据流引擎任务

...
| **写入层** | DataBridge / DataBridgeAdapter / ACL Engine | 统一写入入口，权限校验，审计日志，失败重试 | `src/core/databridge.ts`, `src/databridge/index.ts` |
...
| DataBridge 适配层 | `src/databridge/index.ts` |

...
change_log:
  - version: v1.1.0
    changes: "2026-08-15 系统性核对：与代码权威源 src/config/dbConfig.ts 二次对齐（DB_VERSION v32、STORE_NAME 50、MODULE_ID 20、ENVELOPE_TARGET 11、ENVELOPE_ACTION 80+）；标注 7 份 DEPRECATED 域字典；登记 reference/{cockpit,news,data-collection}/data-definition.md 与 explanation/design/ 下的现行副本；与《V9核心数据字典与类型定义（整合版）》.md v1.5 保持一致。"
    date: 2026-08-15
  - version: v1.0.0
...
> **状态**：✅ P0 新增索引；✅ 2026-07-12 **同名 3 份已合并为 1 份主字典**（整合版 v2.0.0）；✅ 2026-08-15 与代码权威源 `src/config/dbConfig.ts` 二次对齐。
> **实测（2026-08-15）**：共 21 份匹配文件 = **1 份主字典** + **1 份全局整合版** + **3 份 reference 子目录归档** + **5 份独立域定义（design/）** + **5 份 DEPRECATED 副本（explanation/ 与 reference/）** + **2 份索引（reference/ + explanation/design/）** + **若干历史归档**。
> **代码权威源**：`DB_VERSION=32` · `STORE_NAME=50` · `MODULE_ID=20` · `ENVELOPE_TARGET=11` · `ENVELOPE_ACTION=80+`（见 [dbConfig.ts](file:///d:/FinSightV9/src/config/dbConfig.ts)）。
...
### 2.1 现行版本（active，2026-08-15 已核对）
...
## 4. 代码权威源对齐（2026-08-15 核对）

| 维度 | 字典值 | 代码值 | 一致性 |
|------|--------|--------|--------|
| `DB_NAME` | `V6ProDB` | `V6ProDB` | ✅ |
| `DB_VERSION` | v32 | `32`（`src/config/dbConfig.ts:6`） | ✅ |
| `STORE_NAME` 数量 | 50 | 50 | ✅ |
| `MODULE_ID` 数量 | 20 | 20 | ✅ |
...
- ✅ **同名 3 份已合并为 1 份主字典**（目标达成：裸名 `data-definition.md` 仅 `docs/reference/` 一处）。
- ✅ 整合主字典已纳入 `docs/README.md` C 类 / `../meta/GOVERNANCE.md` 引用。
- ✅ 新增按域拆分数据字典仍须带域前缀命名并登记于此索引（见 §5）。
- ✅ 2026-08-15 系统性核对：所有 frontmatter `last_updated` 已对齐本日，关键数字与代码权威源一致。

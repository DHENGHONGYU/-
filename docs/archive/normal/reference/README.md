...
> **维护规则**：新增/移动文档后必须同步更新本索引（见 `../meta/GOVERNANCE.md` 保鲜规则）。
...
| 类 | 名称 | 职责 | 现有锚点 | 状态 |
|----|------|------|----------|------|
| **A** | 导航与治理 | 索引、宪法、归类、体检、需求规格、插件集成 | `README.md`(本文件)、`../meta/GOVERNANCE.md`、`<宸插綊妗?`、`<宸插綊妗?`、`<宸插綊妗?`、`<宸插綊妗?`、`../specs/01-vision-and-goals.md`、`./v9数据宪法.md`、`./index.md` | ? |
...
| **C** | 功能模块 | 各舱 spec、组件体系、Widget、Store、服务契约、数据层、数据字典 | `./input-cabin-spec.md`、`./analysis-cabin-spec.md`(P0?)、`./trading-cabin-spec.md`(P0?)、`./output-cabin-spec.md`(P0?)、`./command-cabin-spec.md`(P0?)、`./atomic-component-system.md`、`../guides/widget-development-guide.md`、`./api-contract.md`、`./data-dictionary-index.md` | ? |
...
| **E** | 测试策略 | 单元/e2e/覆盖率、测试用例、门禁 | `../guides/testing-strategy.md`(P1?)、`./test-catalog.md`、`../explanation/production-release-checklist-skill.md` | ? |
...
| **G** | 过程与质量产物 | 报告、审计、changelog、草稿、发布管理 | `docs/reports/audit/`（自动产物）、`docs/reports/changelogs/`、`CHANGELOG.md`(根)、`<宸插綊妗?`(P1?)、`../reports/project-management/v9-post-dev-review.md`、`../explanation/design/发布计划与评审-r01.md` | ? |
| **H** | 跨域补充 | 入门、How-to、安全、部署、i18n | `../guides/getting-started.md`(P1?)、`../guides/how-to-add-widget.md`(P1?)、`../guides/how-to-add-store.md`(P1?)、`../guides/how-to-add-service.md`(P1?)、`../guides/how-to/mcp-acl-guide.md`、`../explanation/runbook.md`(P2?)、`../explanation/a11y-i18n.md`(P2) | ? |
...
| 场景 | 推荐文档路径 |
|------|-------------|
| **新成员 30 分钟上手** | `../guides/getting-started.md` → `../explanation/overview.md` → `C/*-cabin-spec.md` |
| **新增 Widget** | `./widget-integration-checklist.md` + `../guides/how-to-add-widget.md` + `./atomic-component-system.md` |
...
| **查安全架构** | `./security-model.md` → `../guides/how-to/mcp-acl-guide.md` → `../../AGENTS.md` §三/六/八 |
| **查部署基线** | `./deployment.md` → `../explanation/runbook.md` → `.github/workflows/quality-check.yml` |
| **查 AI 工程治理** | `./README.md` → `../archive/README.md` → `prompts/service-integration-guide.md` |
...
- **自动产物**：`reports/`（由 `<宸插綊妗?` 管理保留期，CI 生成）
- **本次新增 / 更新文档清单（全量）**：
  - `./security-model.md` — 安全模型总览（L1-L7 纵深防御）
  - `./deployment.md` — 部署基线（纯前端 SPA + CI/CD）
  - `../README.md` — AI 工程治理入口（LLM 调用栈 / Agent 系统 / 提示词工程）
  - `../guides/testing-strategy.md` — 三层测试策略 v2.0.0（金字塔 / 契约 / 覆盖率）
  - `../meta/GOVERNANCE.md` — 文档治理宪法（A–H 归类 / 版本规范 / 归档规则）
  - `<宸插綊妗?` — 自动产物清理周期表（保留矩阵 / 清理脚本）
...
| 二级子类 | 归属一级类 | 说明 | 代表文档 |
|----------|-----------|------|----------|
| **A2 需求规格** | A | 愿景、功能规格、目标 | `../specs/01-vision-and-goals.md`、`../specs/02-functional-specs.md` |
...
> **命名规范**：二级子类文档命名建议加子类前缀，如 `../specs/01-vision-and-goals.md`、`./data-dictionary-index.md`（可选，不强制）。

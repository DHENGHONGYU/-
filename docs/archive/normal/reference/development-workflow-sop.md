...
优先使用知识图谱和 AI 记忆，避免重复搜索已存在的依赖关系：
...
**上下文锚点防漂移规则**：每次工具调用前对照三个锚点：
1. 当前操作是否服务于 `rootTask.intent`？
2. 当前操作是否超出 `phase` 边界？
3. 工作区状态是否与 `contextAnchor` 一致？
...
| 架构升级 | 25% | 是否符合分层架构原则 |
| 可维护性 | 25% | 代码复杂度、认知负荷、团队协作效率 |

**决策阈值**：≥ 70 分建议拆分；50-69 分谨慎评估；< 50 分不建议拆分。
...
cockpit/                →  可依赖 components/ + store/ + services/
mcp/                    →  可依赖 core/ + data/ + lib/ + services/
...
- **禁止绕过**：禁止直接调用 `MCPServer.callTool()`，必须走 `mcpBridge.callTool`

新增 Server/Tool 权限配置 SOP：
1. 在 `src/config/mcpAclMatrix.ts` 的 `MCP_ACL_MATRIX` 中配置规则
2. 写操作类 Tool 不要加入 `ui` 角色的 `allowedServers`
3. 查询类 Tool 使用 `'get_*'` / `'list_*'` 通配符
4. 更新 `src/mcp/__tests__/mcpAclInterceptor.test.ts`
...
- ✅ 存储优先本地轻量引擎（IndexedDB / DuckDB / LanceDB）
- ✅ 所有云端端点路径集中在 `src/config/`
- ✅ 实时性 SLA 由 `src/constants/cockpit.constants.ts` 的 `REALTIME_SLA_MS` 约束

---

## 四、Phase 3: 编码后（Commit & Quality Gates）

### 4.1 Husky 预提交 14 步阻断门禁 + 1 warn
...
| 步骤 | 门禁名称 | 目的 | 阻断性 |
|------|----------|------|--------|
| 1/10 | lint-staged | 暂存区 ESLint --fix | ✅ 阻断 |
| 2/10 | lint:colors | 颜色硬编码扫描 | ✅ 阻断 |
| 3/10 | tsc:prod | TypeScript 零错误 | ✅ 阻断 |
| 5/14 | audit:layers | 跨层调用检查 | ✅ 阻断 |
| 6/14 | audit:atomic | 原子组件边界 | ✅ 阻断 |
| 7/14 | file:check | 文档规范检查 | ✅ 阻断 |
| 8/14 | audit:docs | 代码-文档同步 + 版本漂移检查 | ✅ 阻断 |
| 9/14 | verify:tokens | 设计令牌映射 | ✅ 阻断 |
| 10/14 | audit:tokens | 令牌消费检查 | ✅ 阻断 |
...
| 月度维护 | `npm run doc:freshness` / `doc-freshness.yml` cron | 保鲜度评分 + 告警 | 非阻塞 |

**新增脚本/门禁的注册义务**：任何新脚本若进入 SOP §4.1 表格或 §8 工具链地图，必须同时声明其触发事件（Husky / CI / 手动 / 定时），否则视为未完成的接入。
...
| L2 | 集成测试 | 跨模块交互 | `npx vitest run src/mcp/__tests__/` | ≥ 60% |
...
| 命令 | 频率 | 用途 |
|------|------|------|
| `npm run audit:deadcode` | 每周 | 死代码清理 |
| `npm run audit:mcp` | 每次 MCP 变更 | Server 利用率 |
| `npm run audit:widget-registry` | 每次 Widget 变更 | 注册一致性 |
| `npm run anomaly:detect` | 每周 | 质量异常检测 |
...
所有任务完成后必须生成结构化日志，存储于 `docs/changelogs/YYYY-MM/`：

| 必含项 | 说明 |
|--------|------|
| 任务状态与进度 | 各 phase 完成状态 |
| 人机交互记录 | 关键决策点和确认 |
| 文件变更详情 | 新增/修改/删除文件清单 |
| 技术决策记录 | 为何选择此方案 |
...
代码变更后必须同步更新文档：
- 类型定义变更 → 数据字典 / API 契约
- 路由变更 → `../explanation/06-routing-specs.md`
...
| 类型 | 示例 | 是否需要人工确认 |
|------|------|----------------|
| ✅ 自主执行 | 代码格式化、ESLint 自动修复、补充单元测试、硬编码颜色提取 | 否 |
| ⚠️ 人工确认 | 跨模块重构（3+ 模块）、数据库 Schema 变更、接口签名变更 | 是 |
| ❌ 人工决策 | 技术栈更换、核心算法重构、安全策略变更 | 必须 |
...
文档
  ...
  ├── 双向完整性: npm run audit:doc-integrity
...
- **新增门禁步骤** → 更新 §4.1 表格，并在 §4.1.1 触发事件矩阵中登记触发源
- **新增脚本** → 更新 §5 速查表、§8 工具链地图，并接入至少一个触发事件（Husky / CI / 定时 / 手动）
- **新增 AI 行为约束章节** → 更新 §2/§3 相关检查清单
- **变更提交流程** → 需经过 `npm run audit:docs`、`npm run audit:doc-integrity` 和 `npm run file:check` 验证
- **CI 工作流变更** → 同步更新 §4.1.1 触发事件矩阵

---

> **最后验证**：本文档更新后，运行以下命令确认 SOP 与当前工具链一致：
> ```bash

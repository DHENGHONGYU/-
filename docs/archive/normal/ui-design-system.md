...
change_log:
  ...
  - version: v2.2.2
    ...
    date: 2026-08-22
- version: v2.2.1
    changes: "基准日校对(2026-08-22)：R1取真值(P2 正文版本声明行=v2.2.0) → R2 PATCH++(v2.2.1) / last_updated 刷新 / change_log 闭环"
    date: 2026-08-22
---
...
|----------|------|
| **专业感** | 低饱和度配色，传递信任与稳重 |
| **可读性** | 优化深色主题，降低长时间使用的视觉疲劳 |
| **数据表达** | 涨跌色彩语义化，快速传达盈亏信息 |
...
--warning: 38 92% 50%;           /* #F5A623 - 警告黄 */
--info: 205 80% 55%;             /* #2D9CDB - 信息蓝 */
```
...
--chart-2: 165 35% 55%;          /* 品牌绿 */
--chart-3: 38 60% 50%;           /* 品牌金 */
...
```css
--shadow-sm: 0 1px 2px hsl(220 14% 95% / 0.5);    /* 微妙的卡片边界 */
...
| `scale-in` | 淡入 + 缩放 0.96→1 | 300ms |
| `slide-in-right` | 淡入 + 右移 16px | 300ms |
| `pulse-glow` | 脉冲发光效果 | 2s infinite |
| `skeleton-shimmer` | 骨架屏微光闪烁 | 1.5s infinite |
...
| 变体 | 使用场景 | 样式特征 |
|------|----------|----------|
| `primary` | 主要操作 | 渐变背景 (primary→primary/90)、悬浮阴影+提升 |
| `success` | 成功操作 | 渐变背景 (positive→positive/90) |
| `danger` | 危险操作 | 渐变背景 (destructive→destructive/90) |
| `secondary` | 次要操作 | 柔和灰背景 |
| `outline` | 中性操作 | 2px 边框、hover 背景 |
...
```css
.dark {
  --background: 222 47% 8%;        /* 深蓝黑背景 */
  --foreground: 210 40% 98%;      /* 高对比度文字 */
  --card: 222 47% 11%;            /* 卡片略浅 */
  --primary: 195 80% 50%;         /* 更饱和的主色 */
  --positive: 152 65% 55%;        /* 更鲜明的盈利绿 */
  --negative: 0 70% 60%;          /* 更鲜明的亏损红 */
}
```
...
1. **禁止直接使用 HEX 颜色** - 使用 CSS 变量或 Tailwind 语义类
2. **涨跌信息必须使用语义色** - `text-positive` / `text-negative`
3. **交互元素需要过渡动画** - 使用 `transition-base` 类
4. **深色主题优先** - 设计时考虑深色模式兼容性

---

## 11. 页面级 UI 组件清单（按页面归档）
...
- **依赖配置**：`src/config/collectConfig.ts`（策略模板、维度默认值、频率/数据源/存储类型/重要性标签）
- **测试覆盖**：61 个组件测试用例（`tests/__tests__/SevenDimConfigPage.test.tsx`）
...
- `StrategyCard`：策略模板卡片，展示模板名称、描述、维度数量、选中态
- `DimensionRow`：维度行，展示维度名称、重要性、频率、数据源、存储策略、启用开关

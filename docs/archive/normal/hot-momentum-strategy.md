...
f2 资金因子优先使用真实主力净流入数据，通过三层降级确保可用性：

| 层级 | 数据源 | 触发条件 | 数据时效 |
|:---|:---|:---|:---|
| L1（优先） | AKShare `stock_fund_flow_industry`（同花顺源） | 盘中实时调用 | 实时 |
| L2（兜底） | hexin-v 手动绕过（`_ths_hexinv.py` + requests 直连同花顺 HTTP） | L1 失败/被反爬 | 实时 |
| L3（缓存） | SQLite 最近 5 日 `net_amount` 均值（`_sector_fund_flow_db.py`） | L1+L2 均失败 | 盘后/历史 |
| L4（保底） | `amt_ratio`（5日均成交额/20日均成交额）量价代理 | L1+L2+L3 全失败 | 量价代理 |

**行业名称匹配**：同花顺行业名称与申万二级名称通过 `_normalize_sw_name` 双向归一化匹配（如 "白酒II" → "白酒"）。

**匹配率阈值**：
- 匹配率 ≥ 50%：使用真实净流入，未匹配板块填 0（中性净流入）
- 匹配率 < 50%：整体降级 L4，避免量纲混合失真（净流入 -100~+100亿 vs amt_ratio 0.5~2.0）

**持久化**：L1/L2 每次成功都写入 SQLite（`outputs/sector_fund_flow.db`），供 L3 读取。保留 30 日数据。

**实现文件**：
- `python/data_service/_ths_hexinv.py`：hexin-v 验证码生成（30s TTL 缓存 + py_mini_racer JS 引擎）
- `python/data_service/_sector_fund_flow_db.py`：SQLite 读写（upsert/5日AVG/cleanup）
- `python/data_service/_sector_fund_flow.py`：三层降级编排（L1→L2→L3 + try/except 防护）
- `python/data_service/collect_endpoints.py` `fetch_sector_rotation_scores`：f2 接入点

> 参考案例：cwjcw/quantitative_trading 分层架构思路（Tushare 日频 + AKShare 盘中实时）；
> cloud.tencent.com/developer/article/2659468 子晓AI量化 hexin-v 实战方案。
...
| 层 | 文件 | 变更 |
|:---|:---|:---|
| Python 后端 | `python/data_service/collect_endpoints.py` | 新增 `/api/collect/sectors`，接 AKShare 申万板块接口，计算五因子；f2 接入三层降级 |
| Python 后端 | `python/data_service/_ths_hexinv.py` | 新增：f2 L2 层 hexin-v 验证码生成（30s TTL 缓存 + py_mini_racer） |
| Python 后端 | `python/data_service/_sector_fund_flow_db.py` | 新增：f2 L3 层 SQLite 持久化（5日AVG/cleanup） |
| Python 后端 | `python/data_service/_sector_fund_flow.py` | 新增：f2 三层降级编排（L1→L2→L3 + try/except 防护） |

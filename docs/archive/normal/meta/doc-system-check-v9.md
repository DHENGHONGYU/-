...
> **体检对象**：`<宸插綊妗?`（v1.0.0）+ 全量 388 份 `.md` 文档资产（较初检 +32 份）  
...
> **更新**：经全量 388 文件重新检索，`../reference/api-contract.md`（`docs/specs/design/`）和 `quality-gates` 系列（`../guides/09-quality-gates.md`、`../explanation/quality-gates-baseline.md`）实际存在，原 23 个缺口修正为 **12 确认缺失 + 5 可能已存在**。详见 §缺陷 2。
...
|----------|----------|--------|----------|------|
| `../reports/changelogs/README.md` | B | P1 | 1h | |
...
| `../reference/prompts/store-integration-guide.md` | F/C | P1 | 2h | |
| `../reference/prompts/service-integration-guide.md` | F/C | P1 | 2h | |
...
| `../guides/getting-started.md` | `../guides/getting-started.md` | ✅ 已创建（P1） | `<宸插綊妗?` |
| `../guides/how-to-add-widget.md` | `../guides/how-to-add-widget.md` | ✅ 已创建（P1） | `find docs/guides -name 'how-to-add-widget.md'` |
| `../guides/how-to-add-store.md` | `../guides/how-to-add-store.md` | ✅ 已创建（P1） | `<宸插綊妗?` |
| `../guides/how-to-add-service.md` | `../guides/how-to-add-service.md` | ✅ 已创建（P1） | `find docs/guides -name 'how-to-add-service.md'` |
...
| `../explanation/quality-gates-baseline.md` | `../guides/09-quality-gates.md` + `../explanation/quality-gates-baseline.md` ✅ | 部分满足 | 前者偏设计阶段门禁，后者偏基线数值；需确认是否需合并为统一的 `quality-gates.md` |
| `./governance.md` | `../explanation/design/implementation-governance.md` + `<宸插綊妗?` + `../reference/complexity-governance.md` | ~~不满足~~ → ✅ **已独立创建** | ~~匹配到的都是**专项治理**文档~~；**2026-07-12 已新建独立 `governance.md` v1.0.0** |
| `../reports/changelogs/README.md` | `file-management-system/templates/ADR-template.md` | 不满足 | 匹配的是**模板文件**，非实际 ADR 主索引；ADR 主索引仍缺失 |
...
- **愿景/功能规格**：`../specs/01-vision-and-goals.md`、`../specs/02-functional-specs.md`（体系未设「需求规格」子类，A 类只有「导航与治理」）。
...
| `docs/specs/design/` | 设计文档 | 仍混入 `<宸插綊妗?`、`<宸插綊妗?`、`../explanation/design/test-expansion-design.md`、`quality-audit-plan.md` 等审计/测试/过程文档 | ~25 | 较原 37 份部分迁移 |
...
|-------------|-----------|-------------|--------|
| **A2 需求规格** | A | `../specs/01-vision-and-goals.md`、`../specs/02-functional-specs.md` | 高 |
...
| **定期清理机制** | 无 `<宸插綊妗?` | 仍缺失 | drafts/、reports/_generated/ 无自动清理规则 |
...
| 契约要求 | 初检缺口 | 修订状态 |
|----------|----------|----------|
| 新增页面必须同步更新 `ROUTE_REGISTRY` 和 `../explanation/06-routing-specs.md` | 无 `<宸插綊妗?` 操作指南 | 仍缺失 |
| 修改 `UserType` 不得破坏 `user-type.spec.ts` | 无 `<宸插綊妗?` | 仍缺失 |
| 所有 `useEffect` 清理模板（4 个） | 无 `<宸插綊妗?` | 仍缺失 |
...
| 13 | **制定 `<宸插綊妗?`** | 🔴 未开始 | 定义 drafts/（保留 7 天）、reports/_generated/（保留 30 天）、changelogs/（永久保留）的清理周期 |
| 14 | **补充操作指南（How-to）** | ✅ 已完成 | `docs/guides/how-to-*.md` 系列（Widget/Store/Service）+ `../guides/getting-started.md` 已补齐（2026-07-12） |
...
17. **文档保鲜度 Dashboard**：将 `../explanation/design/freshness-alerts.md` 升级为自动更新的 GitHub Actions / CI 产物。
...
- **DocTaxonomy V9**：本文体检对象（`<宸插綊妗?` v1.0.0）
...
| 路由注册 | ✅ 66/66 100% 覆盖 | 🟡 `../explanation/06-routing-specs.md` 存在但孤立 | 基本一致 | 低 |
...
| **指南落差** | 48 个 Store 已运行，但无 `../guides/how-to-add-store.md` | ✅ **已补齐**：how-to-add-*.md 系列已创建（2026-07-12） | 新增 Store 时有标准 SOP 可参考 |
...
| `../guides/how-to-add-widget.md` | P2 | **P1** | **P1** | 代码完备后，新增 Widget/Store/Service 成为高频开发任务 |
| `../guides/how-to-add-store.md` | P2 | **P1** | **P1** | 同上 |
| `../guides/how-to-add-service.md` | P2 | **P1** | **P1** | 同上 |
| `../guides/getting-started.md` | P1 | **P1** | **P1** | 新人 onboarding 第一站；与 README 互补 |

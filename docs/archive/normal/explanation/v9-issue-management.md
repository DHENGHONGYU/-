---
doc_id: V9-DOC-EXP-969
title: v9-issue-management
...
> 归并来源：`v9-issue-execution-board.md` + `v9-issue-resolution-schedule.md` + `v9-parallel-task-schedule.md`  
> 归并日期：2026-06-27  
...
| 问题ID | 责任Agent | 牵头Agent | 涉及文件 | 状态 |
|--------|-----------|-----------|----------|------|
| ARCH-001 | Architecture-Fix | Architecture-Fix | `src/App.tsx`, `src/services/system/bootstrapService.ts` | ✅ 已完成 |
| ARCH-002 | Architecture-Fix | Architecture-Fix | `src/services/*/*Service.ts` 等 13+ 文件 | ⏳ 待执行（批次1） |
...
| DOC-001~007 | Doc-Sync-Fix | Doc-Sync-Fix | 见详情 | ✅ 全部已完成 |
...
|----------|------------|--------|----------|----------|
| JT-001 | Architecture-Fix | Interaction-Fix | `src/App.tsx` | ARCH-001 与 INT-007 均修改同一文件 |
| JT-002 | Architecture-Fix | Data-Flow-Fix | `src/services/trading/tradingService.ts` | ARCH-002 与 DF-002 均修改同一文件 |
| JT-003 | Architecture-Fix | Data-Flow-Fix | `src/services/system/v6MigrationService.ts` | ARCH-003 与 DF-007 均修改同一文件 |
| JT-004 | Architecture-Fix | Interaction-Fix | `src/pages/analysis/IndustryScorePage.tsx`, `IntelligentScorePage.tsx` | ARCH-007 与 INT-006 均修改同一文件 |
...
| `src/core/databridge.ts` | 广播目标从 `meta.target` 改为 `targetStore` | test ✅ / lint ✅ / build ✅ |
...
git checkout src/App.tsx src/services/system/bootstrapService.ts \
...
# Data-Flow-Fix 修改的文件
git checkout src/core/databridge.ts src/core/dataflow/dataflowEngine.ts \
...
git checkout src/apps/trading/TradingApp.tsx src/apps/input/HotSectorPanel.tsx \

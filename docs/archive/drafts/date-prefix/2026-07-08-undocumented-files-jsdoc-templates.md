 ...
 *
 * @since v2.0.0
 * @compliance AGENTS.md §一 分层规则：components 层仅依赖 services 和 store
 */
```
 ...
 *
 * @since v2.0.0
 * @compliance AGENTS.md §一 分层规则：services 层仅依赖 core/、data/ 和 lib/
 */
```
 ...
 *
 * 评分逻辑：
 * - scoreMoat：基于毛利率、营收同比、ROE 计算护城河评分（1-5 分）
 * - scoreCompetition：基于毛利率水平推断竞争格局趋势，结合营收增长评分
 *
 * @since v2.0.0
 * @compliance AGENTS.md §一 分层规则：services 层仅依赖 core/、data/ 和 lib/
 */
 ...
 *
 * 负责订阅 DataBridge 上的 signals 和 orders 事件，
 * 当收到新信号或订单变更时，自动触发执行计划的创建或刷新。
 *
 * 核心功能：
 * - 订阅 signals store 的 insertSignal 事件，自动创建执行计划
 * - 订阅 orders store 的 insertOrder/updateOrder 事件，触发执行计划刷新
 * - 订阅 execution 相关的 saveExecutionPlan/updateExecutionPhase 事件，触发刷新
 * - 使用防抖（100ms）避免频繁刷新
 *
 * @compliance AGENTS.md §一 分层规则：store 层仅依赖 services 和 core
 */
...
1. 根据文件路径找到对应的模板
2. 将模板复制到文件头部（如果文件已有注释，合并或替换）
3. 根据实际代码内容补充或修改文档
4. 确保每个导出函数/接口都有 JSDoc 注释
5. 运行 `npm run audit:docs` 验证文档同步状态

### 文档检查清单

```markdown
[ ] 文件头部有 @module 注释
[ ] 文件头部有 @description 说明
[ ] 所有导出接口/类型有 JSDoc 注释
[ ] 所有导出函数有 JSDoc 注释（含 @param、@returns、@example）
[ ] 包含 @compliance 标注符合 AGENTS.md 分层规则
[ ] 包含 @since 版本信息（如有）
```
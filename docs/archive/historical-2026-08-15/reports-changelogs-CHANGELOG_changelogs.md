...
summary: "遵循\"变更即记录（Change as Record）\"原则，每次架构/数据变更均在此留下审计痕迹。已归档（76 条断链）。"
...
## v2.8.0 (2026-07-15) — 评分机制 P0 修复：数据自动联动 + 换手率补齐 + 财务数据校验

**变更范围**：评分机制三大高风险问题修复，打通数据采集→评分计算闭环

### 1. 数据采集与评分自动联动

| 文件 | 变更内容 |
|------|---------|
| `src/services/scoring/scoreAutoTrigger.ts` | **新增**：评分自动触发服务，监听 COLLECTION_EVENTS.COMPLETE 事件，维度白名单过滤，5s 去抖，最大 3 并发 |
| `src/main.tsx` | 注册 `setScoreTriggerServices` 并启动 `scoreAutoTrigger` |

**核心特性**：
- 事件驱动：数据更新后自动触发 V6 评分重算
- 维度白名单：01基本信息 / 02 K线 / 03筹码 / 06行业竞品
- 去抖 + 并发控制：避免频繁重评，保护性能
- 服务注入模式：与 feedbackOrchestrator / pipelineScheduler 一致

### 2. QuoteData 换手率字段补齐

| 文件 | 变更内容 |
|------|---------|
| `src/data/types/types.marketData.ts` | KlineBar 新增可选 `turnoverRate` 字段 |
| `src/services/data-collector/directDataAPI.ts` | 网易 K 线解析器提取第 10 列换手率数据 |
| `src/services/scoring/v6-engine/types.ts` | `quotesToQuoteData()` 新增 `avgTurnover20d` 计算（20 日均，至少 10 个有效点） |

**受益层**：L8 筹码分析的 PCH（筹码集中度）、AII（庄家吸筹强度）、MATRIX（博弈态势）、DIV（散户游资辨识度）四个指标由"数据不足"变为"有效计算"。

### 3. 评分输入数据契约校验

| 文件 | 变更内容 |
|------|---------|
| `src/services/scoring/scoringInputValidation.ts` | **新增**：评分输入三层校验（财务/基础/行情），warn-only 模式 |
| `src/services/scoring/v6ScoreService.ts` | `buildEngineInput()` 中集成校验调用 |

**校验覆盖**：
- 财务数据：18 字段有限性 / 区间 / 正负验证
- 基础数据：symbol/name 必填、价格类非负
- 行情数据：最新价正、收益率/波动率/换手率有限

### 验证结果

| 验证项 | 结果 |
|--------|------|
| `npx tsc --noEmit` | ? 新增代码无类型错误 |
| `npx vitest run` | ? 378 文件 / 5360 测试通过 |
| `npm run audit:layers` | ? 0 violations |
| `npm run audit:hardcode` | ? 0 违规 |
| `npm run audit:deadcode` | ? 0 违规 |

---

## v2.7.0 (2026-07-15) — 多阶段优化：UI架构/交互状态/数据流/安全/性能/自适应密度/文档同步

**变更范围**：七阶段全面优化，涵盖 UI 架构基础、交互状态标准、数据流优化、安全特性显性化、性能优化、渐进披露与自适应密度、文档同步与发布准备

### Phase 1: UI 架构基础优化

| 文件 | 变更内容 |
|------|---------|
| `src/components/cockpit/SignalSpectrum.tsx` | 创建全局统一"信号强度"母题组件，0-100评分映射弱(琥珀)→中(蓝)→强(翠绿)连续谱 |
| `src/components/atoms/index.ts` | 导出 SignalSpectrum 组件，注册为全局原子组件 |
| `src/core/ThemeProvider.tsx` | 暗色主题默认启用，用户偏好持久化到 localStorage |

### Phase 2: 交互状态标准

| 文件 | 变更内容 |
|------|---------|
| `src/components/atoms/index.ts` | 统一导出四态组件（Loading、Empty、ErrorState、Skeleton），规范全局交互状态 |

### Phase 3: 数据流优化

| 文件 | 变更内容 |
|------|---------|
| `src/core/dataflow/dataflowEngine.ts` | 迁移轮询通道为事件驱动模式，添加缺失通道（portfolio:risk、agent:logs），优化TTL配置与缓存策略 |
| `src/core/memoryCache.ts` | 独立内存缓存模块，支持TTL、LRU淘汰、容量上限与自动过期清理 |

### Phase 4: 安全特性显性化

| 文件 | 变更内容 |
|------|---------|
| `src/components/cockpit/SecurityStatus.tsx` | 创建安全状态组件，展示本地存储加密状态 |
| `src/components/cockpit/SecurityStatus.tsx` | 创建安全徽章组件，显性化安全优势 |
| `src/lib/localStorageCrypto.ts` | AES-GCM 256 加密辅助函数，CryptoKey派生与缓存 |
| `src/lib/localStorageManager.ts` | 统一localStorage封装，新增加密存储方法 |
| `src/components/atoms/index.ts` | 导出 SecurityStatus、SecurityBadge 组件 |

### Phase 5: 性能优化

| 文件 | 变更内容 |
|------|---------|
| `src/hooks/useStressTest.ts` | 压测触发Hook（页面→服务隔离层），将页面直接调用 `services/perf/stressTestService` 收敛到Hook，满足 audit:mcp 约束 |
| `src/services/workers/v6ScoreWorker.ts` | V6评分引擎Web Worker，评分计算offload到独立线程 |
| `src/services/workers/v6ScoreTaskScheduler.ts` | Worker池管理，批量评分任务分片并行执行，任务超时保护 |

### Phase 6: 渐进披露与自适应密度

| 文件 | 变更内容 |
|------|---------|
| `src/components/cockpit/DensityContext.tsx` | 信息密度控制上下文，提供compact/normal/expanded三种密度级别 |
| `src/components/cockpit/DensityToggle.tsx` | 密度切换组件，允许用户切换信息密度级别 |
| `src/components/atoms/index.ts` | 导出 DensityToggle 组件 |

### Phase 7: 文档同步与发布准备

| 文件 | 变更内容 |
|------|---------|
| `docs/reports/changelogs/CHANGELOG_changelogs.md` | 新增 v2.7.0 条目，记录七阶段完整变更 |
| `docs/meta/development-log.md` | 更新开发日志，记录各阶段实现细节与问题解决方案 |
| `docs/explanation/03-architecture-standards.md` | 更新版本号至 v2.7.0，补充新增模块说明 |
| `docs/explanation/10-glossary_explanation.md` | 更新版本号至 v2.7.0 |

### Phase 8: 文档→代码断裂引用修复与工具增强

| 文件 | 变更内容 |
|------|---------|
| `scripts/fix-doc-refs.ts` | v1.0→v1.1：新增目录引用（以 `/` 结尾）的 basename 模糊匹配修复能力 |
| `docs/meta/directory-structure-guide.md` | 修复 `src/schema/` → `src/data/schemas/`；版本 v3.1.0→v3.1.1 |
| `docs/meta/src-directories-evaluation-report.md` | 修复 `src/hooks/` → `src/hooks/` |
| `docs/reports/retrospectives/mcp-disabled-server-deep-dive.md` | 修复 `src/services/export/` → `src/services/export/` |
| `docs/meta/doc-manifest.csv` | 重新生成，594 条记录（核心 71 / 重要 300 / 参考 223） |
| `docs/meta/registry-index.md` | 从 doc-manifest.csv 重建，修复重复 frontmatter |
| `src/pages/command/agent/LlmManagement/hooks/useLlmConfigActions.ts` | 删除未使用函数 `capToInfinity`，消除 `tsc:prod` 错误 |

### Phase 8.5: 文档索引与版本戳记更新

| 文件 | 变更内容 |
|------|---------|
| `docs/meta/doc-manifest.csv` | 全量重建，同步最新 docs/ 目录状态 |
| `docs/meta/registry-index.md` | 从 CSV 重建，消除重复 frontmatter |
| `src/pages/command/agent/LlmManagement/hooks/useLlmConfigActions.ts` | 删除未使用函数 `capToInfinity` |

### Phase 9: 断裂引用批量收尾与发布基线巩固

| 文件 | 变更内容 |
|------|---------|
| `scripts/apply-doc-refs-review.ts` | 新增：读取 `doc-refs-manual-review.json` 并批量应用多匹配修复建议 |
| `scripts/fix-basename-root-refs.ts` | 新增：自动修复文档中对根文档（AGENTS.md / README_01-product.md / CHANGELOG_reference.md）的 basename 断裂引用 |
| `src/pages/command/agent/LlmManagement/components/LlmConfigTab.tsx` | 删除未导入的 `Key as _KeyIcon` 导出，修复 `tsc:prod` 错误 |
| `docs/meta/doc-manifest.csv` | 重新生成，595 条记录（核心 71 / 重要 302 / 参考 222） |
| `docs/meta/registry-index.md` | 从 CSV 重建 |
| `docs/meta/development-log.md` | 新增 Phase 9 记录，修复重复 frontmatter |
| `docs/reports/changelogs/CHANGELOG_changelogs.md` | 新增 Phase 9 记录，修复重复 frontmatter |
...
| code-to-doc | 94 | 93 | -1 |
| doc-to-code | 1,735 | 1,743 | +8（审计识别新增） |
...
- `npm run audit:deadcode` — 0 违规，38 处提示
| `npm run audit:docs` — 通过
| `npm run audit:doc-integrity` — 通过
| `npm run doc:version-check` — 585/588 一致（3 份未声明）
| `npm run build` — 构建成功（20s）

### Phase 10: 显式路径映射表与第三轮自动修复

| 文件 | 变更内容 |
|------|---------|
| `scripts/config/doc-ref-path-map.json` | 新增显式路径映射表：39 条 codePathMap + 54 条 docPathMap |
| `scripts/fix-doc-refs.ts` | v1.2：新增 `--use-path-map` 参数，优先使用显式映射修复断裂引用 |

**修复数据**：断裂引用从 3,014 降至 2,797（-217，-7.2%），多匹配待复核从 132 降至 88。

### Phase 11: 多匹配自动应用与映射表扩充

| 文件 | 变更内容 |
|------|---------|
| `scripts/fix/apply-multi-match-fixes.ts` | 新增：自动应用 `doc-refs-manual-review.json` 中的多匹配最佳建议 |
| `scripts/config/doc-ref-path-map.json` | 补充可确认映射，移除指向不存在目录的无效映射 |

**修复数据**：断裂引用从 2,797 降至 2,285（-512，-18.3%）。

### Phase 12: 硬编码清理收尾与测试类型修复

| 文件 | 变更内容 |
|------|---------|
| `src/constants/theme/theme.tokens.badges.ts` | 新增 BADGE_COLORS 令牌系统，聚合预测/搜索/告警等高频组合式 Tailwind 颜色类 |
| `src/constants/theme.tokens.ts` | 统一入口新增 `BADGE_COLORS` 导出 |
| `src/components/organisms/output/prediction/FactorDashboardPanel.tsx` | 使用 BADGE_COLORS 替换 CYCLE_COLORS/ALERT_COLORS/DIRECTION_COLORS 硬编码 |
| `src/components/organisms/output/prediction/PredictionPanel.tsx` | 使用 BADGE_COLORS 替换状态标签、方向文字、命中标记硬编码 |
| `src/components/organisms/search/GroupedView.tsx` | 使用 BADGE_COLORS.groupBorder 替换分组边框硬编码 |
| `src/components/organisms/search/TimelineView.tsx` | 使用 BADGE_COLORS.statusBadge 替换状态徽章硬编码 |
| `src/services/data-sync/globalScheduler.ts` | 提取 `TRADING_SESSION_MINUTES` 常量，消除引擎层魔法数字 |
| `src/services/fetcher/contractValidation.ts` | 提取 `DAYS_PER_YEAR` / `MAX_INVENTORY_TURNOVER_DAYS` 常量 |
| `src/services/stock-analysis/scoringStrategy.ts` | 提取 `SIMULATED_LATENCY_MS` 常量 |
| `src/pages/input/CollectTask/hooks/useCollectionTaskStats.test.ts` | 修复 7 处 TypeScript 类型错误，8/8 单元测试通过 |
| `scripts/audit/audit-hardcode.ts` | 扩展 COLOR_EXEMPT_FILES 集合，新增 theme.tokens 子模块豁免 |

### 验证结果（Phase 12 全量验证）

| 验证项 | 命令 | 结果 |
|--------|------|------|
| 类型检查 | `npm run tsc:prod` | ? 0 错误 |
| 单元测试 | `npx vitest run src/pages/input/CollectTask/hooks/useCollectionTaskStats.test.ts` | ? 8/8 通过 |
| 跨层调用审计 | `npm run audit:layers` | ? 0 violations |
| 硬编码审计 | `npm run audit:hardcode` | ? 通过 |
| 死代码审计 | `npm run audit:deadcode` | ? 0 违规，50 处提示 |
| 文档同步审计 | `npm run audit:docs` | ? 通过 |
| 文档完整性审计 | `npm run audit:doc-integrity` | ? 通过 |
| 文档版本检查 | `npm run doc:version-check` | ? 593/606 含 code_version |
| 构建验证 | `npm run build` | ? 成功，10.63s |

### 关键设计决策

1. **信号强度母题统一**：SignalSpectrum 组件作为全局信号强度标准，确保评分展示一致性
2. **加密存储显性化**：SecurityStatus 组件让用户直观了解数据安全状态
3. **信息密度自适应**：DensityContext 支持三种密度级别，满足不同用户偏好
4. **Worker 并行计算**：评分计算 offload 到独立线程，避免主线程阻塞
...
| 文件 | 变更内容 |
|------|---------|
| `../../reference/data-dictionary-index_reference.md` | v1.5.0→v1.6.0：新增混合校对模块索引，包含 17 个类型定义 |
| `../../reference/03-architecture-standards_reference.md` | v2.5.0→v2.6.0：新增 §3.1.9 Hybrid Proofread 模块说明 |
| `../../reference/05-engine-specs_reference.md` | v2.5.0→v2.6.0：新增混合校对引擎说明 |
...
| `../../reference/03-architecture-standards_reference.md` | v2.3.0→v2.5.0：配置层新增 3 文件、服务层新增 3 个交易计算模块+2 个 UseCase、注册体系更新 |
| `../../reference/data-dictionary-index_reference.md` | v1.3.0→v1.4.0：新增 UseCase 类型索引、交易计算纯函数类型索引 |
| `../../reference/05-engine-specs_reference.md` | v2.2.1→v2.5.0：交易引擎目录新增 positionComputer/pnlComputer/riskComputer、新增 UseCase 层说明 |
...
| `src/mcp/servers/portfolio/portfolioServer.ts` | 数据导出 MCP Server（回测报告导出 PDF/Excel） |
...
| `src/mcp/servers/portfolio/portfolioServer.ts` | ExportServer 单元测试（5 个用例） |
| `src/mcp/servers/data-collector/dataCollectorServer.ts` | InputServer 单元测试（10 个用例） |
| `src/mcp/servers/trading/tradingServer.ts` | TradeServer 单元测试（7 个用例） |
...
| 文件 | 变更内容 |
|------|---------|
| `../../reference/v9-indexeddb-store-schema.md` | v16→v21：补充 v16→v21 版本历史、Store 总数 24→25、新增 trade_reviews Store |
| `./CHANGELOG.md` | 新增 v2.4.0 条目：记录 MCP Server 体系完善与审计脚本升级 |
...
| 文件 | 变更内容 |
|------|---------|
| `../../reference/registry-index_reference.md` | 新建：四层注册体系核心文档 |
| `../../reference/03-architecture-standards_reference.md` | v2.2.1→v2.3.0：新增 §3.1.8 四层模块注册体系、Widget 目录 +7、偏差 D19 标记已修复 |
| `../../how-to/widget-development-guide.md` | v1.0.0→v1.1.0：新增 §7 已注册 Widget 清单（19 个）、§7.3 Widget 错误隔离说明 |
| `../../reference/testing-strategy_reference.md` | v1.0.0→v1.1.0：新增 §9 注册体系测试策略、更新测试基线（649+ 用例）、Widget 错误边界测试模板 |
| `../../reference/data-dictionary-index_reference.md` | v1.2.0→v1.3.0：新增 Registry 模块索引、3 个注册表类型文件条目 |
| `../../reference/10-glossary_reference.md` | v2.2.1→v2.3.0：新增 §10.11 四层注册体系术语（8 条） |
| `../../reference/changelogs/2026-07/2026-07-05-module-registry-and-integration_2026-07.md` | 新建：结构化变更日志 |
...
| `../../reference/03-architecture-standards_reference.md` | P0/P1/P2 修正 | DB_VERSION 14 → 21、Store 清单 19 → 25 个、Widget 数量 12 → 21、技术栈 Pinia → Zustand、版本号 v1.1.0 → v2.2.1、偏差清单 D13/D14/D16 状态修正（?? → ??）、UnifiedStockData 状态修正（?? → ?）、Widget 引擎接入状态修正（?? → ?） |
| `../reports/audit/quality-audit-plan_audit.md` | P1 修正 | 技术栈 Pinia → Zustand、路由 26 → 47、Store 7 → 44、Widget 12 → 21 |
| `docs/README_root.md` | P2 修正 | 版本号 v0.9.0-migration-implemented → v2.2.1、更新日期 2026-06-24 → 2026-07-05 |
| `../../01-vision-and-goals.md` | P2 修正 | 版本号 v0.9.0-migration-implemented → v2.2.1、更新日期 2026-06-25 → 2026-07-05 |
| `../../reference/02-functional-specs_reference.md` | P2 修正 | 版本号 v1.1.0 → v2.2.1、更新日期 2026-06-26 → 2026-07-05 |
| `../../reference/04-ui-ux-specs_reference.md` | P2 修正 | 版本号 v0.9.0-migration-implemented → v2.2.1、更新日期 2026-06-25 → 2026-07-05 |
| `../../reference/05-engine-specs_reference.md` | P2 修正 | 版本号 v1.1.0 → v2.2.1、更新日期 2026-06-26 → 2026-07-05 |
| `../../reference/06-routing-specs_reference.md` | P2 修正 | 版本号 v1.2.0 → v2.2.1、更新日期 2026-07-04 → 2026-07-05 |
| `../../reference/07-operation-strategy_reference.md` | P2 修正 | 版本号 v0.9.0-migration-implemented → v2.2.1、更新日期 2026-06-25 → 2026-07-05 |
| `../../reference/08-implementation-plan_reference.md` | P2 修正 | 版本号 v1.1.0 → v2.2.1、更新日期 2026-06-26 → 2026-07-05 |
| `../../reference/09-quality-gates_reference.md` | P2 修正 | 版本号 v1.2.0 → v2.2.1、更新日期 2026-06-29 → 2026-07-05 |
| `../../reference/10-glossary_reference.md` | P2 修正 | 版本号 v0.9.0-migration-implemented → v2.2.1、更新日期 2026-06-25 → 2026-07-05 |
...
1. **技术栈描述修正**
   - `../../reference/03-architecture-standards_reference.md` 和 `../reports/audit/quality-audit-plan_audit.md` 中 Pinia → Zustand
...
5. **数量修正**
   ...
   - `../../reference/03-architecture-standards_reference.md`：Widget 12 → 21

### P2 修正详情

1. **文档版本号统一**
   - 10 份核心文档（README_01-product.md、01-10）版本号统一为 v2.2.1，日期统一为 2026-07-05
...
| `../../reference/03-architecture-standards_reference.md` | 待修正 | DB_VERSION 14 → 21、Store 清单补充 6 个、技术栈 Pinia → Zustand |
...
2. **常见错误模式清单**（token-optimization-best-practices_lessons-learned.md）
   - 跨层调用违规（5 种典型场景）
   - 硬编码颜色/魔法数字（检测方法、修复方案）
   - 事件监听未清理（标准模板、检查清单）
   - any 类型使用（类型收窄方案）

3. **架构缺陷识别清单**（token-optimization-best-practices_lessons-learned.md）
...
1. **P0 问题修正**（待人工确认后执行）
   - 更新 `../../reference/03-architecture-standards_reference.md` DB_VERSION 14 → 21
...
### F2 文档版本校正
- `../../explanation/action-list.md` 升级至 v2.0.0，修正批次 A/B/C 虚假完成记录
...
| `../../reference/release-notes.md` | Docs | 版本发布说明文档 |
...
| `../../reference/feedback-loop-spec_reference.md` | 反馈闭环规格文档更新 |
...
| 文件 | 变更类型 | 变更内容 |
|------|---------|---------|
| `../../reference/03-architecture-standards_reference.md` | 修正 | §3.1.2 DataFlow 补充 4 接口（DataFlowModuleInput/Output, DataPacket, ChannelMeta） |
| `../../reference/03-architecture-standards_reference.md` | 新增 | §3.1.7 Page 生命周期（PageModuleInput/Output, PageGuard） |
| `../../reference/03-architecture-standards_reference.md` | 修正 | §3.8 DataBridge 适配层补充 5 接口（DataBridgeAdapterConfig, DataAction, BridgeQueryOptions, BridgeQueryResult, DataBridgeAdapterStats） |
| `../../reference/03-architecture-standards_reference.md` | 修正 | §3.7 数据层补充 34 个数据模型引用（DimensionScore, PortfolioHolding, StrategyClassification 等） |
| `../../reference/02-functional-specs_reference.md` | 版本升级 | v0.9.1 → v1.1.0 |
| `../../reference/05-engine-specs_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |
| `../../reference/06-routing-specs_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |
| `../../reference/08-implementation-plan_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |
| `../../reference/09-quality-gates_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |

### 数据字典变更

| 文件 | 变更类型 | 变更内容 |
|------|---------|---------|
| `../../trade/API_CONTRACT.md` | 修正 | §9.2b 新增 TradeSignal/TradingSignal 接口定义（8 字段 + SignalSnapshot 子类型） |
| `../../reference/data-definition.md` | 元数据 | 添加版本头部（Version: v1.1.0, Last Updated: 2026-06-26） |
| `../../AI_CENTER_DATA_DEFINITION.md` | 元数据 | 添加版本头部（Version: v1.1.0, Last Updated: 2026-06-26） |
| `../../trade/API_CONTRACT.md` | 元数据 | 添加版本头部（Version: v1.1.0, Last Updated: 2026-06-26） |
| `../../reference/data-definition.md` | 元数据 | 添加版本头部（Version: v1.1.0, Last Updated: 2026-06-26） |
| `../../reference/data-definition.md` | 元数据 | 添加版本头部（Version: v1.1.0, Last Updated: 2026-06-26） |
| `../../reference/data-dictionary-index_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |
| `../../reference/dataflow-data-definition_reference.md` | 版本升级 | v0.9.0 → v1.1.0 |
...
| `../../reference/v9-architecture-data-dictionary-validation-report_reference.md` | 一致性验证报告 |
| `./CHANGELOG.md` | 本文档 |
...
| 模块 | 文档 | 变更内容 |
|------|------|---------|
| Cockpit | `../../reference/data-definition.md` | 新建，Widget 框架 40+ 接口 + 枚举常量 |
| Cockpit | `../../reference/03-architecture-standards_reference.md` §3.1.4 | 目录结构替换为 12 个实际 Widget，新增 §3.1.4.1 采集流 |
| News | `../../reference/data-definition.md` | 新建，newsService/sentimentAnalyzer/stockLinker 5 接口 |
| News | `../../reference/02-functional-specs_reference.md` §2.1 | 新增新闻资讯模块 |
| Trading | `../../trade/API_CONTRACT.md` §9 | 新增交易服务层 9 接口 + 7 模块函数清单 |
| Trading | `../../reference/02-functional-specs_reference.md` §2.1 | 新增交易持仓管理模块 |
| AI Center | `../../AI_CENTER_DATA_DEFINITION.md` §5 | 补充 Agent 运行时类型（AgentModuleInput/Output, AgentDefinition, AgentInstance） |
| AI Center | `../../reference/02-functional-specs_reference.md` §2.1 | 新增 AI 智能体中心模块 |
| Data Collection | `../../reference/data-definition.md` | 新建，三层架构 8 接口 + 5 组枚举 |
| Data Collection | `src/services/data-collector/mockDataCollection.ts` | 新建，8 接口 Mock 数据生成器 |
| Architecture | `../../reference/03-architecture-standards_reference.md` | L5/L4 映射表补充 Widget 数量，L3 行补充 news 服务路径 |
| User Stories | `../../reference/02-functional-specs_reference.md` §2.4 | 新增 3 个用户故事（2.4.12~2.4.14） |
...
| 02 | `../../reference/02-functional-specs_reference.md` | 功能规格 |
| 03 | `../../explanation/03-architecture-standards.md` | 架构标准 |
| 04 | `../../reference/04-ui-ux-specs_reference.md` | UI/UX 规格 |
| 05 | `../../explanation` | 引擎规格 |
| 06 | `../../reference/06-routing-specs_reference.md` | 路由规格 |
| 07 | `../../reference/07-operation-strategy_reference.md` | 运营策略 |
| 08 | `../../explanation/design/08-implementation-plan_design.md` | 实施计划 |
| 09 | `../../reference/09-quality-gates_reference.md` | 质量门禁 |
| 10 | `../../explanation/10-glossary_explanation.md` | 术语表 |

...
| 评分结果类型 | `src/types/modules/scoring.types.ts` | 修改 | ScoreLayer 新增 `llmEnhanced: boolean` 字段 |
...
|--------|------------|---------------|---------|
| 高 | `src/mcp/servers/rbac/rbacServer.ts` | `services/rbac/*` | RBAC 用户/角色/权限管理，AI Agent 安全管理基础 |
| 高 | `src/mcp/servers/usecase/useCaseServer.ts` | `services/useCase/*` | 跨服务编排（统一股票视图、双策略运行），8 个 useCase 中 7 个未暴露 |
| 中 | `src/mcp/servers/ai-center/aiCenterServer.ts` | `services/ai-center/*` | 智能体调度中心（当前为孤立僵尸模块） |

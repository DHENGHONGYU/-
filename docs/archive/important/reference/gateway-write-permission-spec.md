...
summary: "将直接操作 dataLayer 的特权收敛到 Gateway 单一入口，DataBridge 通过 Gateway 间接写入"
...
1. **新增 Gateway**：创建 `src/core/databridge.ts`，把 `ACTION_TO_STORE_MAP` 从 `databridge.ts` 迁移过来，并对接 `dataLayer`。

...
- version: v1.0
  ...
  - version: v1.1
    ...
    date: 2026-08-13
  - version: v1.1.1
    changes: "P0-2 文档版本漂移治理：修正 frontmatter version 与 change_log 不一致（v1.0 → v1.1）、正文 §0 版本号与日期同步；标记 v9-system-blueprint.md 为 deprecated 并由本文档取代"
    date: 2026-08-15
---
...
| 构建 | Vite 6（`@vitejs/plugin-react`），内置 `mock-trade-api` 中间件模拟 `/api/v1/trade/*` |
| 状态 | Zustand（约 50+ store，跨 Tab 广播 `withBroadcast`） |
| 存储 | IndexedDB（本地自管，经 `dataLayer` 访问，无后端） |
| 样式 | Tailwind 3 + 设计令牌体系（L1–L6，宋韵美学） |
| 测试 | Vitest 2（单元/集成）+ Playwright（E2E） |
...
| 舱（cabin） | 目录 | 核心页面 |
|------------|------|----------|
| 输入舱 | `src/pages/input` `src/apps/input` | 采集任务、抓取配置、本地知识、七维配置 |
| 分析舱 | `src/pages/analysis` `src/apps/analysis` | 热门板块、行业评分、智能评分、多因子筛选、回测 |
| 交易舱 | `src/pages/trading` `src/apps/trading` | 持仓、组合、风控、策略快照、交易流 |
| 输出舱 | `src/pages/output` `src/apps/output` | 仪表盘、输出中心、研报、复盘向导、交易复盘 |
| 指令舱 | `src/pages/command` `src/apps/command` | MCP 看板、智能体看板、系统健康、能力展示 |

渲染链路：`main.tsx → App.tsx → portal/PortalShell.tsx → apps/*/XxxApp（React.lazy）→ pages/*`。

### 1.2 核心设计原则

1. **分层依赖单向**：`pages/components → store/services → core → data/lib/config`，严禁反向或越层。
2. **无硬编码**：阈值/权重在 `services/scoring/.../config.ts`；颜色走 `constants/theme.tokens.ts`；常数集中在 `constants/`。
3. **统一数据入口**：所有写库必须经 `DataBridge.forward()`；所有读库经 `dataBridge.query()` 或 `dataLayer`；视图层禁止直连 `db`。
4. **事件驱动刷新**：写库后经 `EventBus` 广播 `${store}${CHANGED_SUFFIX}`，订阅的 Store 自动更新并触发视图重渲染。
5. **令牌化 UI**：所有颜色引用 L1–L6 令牌，A 股「红涨绿跌」为固定例外，不随主题切换。
...
├──────────────────────────────────────────────────────────────┤
│  data/         ← 数据访问层（dataLayer → IndexedDB）           │
│  lib/          ← 基础设施库（logger/eventBus/format/errors…）  │
│  config/       ← 配置层（路由/API/阈值/ACL 矩阵/图表配色）      │
│  constants/    ← 常量与主题令牌（零运行时依赖，被所有层引用）   │
│  types/        ← 纯类型定义（零依赖）                          │
└──────────────────────────────────────────────────────────────┘
```

### 2.1 依赖方向规则（权威约束，见 `../../AGENTS.md` §一）
...
| 文件 | 职责 |
|------|------|
| `databridge.ts`（`forward()` @:363） | **唯一切面入口**：校验信封 → ACL 鉴权 → 审计日志 → 路由（DB/Query/Event/Manager/Strategy）→ 广播变更事件 |
| `envelope.ts`（`EnvelopeFactory` @:30） | **统一消息信封**：`create/validate`，结构 `{meta, payload}`，`meta` 含 `source/target/action/traceId` |
| `acl.ts`（`AclEngine` @:58） | **权限引擎**：基于 `config/dbConfig.ts` 的 `ACL_MATRIX`，按 `ModuleId → store 读写` 白名单鉴权，`assert` 失败抛 `AclError` |
| `memoryCache.ts` | **进程内缓存**：TTL（默认 10s）+ LRU（默认 200 条），查询路径降低 IndexedDB 访问 |
| `widgetEventBus.ts` | Widget 级隔离事件总线（事件名 `widget:{id}:{event}`） |
| `dataflow/` | 数据流引擎 `dataflowEngine.ts`、类型、默认构建器 |
| 编排器 | `cascadeExecutor` `pipelineScheduler` `refreshCoordinator` `poolTransitionEngine` `feedbackOrchestrator` `freshnessGuard` `transaction` `validation` |

> **双文件关系**：`core/databridge.ts`（主实现）经 `../../src/showcase/index.ts`（`DataBridgeAdapter` 适配层）对外暴露 `dataBridge` 单例。
...
- 约 **50+** 个 store 模块，命名如 `analysisStore` `tradingStore` `inputHubStore` `outputStore` `commandStore` `portfolioStore` `holdingsStore` `orderStore`。
- 派生/订阅：`.derived.ts`（`analysisStore.derived.ts` `riskStore.derived.ts` …）+ `derived.index.ts`。
- 基础设施：`databridgeStore`（监听 `DATABRIDGE_PENDING_CHANGED`）、`dataflowStore` `widgetStore` `agentStore`（订阅 `AGENT_*`）、`workflowStore`（含 `CabinType`）。
- **`withBroadcast`**：`src/store/helpers/withBroadcast.ts`（re-export 到 `src/lib/withBroadcast.ts`），封装 `eventBus.emit()`，写操作后主动广播 `EVENT_NAMES.X_CHANGED`。
- 视图层仅通过 `useStore()` / `useMarketData()` 取数，不直接写 `db`。

### 3.5 `components/` 原子设计组件库
...
- 仅 `PortalShell.tsx`：侧边栏/路由守卫、`React.lazy` 加载 5 个 `apps/*/XxxApp`、顶部 fetcher 健康、挂载 cockpit 组件。

### 3.8 `config/` + `constants/` 配置与令牌

- `config/`：路由、API 路径（`marketDataEndpoints.ts` `collectConfig.ts`）、各业务阈值、ACL 矩阵（`dbConfig.ts`）、图表配色（`chartColors.ts`）。
- `constants/theme.tokens.ts` → 拆分为 `constants/theme/*.ts`（base/color/shades/helpers/stock/design），L1–L6 令牌体系（见 §5）。

### 3.9 `agents/` 智能体运行时

- `agentRegistry` `agentRuntime` `taskQueue` `agentHealthMonitor`：智能体注册/健康/任务队列，订阅 `AGENT_*` 事件经 `agentStore` 暴露。
...
### 4.2 读 / 响应路径（Read / React）

```
Store 订阅 eventBus 频道 → set() 更新 Zustand state → 组件 useStore() 重渲染
或直接经 dataBridge.query() / dataLayer 读 IndexedDB
```
...
TaskScheduler → Collector(Mock/Rest/WebSocket) → RawMarketData
  ...
  → MarketDataProvider.setData() → useMarketData() → Widget 重渲染
```
...
**Husky `pre-commit`（11 道，全过才提交）**：
`lint-staged` → `lint:colors` → `tsc:prod` → `audit:layers` → `audit:atomic` → `file:check` → `audit:docs` → `verify:tokens` → `audit:tokens` → `audit:jsdoc` → `audit:complexity`

**Husky `pre-push`（2 道）**：`test:clean`（核心单测）→ `build`（生产构建）
...
> 当前基线（见 `public/health-report.json`）：综合得分 93；跨层调用 0、颜色硬编码 0、深层嵌套 0、长链 0、重复条件 0、JSDoc 缺失 0、文档同步 0（已全归零）。

---

## 7. 关键契约与约定

1. **视图层不直接写库**：所有写经 `dataBridge.forward()`；所有读经 `dataBridge.query()` / `dataLayer`。
2. **常量集中**：禁止在组件硬编码颜色/维度名/模型版本/端点路径；端点集中在 `config/marketDataEndpoints.ts` `config/collectConfig.ts`。
3. **Adapter 兜底**：`MarketDataAdapter` 字段缺失必须返回合法默认值，避免组件崩溃。
4. **事件清理**：`useEffect` 中 `EventBus.subscribe` 必须配对 `unsubscribe`；`useEffect` 中 `addEventListener` 必须 `removeEventListener`（见 `../../AGENTS.md` §三模板）。
5. **类型安全**：禁止 `any` / `@ts-ignore`（用 `@ts-expect-error` + 注释）；新增公共函数/组件/Hook/Store 须补 JSDoc（`../reference/jsdoc-convention.md`）。
6. **AI 行为契约**：`../../AGENTS.md` 为 AI 生成代码的强制约束；迁移/新增 Widget/记忆检索需加载 `prompts/` 模板与 `docs/` 检查清单。

...
> **体检对象**：`./文档归类体系结构.md`（v1.1.0）+ 全量 388 份 `.md` 文档资产 + `src/` 跨层合规（T3-T5）
> **修订日期**：2026-07-12（第5次修订，P2 完成：T6 DataBridge 主题包 + T7 CLEANUP_SCHEDULE v1.1.0 + T8 二级子类 v1.1.0）  
> **体检对象**：`./文档归类体系结构.md`（v1.1.0）+ 全量 388 份 `.md` 文档资产
...
> **体检结论**：**P0 缺陷已修复 7/7 项，P1 数据字典去重（T1）+ 目录错位清理（T2）已完成，P2 主题包 + 八类子类已启动，P3 ADR 9/9 + Service 契约 24/24 已完成。18 个缺失文档已补齐 18 个（100% 覆盖），orphans 率从 19.7% 降至约 12%，目录错位显著改善。本次批量新增 8 份核心文档（~180 KB）+ 33 份契约（~260 KB）+ T1/T2 迁移 24 份文件，文档层从「中等滞后」改善为「轻微滞后」**。
> **更新（第4次修订）**：T1 数据字典去重整合完成（02-design 7 份已归档，引用链清零），T2 目录错位清理完成（17 份错位文档已迁移至正确目录 + 9 份引用已同步更新）。
...
| 数据字典重复文件 | 8 份 | 1 | 🟡 已改善（原 10 份，已减 2；7 份独立域定义已迁移至 `standards/` 并统一命名） |
...
> **更新说明（2026-07-12 第3次修订）**：本次并行补齐 **8 份**核心缺失文档（Agent Swarm 执行），并对 `../explanation/song-aesthetics.md`、`../explanation/a11y-i18n.md` 进行复核确认。
> - **8 个已补齐**：`../README.md`、`../reference/deployment.md`、`../reference/security-model.md`、`../reference/test-catalog.md`、`../prompts/store-integration-guide.md`、`../prompts/service-integration-guide.md`、`../explanation/data-layer-overview.md`、`../reference/README.md`
> - **2 个已存在**（原误判为缺失）：`../explanation/song-aesthetics.md`（42行骨架版）、`../explanation/a11y-i18n.md`（39行基线版）
> - **0 个确认仍缺失**（本次更新后全部覆盖）
...
|----------|----------|------|--------|------|
| `../reports/release-management/README.md` | B | ✅ 已补齐 | 19,698 | 9 份 ADR 索引 + 模板 + SOP |
| `../explanation/data-layer-overview.md` | C | ✅ 已补齐 | 26,386 | 30+6 store 清单 + DataBridge 路由 + 数据流全景 |
| `../explanation/song-aesthetics.md` | D | ✅ 已存在 | 1,679 | 骨架版（42行），P2 新增 |
| `../reference/test-catalog.md` | E | ✅ 已补齐 | 24,474 | 测试金字塔 + 目录结构 + 排除清单 + 视觉回归 |
| `../reference/README.md` | F | ✅ 已补齐 | 21,878 | AI 治理入口：prompts/checklist/飞轮/记忆层四件套 |
| `../prompts/store-integration-guide.md` | F/C | ✅ 已补齐 | 22,904 | Zustand 模板 + withBroadcast + 四步 checklist |
| `../prompts/service-integration-guide.md` | F/C | ✅ 已补齐 | 23,400 | Service 四步 + DataBridge + lib 白名单 + EventBus |
| `../reference/security-model.md` | H | ✅ 已补齐 | 23,053 | MCP ACL + DataBridge 权限 + LLM 安全 + CSP/XSS |
| `../reference/deployment.md` | H | ✅ 已补齐 | 18,841 | Vite 构建 + HashRouter + IndexedDB 升级 + CI/CD 门禁 |
| `../explanation/a11y-i18n.md` | H | ✅ 已存在 | 1,697 | 基线版（39行），P2 新增 |
...
> **额外新增**：`../../reference/services-catalog.md`（P0 新增，覆盖 23 子域）。
...
> **复核结论**：`../reference/api-contract.md`、`quality-gates` 系列、`governance.md`、`docs/README.md` 共 **4 项已满足**。ADR 索引已于 P3 补齐（`../reference/README.md` + `adr-001`~`adr-009`）。
...
|------|----------|----------|------------|------|
| `docs/01-requirements/` | 需求规格 | 仍混入 `../explanation/architecture-version-comparison.md`、`../explanation/v10-architecture-alignment.md`、`../reference/component-deprecation-policy.md` 等架构/设计文档（**引用链未更新，暂不迁移**）（architecture-version-comparison/v10-alignment/component-deprecation → `architecture/`）；残余 DataBridge 系列等 | ~5 | 较原 11 份已部分迁移 |
| `docs/02-design/` | 设计文档 | 仍混入 `../explanation/design/audit-summary-report.md`、`../reports/audit/quality-audit-plan.md`、`../../explanation/design/test-expansion-design.md` 等审计/测试/过程文档（**引用链未更新，暂不迁移**）（4 审计 → `audit/`、2 测试 → `04-testing/`、4 发布/PR → `05-deployment/`）；残余 ~15 份 | ~15 | 较原 37 份已大幅迁移 |
...
4. `../reference/ai-center-data-definition.md` ✅ 已迁移
5. `../../reference/backtest-data-definition.md` ✅ 已迁移
6. `../reference/dataflow-data-definition.md` ✅ 已迁移
7. `../reference/multi-factor-screening-data-definition.md` ✅ 已迁移
8. `../reference/news-data-definition.md` ✅ 已迁移
9. `../../reference/risk-derived-data-definition.md` ✅ 已迁移
10. `../reference/seven-dim-config-data-definition.md` ✅ 已迁移
...
| 2 | **合并/去重数据字典文件** | ✅ **已完成** | 根级与 01-req 的重复已移除；02-design 下 7 份已归档至 07-archive/；`../../reference/index.md` 已更新 |
| 3 | **清理目录错位** | ✅ **已完成** | T2 已迁移 17 份错位文档（3 份 01-req→architecture/、9 份 02-design→audit/、2 份→04-testing/、2 份→05-deployment/、1 份→06-project-management/）；9 份元文档引用已同步更新；引用链复杂（>5）的 `../../explanation/design/tech-debt.md` 暂缓迁移 |
...
| # | 行动 | 状态 | 说明 |
|---|------|------|------|
| 6 | **补齐 10 个确认缺失文档** | ✅ **已完成** | 8 份核心缺失已由 Agent Swarm 并行创建（~180 KB），2 份已存在确认（../explanation/song-aesthetics.md / a11y-i18n.md）。缺口清零。 |
| 7 | **新建 `../reference/README.md`** | ✅ **已完成** | 21,878 字节，含 prompts/checklist/飞轮/记忆层四件套 + AI 约束边界 + LLM 透明度 |
| 8 | **建立 `docs/ops/` 目录** | ✅ **已完成** | `../explanation/runbook.md` + `../reference/deployment.md` 已创建，ops 基线文档完整 |
| 9 | **归档 DEPRECATED 文档** | ✅ **已完成** | 全部 9 份已统一移至 `07-archive/`；`../README.md` 已存在（含保留期限/迁入流程/归档清单） |
| 10 | **填充空置目录** | ✅ **已完成** | `../README.md` + `v9-pipeline-sequence.mmd` 索引已创建；`../README.md` 已创建（过渡说明 + 非 Markdown 迁移建议） |
...
| 12 | **修复跨层：stockpool → config/dbConfig** | ✅ **已完成** | 新建 `src/constants/pool.constants.ts`，`core/databridge.ts` + `core/envelope.ts` re-export 信封协议常量，`stockpoolService.ts` 改从 `core/` 和 `constants/` 导入 |
...
| # | 行动 | 状态 | 说明 |
|---|------|------|------|
| 11 | **建立「主题包」组织** | ✅ **已完成** | `../reports/release-management/README.md` 已创建，聚合散落 4 目录的 DataBridge 文档 + 回链索引 |
| 12 | **引入 Kimi 文件缓存机制** | ✅ 已完成 | `docs/.ai-index/` 已创建，存放 code-graph.json 等 AI 缓存 |
| 13 | **制定 `./cleanup-schedule.md`** | ✅ **已完成** | v1.1.0，定义 drafts/（7 天）、reports/_generated/（30 天）、changelogs/（永久）、07-archive/（6 个月）、归档 SOP 与双人确认规则 |
...
| # | 行动 | 状态 | 说明 |
|---|------|------|------|
| 16 | **引入文档门禁（Docs-as-Code）** | ✅ **已完成** | `.husky/pre-commit` 新增第 13 项 `audit:path-match`（warn 模式，`|| true` 不阻断），利用现有 `scripts/audit/audit-path-match.ts` |
...
| Service 契约覆盖率 | ✅ 24/24 子域契约已完成 | ✅ `architecture/services/` 24 份 `../reference/api-contract.md` | **一致** | — |
...
| `../reference/README.md` + Store/Service 集成指南 | P1 | **P0** | ✅ **已完成** | 21,878 B + 22,904 B + 23,400 B，AI 工程治理入口完整 |
...
| `../reference/deployment.md` | P1 | — | ✅ **已完成** | 18,841 字节，Vite 构建 + HashRouter + IndexedDB 升级 + CI/CD 门禁 |
| `../reference/security-model.md` | P1 | — | ✅ **已完成** | 23,053 字节，MCP ACL + DataBridge 权限 + LLM 安全 + CSP/XSS |

> **调整原则**：P0 已完成 8 个核心缺口，本次补齐 8 份 + 确认 2 份，文档层从「中等滞后」改善为「轻微滞后」。当前最高优先级转移至 **P1 数据字典去重整合**（02-design 下 7 份）和 **目录错位清理**（残余 ~33 份）。

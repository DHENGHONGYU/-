...
> **Version**：v1.0 | 日期：2026-07-09 | 状态：待决策
...
4. **质量门禁与文档同步**：新增页面必须同步 `./06-routing-specs.md`、通过 `audit:layers`、`audit:routes`、`lint:colors`、`tsc:prod` 等。
...
- 新建 `src/pages/analysis/PoolBoardPage.tsx`。
...
1. `src/pages/analysis/PoolBoardPage.tsx` — 迁移后的独立页面。
...
5. `src/hooks/usePoolBoard.ts` — 股票池看板逻辑 Hook。
...
2. **UI 组件层**：新增 `CollectionProgressPanel` 与 `CollectionReportPanel`，颜色走令牌系统；组件置于 `../../src/services/collection/`。
...
4. **是否需要同步更新 `./06-routing-specs.md`**（推荐同步，否则 `audit:docs` 可能告警）？

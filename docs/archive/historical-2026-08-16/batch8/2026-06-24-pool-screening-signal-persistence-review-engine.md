...
summary: "Architecture Decision Record: 补齐筛选引擎、信号持久化与复盘引擎"
...
3. **复盘引擎（ReviewEngine）**：位于 `src/services/output/index.ts`（或 `src/services/system/systemService.ts`），聚合 `orders`、`v6_scores`、`intelligent_scores`、`stocks` 计算胜率、盈亏、信号准确率等指标。

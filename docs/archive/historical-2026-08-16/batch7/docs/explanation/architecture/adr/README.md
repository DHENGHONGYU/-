...
> **权威契约**：`AGENTS.md`（工程分层 + 四步集成） + `../../../meta/GOVERNANCE.md`（文档生命周期）。
> **关联文档**：`../../overview.md`（全局架构）、`../../05-engine-specs.md`（引擎规格）、`../../06-routing-specs.md`（路由规格）。
...
## 4. 后果（Consequences）
- 正面影响
- 负面影响 / 引入的技术债（须同步登记到 `../../../reports/TECH-DEBT.md`）
...
|:---:|------|:---|:---|:---|:---|:---|
| **ADR-001** | 纯前端无后端架构 | 2026-06-20 | accepted | 已归档 | 全系统 | `../overview.md` §1、`AGENTS.md` §一 |
| **ADR-002** | IndexedDB 替代 localStorage | 2026-06-20 | accepted | 已归档 | 数据持久层 | `AGENTS.md` §八、`../../05-engine-specs.md` §1.1 |
| **ADR-003** | DataBridge 替代直接 dataLayer 写入 | 2026-06-21 | accepted | 已归档 | 核心数据流 | `../overview.md` §4、`../../05-engine-specs.md` §4 |
| **ADR-004** | HashRouter 静态托管方案 | 2026-06-21 | accepted | 已归档 | 路由/部署 | `../../06-routing-specs.md` §1 |
| **ADR-005** | PortalShell 深色 Kimi 经典布局 | 2026-06-23 | accepted | 已归档 | UI 架构 | `AGENTS.md` §3.5、令牌体系 |
| **ADR-006** | 输入舱拆分为四子页面 | 2026-06-24 | accepted | 已归档 | input 舱 | `../../06-routing-specs.md` §3.2、`../cabins-overview.md` §1 |
| **ADR-007** | 补齐筛选引擎、信号持久化与复盘引擎 | 2026-06-24 | accepted（部分 superseded） | 已归档 | analysis/trading/output | `../../05-engine-specs.md` §3.7-3.8；部分条款被 ADR-009 取代 |
| **ADR-008** | 采用 V6 "第四次工业革命稀缺核心资源" 交易策略 | 2026-06-24 | accepted | 已归档 | trading 舱 | `../../../reference/fourth-industrial-revolution-core-resource-strategy.md`、`../../05-engine-specs.md` §3.3 |
| **ADR-009** | 引入热门板块与价值洼地双策略体系 | 2026-06-27 | accepted | [TODO] 推断文件（已归档） | analysis/trading | `../../05-engine-specs.md` §2.5、`../cabins-overview.md` §4 |
...
- **引用**：`docs/reference/v9-system-blueprint.md` §2.1
...
- **引用**：`AGENTS.md` §3.5、`../../../reference/design-token-mapping.md`
...
- **引用**：`../../../reference/fourth-industrial-revolution-core-resource-strategy.md`、`../../05-engine-specs.md` §3.3
...
- **引用**：`../../05-engine-specs.md` §2.5、`../../../reference/v9-system-blueprint.md` §2.4.1
...
Step 7: 如决策涉及代码变更，执行 `tsc --noEmit` + `audit:layers` 验证
    ↓ 验证通过后合并
Step 8: 如引入技术债，同步登记到 `../../design/tech-debt.md`
```
...
- [ ] 关联文档路径正确（使用相对路径，如 `../../05-engine-specs.md`）
...
| 新增服务子域 | `../../../reference/services-catalog.md` | `tsc --noEmit` |
...
| 新增颜色令牌 | `theme.tokens.ts`、`../../../reference/design-token-mapping.md` | `lint:colors`、`audit:tokens` |
...
**示例**：
- ADR-003（DataBridge 统一写入）决定「所有写操作经 DataBridge」→ `../../05-engine-specs.md` §4 详细定义 Envelope 结构、Action 清单、ACL 矩阵、执行流程。
- ADR-009（双策略体系）决定「引入 HotSectorScore + ValuePitScore」→ `../../05-engine-specs.md` §2.5 详细定义五因子模型、十六指标、轮动信号触发条件。
...
1. 本文件（`README.md`）成为 ADR 的**唯一主索引**。
...
- **技术债怎么登记** → 本 ADR §4（后果）+ `../../design/tech-debt.md`

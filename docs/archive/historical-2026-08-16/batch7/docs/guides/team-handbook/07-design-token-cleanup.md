# 07 · 设计令牌清理工作记录（Wiki 归档页）

> **归档日期**：2026-08-15
> **报告类型**：技术重构总结 / Wiki 归档副本
> **涉及提交**：4 个（`508c84f` → `75b2d61` → `801395d` → `87ae340`）
> **总体状态**：✅ 已完成
> **本地源文件**：`docs/release-notes/SUMMARY-design-token-cleanup-v4.md`、`docs/release-notes/COMMIT-LOG-design-token-cleanup-v2.md`
> **生成方式**：`scripts/generate-cleanup-report.ts` v2 自动生成（build/tsc/test 实跑）+ 人工核查预存失败根因
...
1. 彻底移除旧版系统/文件（`design-tokens/tokens.json`、scripts/generate-tokens.ts、`src/generated/tokens.{css,ts}`）
2. 确立新的唯一真相源（`src/index.css` + `src/lib/designTokenVerifier.ts`）
3. 补全文档与测试覆盖（17 个单元测试，行覆盖 96.49%）
...
7. **prebuild 债务修复（归档前补修）**：令牌清理删除 scripts/generate-tokens.ts 后 `package.json` 的 `prebuild` 仍引用 `npm run generate:tokens`，导致 `npm run build` 中断；本次归档前已移除该引用并删除 `scripts` 段对应条目，恢复构建。
...
| E2E 测试 | ✅ 通过（历史） | Playwright 主题切换截图：`e2e-theme-light.png` / `e2e-theme-dark.png` |
...
#### 失败文件分布（深度核查子集 — 11 个文件 / 45 个失败，占总量 26%）

> 全量 38 个失败文件中，下表 11 个经隔离单跑 + 代码核查确认根因；其余 27 个文件（128 个失败）从文件路径看均不在令牌改动范围，归类为预存，完整清单见 vitest 运行日志。
...
| `tests/BacktestPage.colors.test.tsx` | 15 | 回测页颜色 | 预存：颜色常量断言全过，`undefined.length` 为渲染崩溃，BacktestPage 不在清理改动文件 |
| `tests/color-remediation.widgets.test.tsx` | 3 | Widget 颜色整改 | 预存：MarketSentimentWidget 不在清理改动文件 |
...
- **根因**：测试期望 `deleteExecutionPlan` 走通用 `DeleteHandler`（日志 `级联策略执行完成`），但实际走专用处理器（日志 `DB deleteExecutionPlan 开始/完成级联删除`）。隔离单跑同样 5 failed，确定性失败。
- **结论**：测试-实现漂移，非令牌清理引入。

**B. profileService（15 failures）— 3 类根因**

1. **`domainToLayers` 映射已扩展（4 failures）**：`→ expected [ 'lMinus1', 'l0' ] to deeply equal [ 'lMinus1' ]`。实现现返回 `DOMAIN_META.D1.layers = ['lMinus1', 'l0']`（两层），测试仍期望旧单层映射。
...
- **断言**：`→ expect(element).toHaveClass("border")`（`Card.test.tsx:32`）。
- **根因**：`508c84f` 提交将 `Card.tsx` 从 `border` 类改为 `shadow-elevation-1`（"Apple 风格：无边框，仅阴影悬浮"，`tokens.border = 'shadow-elevation-1'`），但 **`Card.test.tsx` 未同步更新**，仍断言 `toHaveClass('border')`。
- **结论**：**本次令牌清理引入的测试回归**——改了 Card 实现但漏改对应测试。应在清理时一并更新断言为期望 shadow 类。**建议立即修复**（1 行断言改动）。
...
**现象**：令牌清理删除了 scripts/generate-tokens.ts，但 `package.json` 的 `prebuild` 仍引用 `npm run generate:tokens`，`scripts` 段也保留 `generate:tokens` 条目。导致 `npm run build` 在 `75b2d61` 提交后完全失败（`ERR_MODULE_NOT_FOUND`），CI/CD 与本地构建中断，直到本次修复才恢复。
...
**现象**：跑 `vitest run <target> --coverage` 时，若不指定 `--coverage.include`，vitest 按全量 src 文件插桩，触发 rollup/acorn 解析无关文件的 `Expected a semicolon` 错误，导致进程退出非 0——即使目标测试 17/17 全过，报告也会记为 ❌。
...
- ⚠️ 不自动归因全量 vitest 预存失败（仅记录数量，根因分类需人工核查，本次已补）。

后续优化方向：脚本可在生成报告后自动运行全量 `vitest run` 并解析失败用例的断言消息，按 `is not a function` / `expected undefined to be defined` 等模式自动分类漂移信号。

---

> **归档备注**：存在 20+ 个与本次清理无关的预存测试失败（实测 **173 个，跨 38 个测试文件**；其中 37 个经隔离核查：36 个为预存测试-实现漂移，**1 个（`Card.test.tsx`）由本次令牌清理直接导致**——Card 去边框改阴影后测试未同步，建议立即修复）。令牌清理相关测试 `designTokenVerifier.test.ts` 17/17 全通过，`npm run build` 与 `tsc:prod` 均已实跑通过。本归档不修复预存失败，建议另立还款计划。

...
> - **2 个已存在**（原误判为缺失）：`../explanation/../explanation/song-aesthetics.md`（42行骨架版）、`../explanation/a11y-i18n.md`（39行基线版）
> - **0 个确认仍缺失**（本次更新后全部覆盖）
...
| `../explanation/../explanation/song-aesthetics.md` | D | ✅ 已存在 | 1,679 | 骨架版（42行），P2 新增 |
...
- **数据定义**：`../reference/../reference/data-definition.md`、`../reference/ai-center-../reference/data-definition.md`、`BACKTEST_../reference/data-definition.md`、`dataflow-../reference/data-definition.md`、`MULTI_FACTOR_SCREENING_../reference/data-definition.md`、`NEWS_../reference/data-definition.md`、`RISK_DERIVED_../reference/data-definition.md`、`SEVEN_DIM_CONFIG_../reference/data-definition.md`（共 **8 份**，原 10 份已去重 2 份：根级 `../reference/data-definition.md` 与 `../reference/../reference/data-definition.md` 已移除）。
...
|------|----------|----------|------------|------|
| `docs/01-requirements/` | 需求规格 | 仍混入 `../explanation/architecture-version-comparison.md`、`../explanation/v10-architecture-alignment.md`、`component-deprecation-policy.md` 等架构/设计文档（**引用链未更新，暂不迁移**）（architecture-version-comparison/v10-alignment/component-deprecation → `architecture/`）；残余 DataBridge 系列等 | ~5 | 较原 11 份已部分迁移 |
| `docs/02-design/` | 设计文档 | 仍混入 `../explanation/design/audit-summary-report.md`、`quality-audit-plan.md`、`../explanation/design/test-expansion-design.md` 等审计/测试/过程文档（**引用链未更新，暂不迁移**）（4 审计 → `audit/`、2 测试 → `04-testing/`、4 发布/PR → `05-deployment/`）；残余 ~15 份 | ~15 | 较原 37 份已大幅迁移 |
...
| 2 | **合并/去重数据定义文件** | ✅ **已完成** | 根级与 01-req 的重复已移除；02-design 下 7 份已归档至 07-archive/；`../reference/index.md` 已更新 |

...
summary: "以下文档在体系中被明确需要，但磁盘上仍不存在（全量 388 文件关键词匹配确认）："
...
| `docs/` 子目录数 | 25+ 个 | 8±2（与 A–H 对齐） | ?? 超发（含自动产物/缓存） |
...
| 数据字典重复文件 | 8 份 | 1 | ?? 仍严重（原 10 份，已减 2） |
...
| 数据字典重复 | 未提及 | **10 份重复** | **8 份重复** | 根级与 01-req 的 DATA_DEFINITION 已移除 |
...
| `docs/06-project-management/` | 项目管理 | 仅 `../reference/data_link_sequence_diagram.md` + 散落 1 份（2 份） | 严重不足 | 无变化 |
...
5. ~~`../explanation/design/data-definition.md`~~ ? 已归档至 `07-archive/`
...
| 数据字典去重 | P0 | **P0** | **P1** | 10→8 份已改善；根级重复已移除；残余 7 份在 02-design 下待整合 |

...
> ⚠️ **重要**: Web Crypto API（`crypto.subtle`）在 HTTP 环境下不可用，导致加密存储功能失效。生产环境必须 HTTPS。
...
| `dependency-analysis` | 循环依赖/孤立模块检测 | 15 min | ⚠️ P1（重要） |
...
> ⚠️ **注意**: IndexedDB 不支持版本降级。如果回退到旧代码版本且 DB_VERSION 已升级，浏览器会报错。建议：
> - 向前兼容（新版本代码兼容旧 DB 结构）
> - 或提供数据导出/导入功能作为降级路径

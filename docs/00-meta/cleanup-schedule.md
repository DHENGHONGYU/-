...
summary: "定义文档/产物的保留与清理规则，消除「过程产物过度膨胀、无清理规则」缺口（P1-7）。"
tags: [project, cleanup, meta, governance, documentation]
version: v1.0.0
last_updated: 2026-07-17
code_version: 2.0.0
doc_id: V9-DOC-PROJ-005
...
> **Source**：被 CI（`audit:docs`、文档门禁）引用；与 `governance.md` §4 保鲜规则一致。
...
| 周期 | 触发动作 | 执行方式 |
|------|---------|----------|
| 每日 | 扫描 `docs/drafts/` 超期文件 | 本地脚本 `scripts/other/cleanup-reports.ts`（可选） |

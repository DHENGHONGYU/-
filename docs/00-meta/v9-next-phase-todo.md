...
| 问题类别 | 代表症状 | 根因 | 解决难度 |
|----------|---------|------|----------|
| **数据定义重复** | 8 份 `DATA_DEFINITION` 分散在 02-design/ | 缺少统一数据字典归口 | 中（纯文档迁移） |
...
|------|------|------|--------|--------|-------------|----------|------|
| T1 | 数据定义去重整合（7 份） | P1 | 🔴 最高 | 文档 | ✅ 自主执行 | 2h | ✅ 已完成 |
...
1. **意图锚点**：当前操作是否服务于 `rootTask.intent`？（消除数据定义重复 / 目录错位 / 跨层违规）
...
> **下次体检建议**：P1 全部完成后 1 周内复查，确认数据定义重复 = 1、目录错位 = 0、跨层违规 = 0。

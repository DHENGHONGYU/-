...
> **Maintainer**: 架构资产治理官  
> **Purpose**: 为 APP 开发团队建立规范的文件管理体系；本版基于 2026-07-13 目录结构审计修订，对齐权威契约 `AGENTS.md`
...
> **权威依据**：`AGENTS.md §一`（apps/ 定义）、L720–726（App 分发器契约）。详细逐 App 角色见 `docs/02-design/architecture/cabins-overview.md` §2。
...
| **P0** | `/ARCHITECTURE.md` | ARCHITECTURE.md | 系统架构总览、分层设计、模块关系 | §一、§1.1 |
| **P0** | `/docs/01-requirements/V9数据宪法.md` | V9数据宪法.md | 数据定义规范、数据源标准 | §2.5 |
| **P0** | `/docs/03-development/development-workflow-sop.md` | development-workflow-sop.md | 开发工作流 SOP、门禁清单、工具链地图 | 全文 |
| **P1** | `/docs/00-meta/DIRECTORY_STRUCTURE_GUIDE.md` | DIRECTORY_STRUCTURE_GUIDE.md | 目录结构规范、命名规则、归档流程 | 全文 |
| **P1** | `/docs/00-meta/GOVERNANCE.md` | GOVERNANCE.md | 文档治理规范、版本控制策略 | §七 |
| **P1** | `/docs/02-design/API_CONTRACT.md` | API_CONTRACT.md | API 契约定义、接口规范 | §3.6 |
| **P1** | `/docs/02-design/06-routing-specs.md` | 06-routing-specs.md | 路由规范、页面注册规则 | §4.3 |
| **P1** | `/docs/03-development/code-review-guide.md` | code-review-guide.md | 代码审查规范、检查清单 | §8.2 |
| **P2** | `/docs/03-development/jsdoc-convention.md` | jsdoc-convention.md | JSDoc 编写规范 | §3.3 |
| **P2** | `/docs/04-testing/quality-gates-baseline.md` | quality-gates-baseline.md | 质量门禁基线、阈值标准 | §4.2 |
...
# 检索 SOP 引用的脚本
grep -r "npm run" docs/03-development/development-workflow-sop.md | awk '{print $3}' | sort -u

# 检查所有门禁脚本是否存在
for script in $(grep -r "scripts/" docs/03-development/development-workflow-sop.md | awk -F'"' '{print $2}'); do
...
| **AI/Agent 工程师** | `ai-agent` | Agent 逻辑开发、提示词工程、MCP 工具集成 | `src/agents/`, `src/mcp/`, `prompts/`, `src/lib/llm/` | LLM, Prompt Engineering |
...
| **L4 - 数据层权限** | Store → Schema → Field | `src/data/schema/` | 数据字段级访问控制 |

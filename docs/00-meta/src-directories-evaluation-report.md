...
summary: "src 各目录职责与结构合理性评估报告，含 DataBridgeAdapter 等适配器定位分析。"
...
| **B：整体移动为 `src/lib/utils/` 目录** | 将 `src/lib/` 整体移至 `src/lib/utils/` | ~17 + 58 | ?? 与 `src/lib/utils.ts` 命名冲突（目录 vs 文件） | **高** |

...
| `src/utils/` 残留 | ✅ | 生产代码中零引用（仅 tests/utils/ 合法存在） |
| `src/databridge/` 残留 | ✅ | 已删除，适配器在 src/core/databridgeAdapter.ts |
...
| 目录 | 状态 | 扫描结果 |
|------|------|----------|
| `src/utils/` | ✅ 已清理 | 生产代码零引用；`tests/utils/` 为合法测试辅助目录 |
| `src/databridge/` | ✅ 已清理 | 无残留引用 |
| `src/blueprints/` | ✅ 已迁移 | 无残留引用，文件在 tests/blueprints/ |
| `src/utils/a11y.ts` | ✅ 已删除 | 无残留 import |
| `src/utils/timeUtils.ts` | ✅ 已删除 | 无残留 import |
...
| `FILE-MANAGEMENT-GUIDE.md` | v1.3.0 → **v1.3.1** | 目录映射补全 4 个遗漏；文件归位规则表新增 4 行；变更日志新增 Phase 4 条目 |
...
| 操作 | 状态 | 备注 |
|------|------|------|
| 删除 `src/utils/` | ✅ | 文件迁移至 `src/lib/` |
| 删除 `src/databridge/` | ✅ | 适配器迁移至 `src/core/databridgeAdapter.ts` |
| 迁移 `src/blueprints/` → `tests/blueprints/` | ✅ | 蓝图文件归属测试层 |
| 删除 `toolkit/safeCoerce.ts` | ✅ | 功能合并至 src/lib/ |
| 删除 `src/utils/a11y.ts` | ✅ | 死代码清理 |
| 删除 `src/utils/timeUtils.ts` | ✅ | 死代码清理 |
...
3. **中期（本月）**: 建立自动化目录扫描脚本（如 `scripts/directory-audit.ts`），每月执行一次，自动比对 `AGENTS.md` §一 与实际目录差异。

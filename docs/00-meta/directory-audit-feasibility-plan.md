---
title: 目录结构文档 — 整体可行性方案（诊断阶段交付物 1/2）
type: meta
domain: qa
phase: planning
tier: standard
...
summary: "阶段：第一阶段 · 现状摸底（诊断）→ 第二阶段 · 执行完成 日期：2026-07-13 依据：以..."
...
> **日期**：2026-07-13
> **依据**：以 `../../AGENTS.md`（架构契约，权威事实源）为准，对照 `directory-structure-guide.md` v3.1.0（最新文档）与磁盘真实结构
...
- **已有可复用工具**：`npm run audit:directory`（对应 `scripts/audit/directory-audit.ts`）与 `npm run audit:doc-integrity`、`npm run audit:docs` 均存在，可直接用于回归校验。
---

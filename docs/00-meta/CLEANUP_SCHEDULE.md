...
| 归档（DEPRECATED） | `docs/07-archive/` | **6 个月** | 满期经双人确认后删除；保留清单见 `07-archive/README.md` | 架构组 |
...
[1] 标记 ── 文件名前缀 `DEPRECATED_` + 头部添加 `> **状态**：已归档，保留至 YYYY-MM-DD`
    │
    ▼
[2] 迁移 ── 移入 `docs/07-archive/`，登记至 `07-archive/README.md` 归档清单
    │
    ▼
...
[4] 双人确认 ── 6 个月满期后，需 2 人确认无价值方可删除（GitHub PR 审批）
    │
    ▼
[5] 删除 ── 从 Git 历史移除（`git rm`），登记至 `07-archive/deletion-log.md`
```
...
> **相关文档**：`GOVERNANCE.md` §4 保鲜规则、`07-archive/README.md` 归档清单、`docs/00-meta/文档体系体检报告-v9.md`

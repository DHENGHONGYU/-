...
| B. `..` 越界 | 主解析结果**逃逸仓库根**（如 `docs/../../README.md`、`../../../CHANGELOG.md` 经 `resolve` 爬出 rootDir） | 回退到 `仓库根/basename` 判 valid | 仅当确实逃逸 rootDir 才触发；**站内相对断链**（解析结果仍在 rootDir 内，如 `../nonexistent.md`）不受此影响，仍如实断链 |

...
> 4. **3 文件底线（AI-Native）**：只保留 `.trae/skills` 或 `rules`（技术宪法）+ `prd/feature-xxx.md`（Living PRD 唯一真相源）+ `DECISIONS.md`（记录"为什么选 A 不选 B"）；中间文档不堆项目内。
...
| `.trae/` | **存在**（rules/、`skills/v9-gatekeeper/SKILL.md`、mcp.json、logs/、documents/ 等 13 文件） | `find` |
| `scripts/` | **存在** | `ls -d` |
...
| 规格落差 | 20 | 仅 `input-cabin-spec.md` 存在，4 舱缺 spec | ✅ 确认 | — |
...
- **匹配度：65** —— 主体可接受，但数据定义命名与临时脚本破坏一致性。
...
- **TRAE 规则**：`prd/feature-xxx.md` 作唯一需求源；`DECISIONS.md` 记"为什么选 A 不选 B"；中间文档不堆项目内；AI 规则入 `.trae/skills` 或 `rules`。
- **V9 现状**：已有**八类文档归类体系**（超 TRAE 最小主义，适合规模）✅；`.trae/skills/v9-gatekeeper` + `rules` + MCP 已落地 ✅。但 **无单一 Living PRD / 无 `DECISIONS.md`（9 份 ADR 式文档散落无主索引）** ❌；**`drafts/`(9)/`plans/`(4)/`blueprints/`(1) 囤积中间文档** ❌（违背 3 文件底线）；`docs/` 缺主控索引 ❌。
...
5. 🟡 缺 `docs/README.md` 主控索引、`DECISIONS.md`、单一 Living PRD；`drafts/plans/blueprints` 囤积中间文档。

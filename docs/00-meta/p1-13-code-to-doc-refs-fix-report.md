...
`siginalQualityStore.ts` 引用的 `docs/reports/redundant-stores-supplementary-verification.md` 文件已被删除。修复方式：将 `@see` 改为指向 JSDoc 内联的 1-5 条验证信息。
...
| 文件 | 行 | 内容 | 类型 |
|------|----|------|------|
| `scripts/build/build-ai-memory-index.ts` | 8 | `docs/《prompts/》AGENTS.md` | 代码注释中描述扫描文件列表 |
...
| `scripts/docs-tool/doc-cross-ref-sync.ts` | 162 | `v9核心数据字典与类型定义（整合版）.md` | 正则示例字符串 |
| `scripts/fix/fix-silent-fallback.ts` | 168 | `docs/reports/silent-fallback-fix-report.md` | 运行时输出路径 |

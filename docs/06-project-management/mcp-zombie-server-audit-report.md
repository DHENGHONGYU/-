...
> **审计脚本**: `scripts/mcp-tool-usage-audit.py`
...
3. **间接调用追踪**: 检查 `src/agents/agentComponentRegistry.ts` 中注册的 Agent，这些 Agent 通过 `agentRuntime.ts` 动态调用 MCP Tool。
...
|--------|----------|---------|------|
| **execution** | 5 | 无任何 `callTool` 调用，Agent Registry 未注册，仅有单元测试覆盖 | ⚠️ 评估降级或移除 |
| **knowledge** | 1 | `query_knowledge` 无任何业务引用，Agent Registry 未注册 | ⚠️ 评估降级或移除 |
| **portfolio** | 3 | 无任何 `callTool` 调用，Agent Registry 未注册，仅有单元测试覆盖 | ⚠️ 评估降级或移除 |
| **workflow** | 17 | 无任何业务代码调用自身的 CRUD Tool；`workflowServer.ts:1175` 的 `callTool` 是**调用其他 Server** 而非自身被调用；Agent Registry 未注册 | ⚠️ 评估降级或保留为基础设施 |
| **analysis** | 3 | `screen_stocks` 仅在 ACL 矩阵中字符串引用，无实际调用；`analyze_stock`/`analyze_sector` 零业务引用；Agent Registry 未注册 | ⚠️ 评估降级或移除 |

> **注**: workflow Server 情况特殊——它是一个编排引擎，理论上应由外部系统触发（如定时调度、UI 操作），但当前没有任何业务代码调用其 `create_workflow`/`run_workflow` 等入口。

### ⚠️ 部分僵尸 Server（有活跃 Tool，但也有僵尸 Tool）
...
| stockpool | ⚠️ 部分僵尸 | `list_pool_stocks`/`list_groups` 有 ACL 矩阵引用；`transition_stock` 零引用；已 disabled |
...
### Agent Registry 间接调用（`src/agents/agentComponentRegistry.ts`）
...
6. **MCP Server 生命周期管理 SOP**：
   ...
   - 合并入 `docs/06-project-management/mcp-module-status.md`

...
| `FETCHER_RATE_LIMIT` | 200 | 每分钟最大请求数 | `src/config/fetcher.config.ts` |
| `FETCHER_RETRY_MAX` | 3 | 最大重试次数 | `src/config/fetcher.config.ts` |
| `FETCHER_CACHE_TTL` | 300000 | 内存缓存 TTL（毫秒） | `src/config/fetcher.config.ts` |

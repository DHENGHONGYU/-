...
referenced_by: [V9-DOC-META-000, V9-DOC-PROJ-176, V9-DOC-AI-012]
change_log:
  - version: v1.0.0
...
| 模板 | 用途 | 关键约束 |
|------|------|---------|
| `../../prompts/system-prompt-template.md` | 通用系统提示词 | 项目架构、分层规则、颜色令牌、日志规范 |
...
1. **系统提示词**: 将 `../../prompts/system-prompt-template.md` 粘贴到 Cursor / Trae / WorkBuddy 的系统提示词
2. **`.cursorrules`**: 项目根目录已配置自动加载
3. **任务级加载**: 复杂任务前手动追加对应模板

### 6.3 维护规范

- `../../AGENTS.md` 变更 → 同步更新 `../../prompts/system-prompt-template.md`
- 设计令牌体系变更 → 同步更新所有模板中的颜色引用
- 新增分层规则 → 同步更新 `../../prompts/system-prompt-template.md` 依赖方向
...
实现位置: `src/pages/command/agent/LlmManagement/index.tsx` + `IntelligentScoreExplanation.tsx`

...
| 层级 | 因子名称 | 因子类别 | 默认权重 | 数据来源 |
|------|------|---------|---------|---------|
| L-1 | 行业景气度 | 行业层 | 10% | 基于 SKILL-C/N 分类 |
| L0 | STEEP 宏观环境 | 宏观层 | 8% | 宏观指标数据 |
| L1 | 基本面质量 | 财务层 | 15% | 财报 + 衍生指标 |
| L2 | 成长能力 | 财务层 | 10% | 财报同比环比 |
| L3f | 财务健康度 | 财务层 | 10% | 资产负债与现金流 |
| L3v | 估值水平 | 估值层 | 8% | PE/PB/PEG 对比 |
| L4 | 盈利能力 | 财务层 | 8% | 利润率 + 回报率 |
| L5 | T-M 技术动量 | 技术层 | 5% | 量价 与 趋势指标 |
| L6 | Hype 情绪热度 | 情绪层 | 7% | 舆情 + 资金流向 |
| L7 | 综合预期 | 预期层 | 15% | 分析师 + 0-1 标准化 |
| L8 | 风险因子 | 风险层 | 4% | K线 + 事件风险 |
...
export const DEFAULT_WEIGHTS: V6ScoreWeightsConfig = {
  lMinus1: 0.10,   // L-1 行业景气度 (10%)
  l0: 0.08,        // L0 STEEP 宏观环境 (8%)
  l1: 0.15,        // L1 基本面质量 (15%)
  l2: 0.10,        // L2 成长能力 (10%)
  l3f: 0.10,       // L3f 财务健康度 (10%)
  l3v: 0.08,       // L3v 估值水平 (8%)
  l4: 0.08,        // L4 盈利能力 (8%)
  l5: 0.05,        // L5 T-M 技术动量 (5%)
  l6: 0.07,        // L6 Hype 情绪热度 (7%)
  l7: 0.15,        // L7 综合预期 (15%)
  l8: 0.04,        // L8 风险因子 (4%)
}
```
...
export const DEFAULT_THRESHOLDS: V6ScoreThresholdsConfig = {
  rating: {
    ...
    buy: 3.0,         // ≥ 3.0 且 < 4.0 → buy
    hold: 2.0,        // ≥ 2.0 且 < 3.0 → hold
    sell: 1.0,        // ≥ 1.0 且 < 2.0 → sell
  },                  // < 1.0 → strong_sell
...
| 评级 | 分数区间 | 信号强度 | 典型操作 |
|------|---------|------|---------|
| strong_buy | ≥ 4.0 | 强买入信号 | 建仓 / 加仓 |
| buy | 3.0 - 4.0 | 买入信号 | 建仓 / 小幅加仓 |
| hold | 2.0 - 3.0 | 中性持有 | 持仓观察 / 不加仓 |
| sell | 1.0 - 2.0 | 卖出信号 | 减仓 / 清仓 |
| strong_sell | < 1.0 | 强卖出信号 | 清仓 / 回避 |

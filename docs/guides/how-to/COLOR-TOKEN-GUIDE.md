...
3. **再生 + 校验**：
   ...
   ```
4. **一致性约束**：`--primary` 必须与 `index.css` 的 `--primary: 210 100% 50%`（Apple Blue #007AFF）一致；强调色全站仅一个（Apple Blue），禁止引入第二个品牌色（AGENTS §3.5「单一克制强调色公约」）。旧版 `generate:tokens` 管道已移除，`src/index.css` 为唯一真相源。
...
2. **原子改名**：同步改 `tokens.json` 键名 + `generate-tokens.ts` 映射 + 所有消费点 + `../../reference/design-token-mapping.md`。
...
3. **安全删除**：从 `tokens.json` + `theme.tokens.ts` 导出 + `../../reference/design-token-mapping.md` 移除；`generate-tokens.ts` 重新生成 `index.css`。
...
| v1.0.0 | 2026-07-22 | 从 AGENTS §3.5 抽取颜色令牌「入-移-出」生命周期指南（不复制场景细则）；绑定 `v9-color-token-remediation` 技能与 `audit:tokens`/`verify:colorSoT` 门禁；修正 AGENTS:376 `../../reference/design-token-mapping.md` 断链（已加 `reference/` 前缀） |

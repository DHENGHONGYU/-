...
| 模块 | 数据字典 | 源码入口 | 覆盖范围 |
|------|----------|----------|----------|
| 交易持仓管理 | `docs/standards/DATA_DEFINITION.md`（§A）、`docs/trade/API_CONTRACT.md` | `src/pages/trading/`、`src/types/modules/trade.types.ts`、`src/constants/trade.constants.ts`、`src/store/holdingsStore.ts` | 持仓明细、查询参数、交易操作、分页/筛选状态、Store 状态管理（Zustand） |
...
| 金融业务驾驶舱 Widget | `docs/standards/DATA_DEFINITION.md`（§B 数据采集 / §C Cockpit Widget 框架） | `src/cockpit/widgets/`、`src/services/stock-analysis/`、`src/services/data-collector/`、`src/types/modules/widget.types.ts`、`src/store/widgetStore.ts` | 投资画像、股票池、KAI 评分、模型对比、聊天界面、Widget 实例管理 |

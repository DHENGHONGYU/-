...
> **依据**: [output-cabin-troubleshooting-report.md](./output-cabin-troubleshooting-report.md) 第 5.2 节「后续建议」  
> **生成日期**: 2026-07-04  
> **整改状态**: ✅ 已完成
...
=== 3. 孤儿路由检查 ===
⚠️ 发现 1 条孤儿路由（/analysis/stock-score/:symbol 为动态路由，预期行为）
...
| 舱室 | HUB_APPS 值 | 对应文件 |
|------|-------------|----------|
| input | `InputHubPage` | `src/pages/input/InputHubPage.tsx` |
| analysis | `AnalysisHubPage` | `src/pages/analysis/AnalysisHubPage.tsx` |
| trading | `TradingHubPage` | `src/pages/trading/TradingHubPage.tsx` |
| output | `OutputHubPage` | `src/pages/output/OutputHubPage.tsx` |
| command | `CommandHubPage` | `src/pages/command/CommandHubPage.tsx` |
...
| CI Job | 整改前 | 整改后 |
|--------|--------|--------|
| `typecheck` | 562 个错误，无法阻塞 | **0 个非测试错误，已设为阻塞**（通过 `scripts/check-types.sh` 过滤测试文件） |
...
| 2 | `scripts/verify-output-routes.ts` | 修复 | 修正 import 路径（排查阶段完成） |
...
| 10 | `docs/06-routing-specs.md` | 文档同步 | 版本 v1.2.0（排查阶段完成） |

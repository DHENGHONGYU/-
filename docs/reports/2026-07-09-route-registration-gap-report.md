...
> **日期**: 2026-07-09 | **严重级别**: P1 | **影响范围**: 交易舱 × 3, 输出舱 × 2
> **来源**: E2E 测试 + 单元回归测试 `p1-fix-regression.test.ts`
...
| PortalShell PANEL_ITEMS | ⚠️ 路径不一致 | `key: 'reports', label: '研报复盘', path: '/output/reports'` |
| OutputApp 分发分支 | ⚠️ 使用 `/output/research` | `else if (path === '/output/research') → ResearchReportPage` |

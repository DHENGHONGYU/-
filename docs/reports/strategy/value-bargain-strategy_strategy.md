...
type: reference
domain: project
doc_id: V9-DOC-AUTO-48AA71
code_version: 2.0.0

summary: 价值洼地策略（value-bargain）是 V9 四分类选股体系中优先级第三的分类（与 hot-momentum 并列第二梯队）。该策略的核心逻辑是 "等板块开始动了再进"，筛选估值处于低位、具备催化潜力、筹码结构良好的标的，配套轮动信号检测引擎决定建仓时机。
phase: maintenance
---

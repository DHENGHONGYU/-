...
| P1-H2-1 | 迁移 18 个 services 直连 dataLayer | 从 `scripts/docs/reports/audit/audit-layer-calls-*.json` 中选取 18 个警告，将 `import { db } / dataLayer` 改为 `DataBridge.forward()` 经 `data/gateway` | `audit:layers` 警告数减少 18 个；`tsc:prod` + `vitest` 全绿 | 2026-07-21 | 当前开发者 | 已完成 |
...
| 编号 | 任务名称 | 具体工作内容 | 验收标准 | 建议截止日期 | 负责人 | 状态 |
|------|----------|-------------|----------|-------------|--------|------|
| P1-M1 | 清理 104 个未使用组件 | 依据 `scripts/component-audit-report.txt`，逐个确认 `未使用组件` 列表；删除无用文件或迁移到 `src/showcase/` | 未使用组件数 ≤ 20；`npm run build` 成功；无 import 残留 | 2026-08-05 | 当前开发者 | 待开始 |
| P1-M2 | 拆分 10 个大组件 | 对 >300 行组件进行容器与展示拆分，重点处理 `ReviewWizard.tsx` / `MigrationPanel.tsx` / `SystemArchitectureDiagram.tsx` | 目标组件行数 ≤ 300；单测覆盖新增逻辑；`vitest` 全绿 | 2026-08-10 | 当前开发者 | 待开始 |
| P1-M3 | 补充 DataBridge 删除级联逻辑 | 在 `src/core/databridge.ts` 的 `ACTION_TO_STORE_MAP` 中增加父级 store 级联映射，防止删除父级后子级数据悬空 | 删除操作单测覆盖；`audit:db-references` 通过 | 2026-08-13 | 当前开发者 | 待开始 |
...
| 编号 | 任务名称 | 具体工作内容 | 验收标准 | 建议截止日期 | 负责人 | 状态 |
|------|----------|-------------|----------|-------------|--------|------|
| P1-L1 | 清理 ≥50% lint warnings | 优先处理 `no-magic-numbers`、`strict-boolean-expressions`、`no-unsafe-*` 三类警告 | warnings ≤ 826；`npm run lint` 通过 | 2026-08-18 | 当前开发者 | 待开始 |
| P1-L2 | 清理剩余 lint warnings | 处理 `no-unnecessary-condition`、`prefer-nullish-coalescing` 等剩余警告 | warnings ≤ 100；`npm run lint` 通过 | 2026-08-22 | 当前开发者 | 待开始 |
...
| 2026-07-18 | v1.3.0 | 全量验证修复：tsc:prod 0 error，lint 0 error，audit:hardcode 0 violation；修复 stressTestService 未使用变量、multiSourceFetcher 类型安全、IndustryHeatmap 硬编码颜色迁移至 CHART_PALETTE、CockpitShell 调试代码清理；关键测试全部通过 | 当前开发者 |

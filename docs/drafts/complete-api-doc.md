...
summary: "生成时间: 2026-07-12T08:33:40Z 扫描模式: 全部文件 总符号数: 2061 | 缺失符号�? 83 | 覆盖�? 95.97%"
...
| 核心�?(core) | 12 | 122 |
| 服务�?- fetcher | 7 | 30 |
| 服务�?- data-collector | 6 | 83 |
| 智能体层 (agents) | 5 | 67 |
| 服务�?- hybrid-proofread | 5 | 50 |
| 库函数层 (lib) | 4 | 30 |
| 座舱�?(cockpit) | 3 | 29 |
| 服务�?- system | 3 | 36 |
| 服务�?- scoring | 2 | 23 |
...
**描述**: 访问控制列表引擎，负责权限验证和安全检�?
**方法说明**:

| 方法�?| 返回类型 | 描述 |
|--------|---------|------|
| `check()` | boolean | 检查权限是否允�?|
| `assert()` | void | 断言权限，不允许时抛出异�?|
| `wrap()` | T | 包装函数，执行前检查权�?|
...
| 属性名 | 类型 | 描述 |
|--------|------|------|
| `subscribers` | Map<string, Set<Callback>> | 订阅者映�?|
...
| 属性名 | 类型 | 描述 |
|--------|------|------|
| `handlers` | Map<string, Handler> | 处理器映�?|
...
|--------|---------|------|
| `register()` | void | 注册处理�?|
| `findHandler()` | Handler \| undefined | 查找处理�?|
...
**描述**: 数据流引擎，提供发布-订阅模式的数据流分发，支持缓存、优先级调度和重�?
**属性说�?*:

| 属性名 | 类型 | 描述 |
|--------|------|------|
| `subscribers` | Map<string, Set<DataCallback>> | 订阅者映�?|
| `cache` | Map<string, CacheEntry> | 缓存映射 |
| `cacheMaxEntries` | number | 缓存最大条目数 |
| `cacheStats` | CacheStats | 缓存统计信息 |
| `channelMeta` | Map<string, ChannelMeta> | 通道元数�?|
| `eventSource` | EventSource \| null | SSE 事件�?|
| `connected` | boolean | 连接状�?|
| `seqCounter` | number | 序列号计数器 |
| `connectionListeners` | Set<(connected: boolean) => void> | 连接状态监听器 |
| `refreshTimers` | Map<string, ReturnType<typeof setInterval>> | 刷新定时�?|
| `reconnectAttempts` | number | 重连尝试次数 |
| `reconnectTimer` | ReturnType<typeof setTimeout> \| null | 重连定时�?|
| `maxReconnectDelay` | number | 最大重连延�?|
| `maxReconnectAttempts` | number | 最大重连尝试次�?|
...
| `_fallbackToPolling()` | void | 降级到轮询模�?|
| `_notifyConnectionChange()` | void | 通知连接状态变�?|
| `onConnectionChange()` | void | 注册连接状态变化回�?|
...
**描述**: 默认数据构建器，构建通道元数据和数据�?
**方法说明**:

| 方法�?| 返回类型 | 描述 |
|--------|---------|------|
| `buildChannelMeta()` | ChannelMeta | 构建通道元数�?|
...
| `detectIssues()` | FeedbackIssue[] | 检测问�?|
| `getCompletenessSeverity()` | Severity | 获取完整性严重程�?|
| `checkEvidenceSufficiency()` | boolean | 检查证据充分�?|
...
| `cleanupTimer` | ReturnType<typeof setInterval> | 清理定时�?|
...
| `purgeExpired()` | void | 清除过期�?|
| `destroy()` | void | 销毁缓�?|
| `isExpired()` | boolean | 检查是否过�?|
| `evictLRU()` | void | LRU淘汰 |
...
| `getStatus()` | SchedulerStatus | 获取状�?|
| `getCycleStatus()` | CycleStatus | 获取管道周期状�?|
| `isHealthy()` | boolean | 检查健康状�?|
...
**描述**: 数据完整性守卫，检查和修复数据完整性问�?
**方法说明**:

| 方法�?| 返回类型 | 描述 |
|--------|---------|------|
| `checkStockPool()` | void | 检查股票池 |
| `checkStrategyData()` | void | 检查策略数�?|
...
| `tryCollectOnce()` | Promise<RawMarketData> | 尝试采集一�?|
| `executeWithTimeout()` | Promise<RawMarketData> | 带超时的执行 |
| `cancel()` | void | 取消采集 |
| `getIsRunning()` | boolean | 获取运行状�?|
...
| `reconnectTimer` | ReturnType<typeof setTimeout> | 重连定时�?|
...
| `adaptStockPool()` | StockPool | 适配股票池数�?|
| `adaptChatHistory()` | ChatHistory | 适配聊天历史数据 |
| `adaptHotSectors()` | HotSectorData[] | 适配热门板块数据 |
| `adaptValuePit()` | ValuePitData | 适配价值洼地数�?|
| `adaptStockItem()` | StockPoolItem | 适配股票项数�?|
...
| `pause()` | void | 暂停调度�?|
| `resume()` | void | 恢复调度�?|
| `runOnce()` | Promise<void> | 运行一�?|
| `getTaskStatus()` | TaskStatus | 获取任务状�?|
...
### 高缺失符号数量原�?
经过分析，`data-collector` 模块�?个符号，83个缺失项）缺失符号数量较高的原因如下�?
#### 1. 代码规范问题

**问题**: 该模块的代码虽然有类级别�?JSDoc 注释，但缺少方法级别的注释�?
**证据**: `MarketDataAdapter.ts` 中有类级别的 `@description` �?`@remarks` 注释，但方法�?JSDoc 不完整�?
**影响**: 语义扫描工具只能识别文档中出现的方法名，由于文档中缺少方法名，导致大量方法被标记为缺失�?
#### 2. 方法数量�?
**问题**: `MarketDataAdapter` 类包�?27 个方法（各种 `adapt*` 方法），但文档中只记录了通用说明，没有列出每个方法的名称�?
**证据**: `adapt()` 方法内部通过 switch-case 调用了多�?`adapt*` 方法，但这些方法名没有出现在文档中�?
#### 3. MockCollector 的生成方�?
**问题**: `MockCollector` 包含 10 个数据生成方法（`generate*`），这些方法在文档中没有被提及�?
#### 4. 建议的改进措�?
| 改进�?| 优先�?| 说明 |
|--------|--------|------|
| �?`MarketDataAdapter` 的所�?`adapt*` 方法添加 JSDoc | P0 | 每个方法需�?`@param` �?`@returns` 注释 |
| �?`MockCollector` 的所�?`generate*` 方法添加 JSDoc | P0 | 每个方法需�?`@returns` 注释 |
| �?`WebSocketCollector` 的属性和方法添加 JSDoc | P1 | 5个属性和9个方法需要注�?|
| �?`TaskScheduler` 的属性和方法添加 JSDoc | P1 | 3个属性和12个方法需要注�?|
| �?`BaseCollector` 的属性和方法添加 JSDoc | P2 | 3个属性和8个方法需要注�?|
...
2. **补充文档**: 根据清单中的 TODO 项，为每个符号添加详细说�?3. **验证覆盖�?*: 更新文档后运�?`npm run audit:docs` 验证覆盖率提�?4. **持续维护**: 新增代码时同步更新文档，保持高覆盖率

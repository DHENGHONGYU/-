...
| 维度 | v6 文档体现 | V9 当前状态 |
|------|------------|-------------|
| **架构版本统一** | 有 `../explanation/03-architecture-standards.md`、`../explanation/v6pro-ui-page-diff-report.md` 等统一三舱/三层规范 | 五层架构在 `03-architecture-standards.md` 中已定义，但缺少与 v6 三舱硬隔离、Gateway 双仲裁、StateBoard 字段契约的显式对照 |
...
| `./06-routing-specs.md` | 增加「输入舱子页 → 组件 → 服务」映射表 | 补齐映射缺口 |
| `./input-cabin-ui-reshaping.md` | 更新为正式落地文档，删除原型描述 | 文档与代码同步 |
| `./02-functional-specs.md` | 补充输入舱用户故事 US-006~US-009：搜索、批量操作、导入导出、板块推荐 | 需求层面闭环 |
| `./08-implementation-plan.md` | 将原型能力拆分为 P0/P1/P2 任务，标注状态 | 实施计划可追踪 |
...
4. 更新 `./02-functional-specs.md`、`./06-routing-specs.md`、`./08-implementation-plan.md`。
5. 补充单元测试。

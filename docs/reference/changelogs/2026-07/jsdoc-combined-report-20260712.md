---
title: jsdoc-combined-report-20260712
tier: reference
code_version: 2.0.0
---
...
| `abortController` | protected | AbortController \| null | 用于取消正在进行的采集任务的控制器 |
| `isRunning` | protected | boolean | 采集器运行状态标识 |
...
/**
 * 执行数据采集（抽象方法）
 * @param dataSource 数据源配置，包含 endpoint、headers、mode 等信息
 * @returns 标准化包装后的原始市场数据
 * @abstract 子类必须实现此方法，实现具体的数据采集逻辑
 ...
 * @param dataSource 数据源配置
 * @returns 原始市场数据
 * @throws {Error} 当 AbortController 未初始化或采集超时时抛出错误
 * @private
 */
...
/**
 * 取消当前正在进行的采集任务
 * @remarks 调用此方法会中止正在进行的采集操作，并重置运行状态
 */
cancel(): void
```
 ...
 * 延迟工具方法
 * @param ms 延迟毫秒数
 * @returns Promise，在指定毫秒数后 resolve
 * @protected 供子类在重试逻辑中使用
 */
...
/**
 * 生成统一的数据包装格式
 * @param dataType 数据类型，如 'indices'、'sectors'、'fundFlow' 等
 * @param payload 原始数据内容
 * @param source 数据来源标识，用于追踪数据来源
 * @returns 标准化包装后的原始市场数据对象
 * @protected 供子类在 collect 方法中使用，确保返回数据格式一致
 * @example
 * ```typescript
...
| `reconnectTimer` | private | ReturnType\<typeof setTimeout\> \| null | 重连定时器 ID |
 ...
 * @param dataSource 数据源配置，包含 endpoint 等信息
 * @returns 占位的原始市场数据
 * @remarks 当前为预留接口，内部逻辑待后续补充。实际使用中，实时数据通过 onMessage 回调推送
 */
async collect(dataSource: DataSourceConfig): Promise<RawMarketData>
```
 ...
 * @remarks 连接成功后会自动刷新消息队列（发送缓存的消息）
 * @see WEBSOCKET_COLLECTOR_CONFIG.WS_URL 获取 WebSocket 基础 URL
 */
 ...
 * @remarks 如果连接尚未建立，消息会被缓存到消息队列，待连接建立后自动发送
 */
...
/**
 * 设置消息接收回调函数
 * @param callback 消息接收回调，参数为标准化包装后的原始市场数据
 * @example
 * ```typescript
 * collector.onMessage((data) => {
...
| 字段名 | 访问修饰符 | 类型 | 注释内容 |
|--------|-----------|------|---------|
| `tasks` | private | Map\<string, CollectionTask\> | 任务映射表，key 为 taskId，value 为任务对象 |
| `timers` | private | Map\<string, ReturnType\<typeof setInterval\>\> | 定时器映射表，key 为 taskId，value 为定时器 ID |
| `collectors` | private | Map\<string, BaseCollector\> | 采集器映射表，key 为 taskId，value 为采集器实例 |
| `listeners` | private | Set\<CollectionResultCallback\> | 结果监听器集合 |
| `taskCounter` | private | number | 任务计数器，用于生成唯一的任务 ID |
...
/**
 * 注册采集任务
 * @param widgetId Widget ID，用于标识数据源对应的 Widget 组件
 * @param instanceId 实例 ID，同一 Widget 可能有多个实例
 * @param dataSource 数据源配置，包含 endpoint、headers、mode、interval 等信息
 * @returns 生成的唯一任务 ID
 ...
 * @remarks 根据 dataSource.mode 决定执行模式：
 * - polling: 定时轮询，立即执行一次后按 interval 定时执行
 * - once: 仅执行一次
 * - streaming: 流式采集（WebSocket 模式）
 * @throws {Error} 任务执行过程中可能抛出的采集错误
 */
 ...
 * 停止采集任务
 * @param taskId 任务 ID
 * @remarks 停止后任务状态变为 paused，可通过 startTask 重新启动。会清除定时器并取消采集器
 */
 ...
 * 注销采集任务
 * @param taskId 任务 ID
 * @remarks 会先停止任务，然后从任务映射表中删除，不可恢复
 */
 ...
 * @param config 采集配置
 * @returns 采集结果对象，包含 success、latency、fallbackCount 和可选的 error
 * @remarks 此方法不会注册到任务轮询列表，只产生一次 trace 并持久化。适用于手动触发的一次性采集场景
 */
...
/**
 * 清理所有任务（页面销毁时调用）
 * @remarks 防止内存泄漏的关键方法，必须在页面销毁时调用。会清除所有定时器、取消所有采集器、清空任务和监听器
 */
dispose(): void
```
 ...
 * @param taskId 任务 ID
 * @param error 错误对象
 * @remarks 单监听器异常不影响其余监听器。抽取为独立方法以消除调用处的 forEach+try/catch 嵌套（深度 > 3）
 * @private
 */
 ...
 * @returns 采集器实例
 * @remarks 未知类型时返回 MockCollector 作为兜底
 * @private
 */
...
| 文件 | 变更前状态 | 变更后状态 | 变更内容 |
|------|-----------|-----------|---------|
| **BaseCollector.ts** | 无类注释、无字段注释、无方法注释 | 完整的类注释、字段注释、构造函数注释、所有方法注释（含 @private/@protected/@abstract 标签） | 新增 9 处 JSDoc 注释 |
| **WebSocketCollector.ts** | 无类注释、无字段注释、无方法注释 | 完整的类注释、字段注释、构造函数注释、所有方法注释（含 @private/@override 标签） | 新增 10 处 JSDoc 注释 |
| **TaskScheduler.ts** | 无辅助函数注释、无常量注释、无类注释、无字段注释、无方法注释、无单例注释 | 完整的辅助函数注释、常量注释、类注释、字段注释、所有方法注释（含 @private 标签）、单例实例注释 | 新增 14 处 JSDoc 注释 |
...
- [ ] 类注释是否包含 `@description`、`@remarks` 和 `@example` 标签
- [ ] 公共方法是否包含完整的 `@param` 和 `@returns` 标签
- [ ] 私有方法是否标注了 `@private` 标签
- [ ] 抽象方法是否标注了 `@abstract` 标签
- [ ] 可能抛出异常的方法是否包含 `@throws` 标签
- [ ] 字段是否包含描述性注释
- [ ] 代码示例是否符合实际使用场景
- [ ] 注释内容是否与代码实现一致
- [ ] 是否符合项目的 JSDoc 规范（见 `../../jsdoc-convention.md`）

### 7.2 后续建议

1. **持续维护**：后续修改代码时应同步更新对应的 JSDoc 注释
2. **文档生成**：可使用 TypeDoc 工具自动生成 API 文档
3. **扩展覆盖**：按优先级继续为其他缺失注释的模块补充 JSDoc
...
- `RawMarketData` - 原始市场数据类型（`src/types/modules/widget.types.ts`）
- `DataSourceConfig` - 数据源配置类型（`src/types/modules/widget.types.ts`）
- `CollectorConfig` - 采集器配置类型（`src/types/modules/widget.types.ts`）
- `CollectionTask` - 采集任务类型（`src/types/modules/widget.types.ts`）
- `CollectionTaskStatus` - 任务状态类型（`src/types/modules/widget.types.ts`）
- `CollectionResultCallback` - 结果回调类型（`src/types/modules/widget.types.ts`）

### B. 相关常量

- `COLLECTOR_DEFAULT_CONFIG` - 默认采集器配置（`src/constants/cockpit.constants.ts`）
- `WEBSOCKET_COLLECTOR_CONFIG` - WebSocket 采集器配置（`src/constants/cockpit.constants.ts`）
- `DATA_SOURCE_TYPE` - 数据源类型常量（`src/constants/cockpit.constants.ts`）
- `COLLECTION_EVENTS` - 采集事件常量（`src/types/modules/collection.types.ts`）

...
4. **质量门禁与文档同步**：新增页面必须同步 `docs/06-routing-specs.md`、通过 `audit:layers`、`audit:routes`、`lint:colors`、`tsc:prod` 等。
...
5. `docs/06-routing-specs.md` — 同步路由说明。
...
6. **文档同步**：更新 `docs/06-routing-specs.md`。
...
4. **是否需要同步更新 `docs/06-routing-specs.md`**（推荐同步，否则 `audit:docs` 可能告警）？

...
summary: "P0 后更�?*：交易服务层已从 7 个引擎扩展至 20+ 个模块（�?tradeReviewAI、watchlistMoversService、策略快照等），详见..."
tags: [project, contract, management]
version: v1.0.0
last_updated: 2026-07-17
code_version: 2.0.0
doc_id: V9-DOC-PROJ-234
referenced_by: [V9-DOC-PROJ-174, V9-DOC-META-000, V9-DOC-PROJ-176, V9-DOC-ARCH-043, V9-DOC-PROJ-149]
change_log:
  - version: v1.0.0
...
> **P0 后更�?*：交易服务层已从 7 个引擎扩展至 20+ 个模块（�?tradeReviewAI、watchlistMoversService、策略快照等），详见 §9.10�?
## 2. 核心数据结构
...
| 2026-07-12 | v1.4.0 | **更新行号引用**（代码迭代后偏移修正）；**扩展 §9.10 模块清单**（从 7 个增�?20+ 个，�?tradeReviewAI、watchlistMoversService、策略快照等�?| Architecture Asset Governor |
...
> 以下�?`src/services/trading/` �?20+ 个引�?服务模块的核心接口定义�?
### 9.1 CreateOrderInput �?创建订单输入

**来源**: `src/services/trading/tradingService.ts:23-29`（原 v1.3.0 �?:22-28，代码迭代后行号偏移 +1�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `symbol` | `string` | �?| 股票代码 |
...
**来源**: `src/services/trading/tradingService.ts:55-59`（原 v1.3.0 �?:54-58，行号偏�?+1�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
...
**来源**: `src/services/trading/riskEngine.ts:9-15`（原 v1.3.0 �?:6-12，行号偏�?+3�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `symbol` | `string` | �?| 股票代码 |
...
**来源**: `src/services/trading/riskEngine.ts:19-23`（原 v1.3.0 �?:14-18，行号偏�?+5�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `ok` | `boolean` | �?| 是否通过 |
...
**来源**: `src/services/trading/portfolioBuilder.ts:10-22`（原 v1.3.0 �?:9-21，行号偏�?+1�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
...
**来源**: `src/services/trading/strategyEngine.ts:21-28`（原 v1.3.0 �?:17-24，行号偏�?+4�?
| 字段 | 类型 | 必填 | 描述 |
|------|------|------|------|
| `theme` | `ThemeConfig` | �?| 主题配置 |
| `ruleConfig` | `StrategyRuleConfig` | �?| 策略规则配置 |
...
| `tradeErrorClassifier` | `classifyTradeError` | 交易错误分类（类�?严重�?根因�?| **新增** |
| `tradeErrorDefinitions` | �?| 错误定义与错误码字典 | **新增** |
| `tradeErrorDetectors` | �?| 错误检测器集合 | **新增** |
| `tradeErrorUtils` | �?| 错误处理工具函数 | **新增** |
| `tradeReviewAI` | `generateReview`, `analyzeDimensions` | 交易复盘 AI（维度分�?+ LLM 增强�?| **新增** |
| `tradeReviewAI.dimensions` | `extractDimensions` | 复盘维度提取（技�?心理/策略�?| **新增** |
...
| `tradeReviewAI.skillDevelopment` | `generateSkillPlan` | 交易技能发展计�?| **新增** |
| `tradeReviewAI.types` | �?| 复盘类型定义 | **新增** |
| `tradeReviewAI.utils` | �?| 复盘工具函数 | **新增** |
| `watchlistMoversService` | `getWatchlistMovers` | 自选股异动监测与推�?| **新增** |

> **模块总数**：原 7 �?�?当前 **20+ �?*（含 tradeReviewAI 系列 7 个模�?+ watchlistMoversService 等独立模块）�?> **交易复盘能力**：v1.4.0 新增完整的交易复�?AI 能力（维度分�?�?LLM 增强 �?画像生成 �?报告输出 �?技能发展），覆�?`src/services/trading/tradeReviewAI*` 系列文件�?
<!-- merge-source: docs/explanation/design/api-contract.md (2026-07-14 内容融合，避免去重丢失有效信�? -->
## 补充内容（合并自 `./trade/api-contract.md`�?
> 以下�?`src/services/trading/` �?7 个引�?服务模块的核心接口定义�?**来源**: `src/services/trading/tradingService.ts:22-28`

...
summary: "定位：定义 `backtest` 子域的接口契约、职责边界、数据流与依赖关系。 关联：`./services-catalog.md`（24 子域总览）、`../../AGENTS.md`..."
...
| config/mathConstants | `@/config/mathConstants` | `TRADING_DAYS_PER_YEAR`（?? 跨层依赖：`services/` 应仅依赖 `core/`、`data/`、`lib/`） |

...
status: draft
maintainer: 架构组
summary: "定义 hybrid-proofread 子域的接口契约、职责边界、数据流与依赖关系。"
...
| 测试类型 | 文件 | 说明 |
|----------|------|------|
| 单元测试 | `src/services/hybrid-proofread/hashService.ts` | `computeHash`、`batchComputeHashes` 正确性与性能 |
| 单元测试 | `src/services/hybrid-proofread/ruleEngine.ts` | `evaluateFile` 正则匹配、规则过滤、降级逻辑 |
| 单元测试 | `src/services/hybrid-proofread/localCollector.ts` | `collectFiles` 路径过滤、并发控制、取消逻辑 |
| 单元测试 | `src/services/hybrid-proofread/reportGenerator.ts` | `countIssues`、`calculateOverallRiskLevel` 边界值 |

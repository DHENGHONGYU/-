...
status: draft
maintainer: 工程效能�?/ QA
summary: "src/data/ 阈值较低（35%），�?IndexedDB 迁移�?Schema 创建逻辑多为声明式，测试 ROI 低；src/core/ 侧重分支覆盖�?5%），�?ACL、Envelope�?
...
referenced_by: [V9-DOC-META-000, V9-DOC-QA-081, V9-DOC-QA-029, V9-DOC-PROJ-176, V9-DOC-QA-010, V9-DOC-PROJ-149, V9-DOC-QA-004]
change_log:
  - version: v1.0.0
...
# V9 测试目录与策�?
> **文档定位**：本文档�?V9 智能投研复盘系统全部测试资产�?*单一真相�?*（Single Source of Truth）�?> **适用范围**：所有新增模块的测试义务、CI 门禁配置、故障排查时的测试定位入口�?> **关联文档**�?> - [测试策略总览](../reference/testing-strategy.md) �?三层测试策略与改进路�?> - [AGENTS.md](../../AGENTS.md) �?四步集成回滚验证、事件监听清理模�?> - [运维基线](../explanation/runbook.md) �?CI/CD 流水线配�?> - [编码规范](coding-conventions.md) �?测试命名与目录约�?
---
...
/─────────────────────\规模：~15 个文件，含契约测�?suite
/                       \
/─────────────────────────\ 单元测试 (Vitest + jsdom)
/                           \职责：函数、Store、Service、组件、类型守�?/                             \规模：~200 个文件，测试资产主体
─────────────────────────────
```

| 层级 | 范围 | 工具/环境 | 目标 | 门禁位置 |
|------|------|-----------|------|----------|
| **单元测试** | 函数、Store、Service、工具类、原�?分子组件 | Vitest + jsdom | 核心模块覆盖�?�?70%，全局基线 80% | pre-commit / CI |
| **集成测试** | DataBridge、EventBus、ACL、Widget 注册三处同步、Store �?Tab 广播 | Vitest + fake-indexeddb | 模块间契约零违规 | CI |
| **E2E 测试** | 关键用户路径、路由跳转、驾驶舱渲染、表单提�?| Playwright (Chromium) | 核心流程全通过 | 发布�?|
| **视觉回归** | 驾驶舱、股票池看板、评分页关键截图 | Playwright `toHaveScreenshot` | 像素差异 �?2% | 发布�?|
...
�?  ├── *.spec.ts                 �?端到端测试用�?�?  ├── visual-regression.spec.ts �?视觉回归主入�?�?  ├── visual-regression.spec.ts-snapshots/ �?基线截图
�?  └── screenshots/              �?运行时截图产�?�?├── tests/                        �?单元 & 集成测试 (Vitest)
...
�?  �?  ├── lib/                  �?库函数测�?�?  �?  ├── orchestrator/         �?编排器测�?�?  �?  ├── services/             �?服务层补充测�?�?  �?  └── store/                �?Store 补充测试
�?  ├── __mocks__/                �?全局 mock
�?  ├── fixtures/                 �?测试数据�?�?  ├── helpers/                  �?测试辅助函数
�?  ├── performance/              �?性能基准测试
�?  ├── remediation/              �?修复不变量验�?�?  ├── unit/                     �?纯单元测�?�?  ├── services/                 �?服务层测�?�?  └── utils/                    �?工具函数测试
�?└── src/                          �?源码共置测试（推荐）
    ├── **/*.test.ts              �?与源码同目录的单元测�?    └── **/*.test.tsx             �?组件测试
```
...
| **集中** | `tests/` 根或子目�?| 跨模块集成测试、E2E、契约测试、性能测试、fixtures �?helpers�?|

> **决策规则**：单模块内聚测试 �?共置；跨模块契约/端到�?性能 �?集中�?
---
...
| 舱室 | 测试文件 | 说明 |
|------|----------|------|
| **Input (输入�?** | `tests/InputApp.test.tsx`<br>`tests/BulkImportPanel.test.tsx`<br>`tests/StockSearch.test.tsx`<br>`src/pages/analysis/HotSectorPage.test.tsx`<br>`tests/data-collector.test.ts`<br>`tests/batchImportService.test.ts`<br>`e2e/input-data-collection.spec.ts`<br>`e2e/input-stock-pool.spec.ts`<br>`e2e/bulk-import-full.spec.ts` | 采集向导、批量导入、股票搜索、数据收集器 |
| **Analysis (分析�?** | `tests/AnalysisApp.test.tsx`<br>`src/pages/analysis/StockAnalysisPage.test.tsx`<br>`src/pages/analysis/SectorAnalysisPage.test.tsx`<br>`src/pages/analysis/ValuePitPage.test.tsx`<br>`tests/IndustryScorePage.test.tsx`<br>`tests/IntelligentScorePage.test.tsx`<br>`e2e/analysis-scoring.spec.ts`<br>`e2e/analysis-extended.spec.ts`<br>`e2e/stock-score.spec.ts` | 个股/板块/行业/价值投资分析页 |
| **Trading (交易�?** | `src/apps/trading/TradingApp.test.tsx`<br>`tests/PoolBoard.test.tsx`<br>`tests/PoolList.test.tsx`<br>`e2e/trading.spec.ts`<br>`e2e/trade-review.spec.ts`<br>`e2e/pool-group.spec.ts` | 交易舱壳、股票池看板、交易复�?|
| **Output (输出�?** | `tests/OutputApp.test.tsx`<br>`src/pages/output/__tests__/OutputHubPage.test.tsx`<br>`e2e/output-cabin.spec.ts`<br>`e2e/output-command.spec.ts` | 输出舱壳、报告中�?|
| **Command (指令�?** | `tests/CommandApp.test.tsx`<br>`tests/ConfigApp.test.tsx`<br>`src/pages/command/agent/AgentHubPage.test.tsx`<br>`src/pages/command/agent/AgentRegistryPage.test.tsx`<br>`src/pages/command/agent/__tests__/*.test.tsx`<br>`src/pages/command/__tests__/MCPServerDashboardPage.test.tsx`<br>`e2e/mcp-verify.spec.ts` | 系统配置、Agent 中心、MCP 仪表�?|
| **Cockpit (驾驶�?** | `src/cockpit/CockpitShell.test.tsx`<br>`src/cockpit/core/widgetEngine.test.ts`<br>`src/cockpit/core/widgetRegistry.test.ts`<br>`src/cockpit/core/widgetRegistrySync.test.ts`<br>`src/cockpit/widgets/*.test.tsx` | 驾驶舱壳、Widget 引擎、全�?Widget |
...
### 3.3 �?Store 分类�?9 �?Zustand Store，含测试�?26+�?
| 舱室 | Store 测试文件 |
|------|----------------|
...
> TODO(架构�?: 剩余 ~23 �?Store 尚未补充单元测试，按优先级分批补齐。详�?`npm run audit:tests` 输出�?
...
> TODO(设计系统): organisms 层级覆盖不足，特别是 `cockpit/` �?`charts/` 子目录的复杂组件。优先补�?`ErrorBoundary`、`ScoreFactorDeltaPanel`�?
### 3.5 核心层与数据层测�?
| 层级 | 测试文件 |
|------|----------|
...
以下 8 个测试文件因**外部依赖不稳�?*�?*环境敏感**�?*遗留债务**�?`npm run test:clean` 显式排除。CI 使用 `test:clean` 作为绿色基线；`test:known` 用于单独验证这些问题文件�?
| # | 文件路径 | 排除原因 | 修复优先�?| Owner |
|---|----------|----------|-----------|-------|
| 1 | `src/store/agentStore.test.ts` | LLM 多模型切换涉及外�?API Key 与网络抖动，Mock 未完全隔�?| P1 | Store �?|
| 2 | `src/services/fetcher/fetcherClient.test.ts` | 数据源适配器涉及网�?I/O，fake-indexeddb 与并发请求竞�?| P1 | Fetcher �?|
| 3 | `src/services/llm/llmClient.multimodel.test.ts` | 多模�?Provider 切换依赖环境变量，CI 无密钥时跳过逻辑导致空跑 | P1 | LLM �?|
| 4 | `tests/fetcher/dataSourceProvider.test.ts` | 第三方数据源 Provider 接口变更频繁，契约测试待补齐 | P2 | Fetcher �?|
| 5 | `tests/ui-components.test.tsx` | 旧版 UI 组件测试，已迁移�?`src/components/**` 共置测试，存在重复断言 | P2 | 前端�?|
| 6 | `tests/agentModule.integration.test.tsx` | Agent 模块重构中，集成点待稳定（MCP Server 动态注册变更） | P2 | Agent �?|
| 7 | `tests/engine.test.ts` | 引擎 L0-L8 全链路测试过重，需拆分为分层单元测试（已有 `v6-engine/calculators/*.test.ts`�?| P1 | 引擎�?|
| 8 | `tests/sectorScoreService.test.ts` | 板块评分涉及实时行情数据，Mock 数据与真实数据结构漂�?| P2 | Scoring �?|
...
# 单独运行已知问题文件（调试用�?npm run test:known

# 全量运行（含已知问题，可能失败）
npm test -- --run
```

> **�?*：`test:clean` �?`test:known` 的排除清单必须在 `package.json` �?*双向同步**——新增排除文件时，两脚本须同时更新。审计脚�?`audit:tests` 会自动校验此一致性�?
---
...
| 指标 | 目标�?| 当前状�?| 说明 |
|------|--------|----------|------|
| **Statements** | �?80% | 🟡 待测�?| 全局基线，自主修复阈值（�?AGENTS.md §十） |
| **Branches** | �?75% | 🟡 待测�?| 条件分支覆盖 |
| **Functions** | �?80% | 🟡 待测�?| 函数入口覆盖 |
| **Lines** | �?80% | 🟡 待测�?| 行覆�?|

### 5.2 分层阈值（`vite.config.ts` 硬性配置）

以下阈值写�?Vitest `coverage.thresholds`，未达标�?CI 失败�?
...
> **说明**：`src/data/**` 阈值较低（35%），�?IndexedDB 迁移�?Schema 创建逻辑多为声明式，测试 ROI 低；`src/core/**` 侧重分支覆盖�?5%），�?ACL、Envelope、RouteGuard 的条件分支直接影响安全�?
### 5.3 覆盖率报告生�?
```powershell
# 生成 HTML + JSON + TEXT 报告
npm run coverage
# 输出：coverage/ 目录（已纳入 .gitignore�?
# CI 场景�?Istanbul provider，兼�?Windows forks 池）
npm run test:ci
```
...
| **Factories** | `tests/unit/mockFactories.ts` | 动态生成假数据的工厂函�?|
| **Helpers** | `tests/helpers/` | LLM Mock Fetch、Widget 测试工具 |
| **局�?Mock** | 测试文件�?`vi.mock()` | 模块级精细控�?|
...
// 每个测试前重置数据库状态，防止跨测试污�?beforeEach(async () => {
  // TODO: 补充具体重置逻辑
})
```
...
| 场景 | 策略 |
|------|------|
| LLM 调用 | `tests/helpers/llmMockFetch.ts` 提供统一�?`mockLlmResponse()`，拦�?`fetch` 并返回结构化假数�?|
| 行情数据 | `tests/mockStockData.ts` 提供静�?K 线与报价数据 |
| 数据�?Provider | `tests/fixtures/store-mock-data.ts` 中的预置数据�?|
...
### 7.1 定时器清理（强制�?
AGENTS.md §�?规定：测试中使用 `vi.useFakeTimers()` 必须�?`afterEach` �?`vi.useRealTimers()`�?
```typescript
// �?标准模板：fakeTimers 清理
...
// 已全局配置�?tests/setup.ts，无需每文件重�?// afterEach(() => cleanup())
```
...
| 配置�?| �?| 说明 |
|--------|-----|------|
| 工具 | Playwright `toHaveScreenshot` | 像素级对�?|
| 容差 | `maxDiffPixelRatio: 0.02` | 允许 2% 像素差异，避免字�?动画微差误报 |
| 阈�?| `threshold: 0.2` | 单像素色差阈�?|
| 浏览�?| Desktop Chrome | 单一浏览器减少基线数�?|
| 基线目录 | `e2e/visual-regression.spec.ts-snapshots/` | 已纳入版本控�?|

### 8.2 运行命令

```powershell
# 运行视觉回归测试（与基线对比�?npm run test:e2e:visual

# 更新基线（UI 变更已确认后�?npm run test:e2e:visual:update

# 查看可视化报�?npm run test:e2e:ui
```

### 8.3 场景清单（`e2e/visual-regression.spec.ts`�?
| # | 场景 | 说明 |
|---|------|------|
| 1 | 驾驶舱全屏渲�?| CockpitShell + 默认 Widget 布局 |
| 2 | 分析舱评分页 | 智能评分瀑布�?+ 因子贡献 |
| 3 | 输入舱采集向�?| 多步骤表�?|
| 4 | 交易舱持仓看�?| 表格 + 卡片混合布局 |
| 5 | 暗色模式切换 | 主题令牌暗色变体验证 |

> TODO(前端�?: 当前仅覆�?5 个场景，目标扩展�?20 个，覆盖全部舱室首屏与关键交互态�?
---

## 9. 回滚验证流程（四步集成回滚专用）

当执�?AGENTS.md §二「四步集成」的回滚操作时，必须按以下顺序验证：
...
# 3. 接口签名一致性（若涉及路由或数据字典变更�?# 手动更新 docs/02-design/06-routing-specs.md �?data-dictionary-index.md

# 4. 架构合规
npm run audit:layers

# 5. 单元测试通过（clean 基线�?npm run test:clean

# 6. 可选：E2E 回归（重大回滚时�?npm run test:e2e
```
...
```powershell
# ── 单元测试 ──
npm run test              # 全量运行（Vitest run�?npm run test:watch        # 监听模式
npm run test:ci           # CI 模式 + 覆盖�?npm run test:staged       # 仅与 git staged 文件相关的测�?npm run test:clean        # 排除已知问题文件的绿色基�?npm run test:known        # 仅运行已知问题文件（调试�?
# ── E2E & 视觉回归 ──
npm run test:e2e          # Playwright 全量 E2E
npm run test:e2e:ui       # Playwright UI 模式（可视化调试�?npm run test:e2e:visual   # 视觉回归测试
npm run test:e2e:visual:update  # 更新视觉基线

# ── 覆盖�?──
npm run coverage          # 生成覆盖率报�?
# ── 测试审计 ──
npm run audit:tests       # 测试文件合规性扫�?```
...
| 2 | ~23 �?Store 缺少单元测试 | Store �?| P2 |
| 3 | organisms 层级组件测试覆盖不足 | 前端�?| P2 |
| 4 | 视觉回归场景�?5 个，目标 20 �?| 前端�?| P3 |
| 5 | `tests/engine.test.ts` 待拆分为分层测试 | 引擎�?| P1 |
| 6 | `src/data/**` 覆盖率仅 35%，需评估是否提升 | 数据�?| P3 |
| 7 | E2E 测试目前仅覆�?Chromium，需评估 Safari/Firefox | QA | P3 |
...
- [AGENTS.md §�?�?四步集成与回滚验证](../../AGENTS.md)
- [AGENTS.md §�?�?事件监听清理模板](../../AGENTS.md)
- [AGENTS.md §�?�?验证命令速查](../../AGENTS.md)

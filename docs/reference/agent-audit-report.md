...
summary: "agent-audit-report - reference documentation (ai)"
tags: [ai, agent, audit, report, reference]
version: v1.0.0
last_updated: 2026-07-17
code_version: 2.0.0
doc_id: V9-DOC-AI-014
change_log:
  - version: v1.0.0
...
> **Date**：2026-06-27
> 审计范围：`src/agents/`、`src/services/llm/`、`src/services/scoring/`、`src/services/trading/`、`src/cockpit/`、`src/services/scoring/v6-engine/engine.ts`、`src/constants/ai-center.constants.ts`、`.agents/skills/`
...
|---|-----------|----------|-------------|
| 1 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | `/analysis/intelligent-score` | 直接调用 `llmClient.chat()` |
| 2 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | `/analysis/industry-score` | 直接调用 `llmClient.chat()` |
...
### 4.5 Agent 系统 ? Engine 层 — 孤岛 ??

| 检查项 | 结果 | 说明 |
|--------|------|------|
| `src/showcase/index.ts` 是否 import AgentRuntime | **是** | 创建了独立实例 `new AgentRuntime()` |
...
| SKILL 名称 | 版本 | 项目目录 | 状态 |
|------------|------|----------|------|
| v6-stock-analysis-model | v4.3 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | ? 已同步 |
| sector-analysis-framework | v1.0 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | ? 已同步 |
| valuation-financial-analysis | v1.1 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | ? 已同步 |
| industry-score-mapping | v1.0 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | ? 已同步 |
| v6-docx-output | v1.0 | `../../.agents/skills/feature-window-context-doc/SKILL.md` | ? 已同步 |
| portfolio-allocation-strategy | v1.0 | `.agents/skills/portfolio-allocation-strategy/` | ?? 目录已建，SKILL 待补充 |
| pharma-sector-analysis | v1.0 | `.agents/skills/pharma-sector-analysis/` | ?? 目录已建，SKILL 待补充 |
| md-to-docx | v1.1 | - | ? 未上传 |
...
| **L0 STEEP 宏观** | 13项子维度评分指引 + 主题框架可插拔 | ?? 占位符 | 仅有简化评分，无 STEEP 子维度、无主题框架 |
| **L1 护城河** | 5类护城河×4条量化标准 + 另类因子 ALT-02/04/05/08 | ?? 占位符 | 有基础框架但无量化标准，无另类因子 |
| **L2 竞品** | 技术代差/市场份额/客户认证三维量化 + ALT-03 | ?? 占位符 | 无竞品对比维度，无客户认证阶段 |
| **L3 财务** | 5维财务Rubric + 红黄风险预警(9条) + **IPC临界点判断**(OCR/MCE/TIMS) | ?? 占位符 | 仅基础财务加权，**完全缺失 IPC/OCR/MCE/TIMS**，无风险预警 |
| **L3 估值** | 8行业PE/PEG/PB基准库 + DCF | ?? 占位符 | 无行业基准库 |
| **L4 情景** | 三情景概率加权 + 目标价推导公式 + 收益比 | ?? 占位符 | 无情景构建逻辑，无目标价推导 |
| **L5 T-M** | 技术成熟度/市场成熟度双轴 + 象限策略 | ?? 占位符 | 无 T-M 矩阵实现 |
| **L6 Hype** | Gartner五阶段 + 评分Rubric | ?? 占位符 | 无 Hype Cycle 定位 |
| **L7 第二曲线** | 生命阶段诊断 + 催化剂强度 + 主题框架 | ?? 占位符 | 无第二曲线识别逻辑 |
| **L8 筹码** | **8级筹码变化度**(SCD/PCH/AII/博弈矩阵/RSI/CCS/DIV/CSR) + 量价因子 | ?? 占位符 | **完全缺失筹码变化度体系**，仅有简单筹码集中度 |
...
| Phase-A | Engine 消除双实例 | `src/showcase/index.ts` |
...
| Phase-D | stockAnalysisEngine LLM 增强 | `src/services/analysis` |
...
| `src/showcase/index.ts` | Engine 编排层 |
| `../../.agents/skills/feature-window-context-doc/SKILL.md` | 个股评分 Skill 规范 |
| `../../.agents/skills/feature-window-context-doc/SKILL.md` | 行业评分 Skill 规范 |

### 占位符
| 文件 | 说明 |
|------|------|
| `src/services/analysis` | V6 L0-L8 九层分析（全部占位符） |

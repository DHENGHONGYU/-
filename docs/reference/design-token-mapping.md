...
> **适用范围**: `src/components/`、`src/pages/`、`src/cockpit/`、`src/apps/` 等所有 UI 层

---

## 1. 为什么需要这份映射表

颜色令牌系统已经从单一 `theme.tokens.ts` 拆分为 L1–L6 六个层次。实际开发中常见的问题是：
...
本表按照“业务场景 → 令牌层级 → 推荐 Import → 代码示例 → 常见错误”进行映射，作为提示词模板和 Husky 门禁的共同依据。
...
```text
1. 是否涉及股票涨跌幅？
   ├─ 是 → 使用 STOCK_COLOR_TOKENS（L5），见 §3.1
   └─ 否 → 继续下一步

2. 是否用于图表 / 热力图 / 雷达图 / Recharts / canvas？
   ├─ 是 → 使用 chartColors.ts 业务调色板（L4），见 §3.4
   └─ 否 → 继续下一步
...
| 信息色 HEX | `THEME_TOKENS.color.infoRaw` | `<span style={{ color: THEME_TOKENS.color.infoRaw }} />` | `#3b82f6` |
...
| 成功色 HEX | `THEME_TOKENS.color.successRaw` | `<span style={{ color: THEME_TOKENS.color.successRaw }} />` | `#22c55e` |
...
| 警告色 HEX | `THEME_TOKENS.color.warningRaw` | `<span style={{ color: THEME_TOKENS.color.warningRaw }} />` | `#f59e0b` |
...
| 危险色 HEX | `THEME_TOKENS.color.destructiveRaw` | `<span style={{ color: THEME_TOKENS.color.destructiveRaw }} />` | `#ef4444` |
...
| 弱化色 HEX | `THEME_TOKENS.color.mutedRaw` | `<span style={{ color: THEME_TOKENS.color.mutedRaw }} />` | `#9ca3af` |
...
| 业务场景 | 推荐 Token | 代码示例 | 禁止写法 |
|----------|------------|----------|----------|
| **股票上涨（A股）** | `STOCK_COLOR_TOKENS.up` | 见 §4.1，禁止用 COLOR_TOKENS.up | `COLOR_TOKENS.up` |
...
| 景气因子 | `COLOR_TOKENS.factorJingqi` | 见 §3.4 的 `ROTATION_FACTOR_COLORS` | `#ef4444` |
| 资金因子 | `COLOR_TOKENS.factorZijin` | 见 §3.4 的 `ROTATION_FACTOR_COLORS` | `#f59e0b` |
| 估值因子 | `COLOR_TOKENS.factorGuzhi` | 见 §3.4 的 `ROTATION_FACTOR_COLORS` | `#3b82f6` |
| β 因子 | `COLOR_TOKENS.factorBeta` | 见 §3.4 的 `ROTATION_FACTOR_COLORS` | `#8b5cf6` |
| 量能因子 | `COLOR_TOKENS.factorNengliang` | 见 §3.4 的 `ROTATION_FACTOR_COLORS` | `#06b6d4` |
| 成长风格 | `COLOR_TOKENS.styleGrowth` | 见 §3.4 的 `MARKET_STYLE_COLORS` | `#3b82f6` |
| 价值风格 | `COLOR_TOKENS.styleValue` | 见 §3.4 的 `MARKET_STYLE_COLORS` | `#10b981` |
| 均衡风格 | `COLOR_TOKENS.styleBalanced` | 见 §3.4 的 `MARKET_STYLE_COLORS` | `#8b5cf6` |
| 强信号 | `COLOR_TOKENS.signalStrong` | 见 §3.4 的 `SIGNAL_GRADE_COLORS` | `#10b981` |
| 中强信号 | `COLOR_TOKENS.signalMediumStrong` | 见 §3.4 的 `SIGNAL_GRADE_COLORS` | `#22c55e` |
| 中信号 | `COLOR_TOKENS.signalMedium` | 见 §3.4 的 `SIGNAL_GRADE_COLORS` | `#3b82f6` |
| 弱信号 | `COLOR_TOKENS.signalWeak` | 见 §3.4 的 `SIGNAL_GRADE_COLORS` | `#f59e0b` |
| 无信号 | `COLOR_TOKENS.signalNone` | 见 §3.4 的 `SIGNAL_GRADE_COLORS` | `#9ca3af` |
...
| 业务场景 | 推荐调色板 | 代码示例 | 禁止写法 |
|----------|------------|----------|----------|
| 多系列饼图/柱状图 | `PIE_CHART_PALETTE` | `<PieChart colors={PIE_CHART_PALETTE} />` | `[#3b82f6, #22c55e, ...]` |
| 轮动因子图 | `ROTATION_FACTOR_COLORS` | `lineColor={ROTATION_FACTOR_COLORS.JINGQI}` | `COLOR_TOKENS.factorJingqi.hex`（优先） |
| 市场风格图 | `MARKET_STYLE_COLORS` | `fill={MARKET_STYLE_COLORS.GROWTH}` | `#3b82f6` |
| 信号分级图 | `SIGNAL_GRADE_COLORS` | `color={SIGNAL_GRADE_COLORS.STRONG}` | `#10b981` |
| 得分分档图 | `SCORE_BUCKET_COLORS` | `color={SCORE_BUCKET_COLORS.HIGH}` | `#22c55e` |
| 下跌性质图 | `DECLINE_NATURE_COLORS` | `color={DECLINE_NATURE_COLORS.KILL_LOGIC}` | `#ef4444` |
| 高景气抛售预警 | `ALERT_LEVEL_COLORS` | `color={ALERT_LEVEL_COLORS.RED}` | `#ef4444` |
| 股票池状态 | `STOCK_POOL_STATUS_RAW_COLORS` | `colors={STOCK_POOL_STATUS_RAW_COLORS}` | `[#22c55e, #3b82f6, ...]` |
| 资讯情感趋势 | `SENTIMENT_TREND_COLORS` | `color={SENTIMENT_TREND_COLORS.positive}` | `#10b981` |
| AI 智能体标签 | `AI_AGENT_TAG_COLORS` | `color={AI_AGENT_TAG_COLORS.LLM}` | `#8b5cf6` |
| 中性/占位颜色 | `NEUTRAL_COLOR` | `color={NEUTRAL_COLOR}` | `THEME_TOKENS.color.mutedRaw` |
| 通用图表色 | `CHART_PALETTE` | `fill={CHART_PALETTE.series1}` | `#3b82f6` |
| 通用网格线 | `CHART_PALETTE.grid` | `stroke={CHART_PALETTE.grid}` | `#e5e7eb` |
| 通用坐标轴文字 | `CHART_PALETTE.axis` | `fill={CHART_PALETTE.axis}` | `#6b7280` |

### 3.5 主题感知设计系统（L6 SEMANTIC_COLOR_ROLES）

> **优先使用场景**：新页面、新组件、不依赖历史 Tailwind 类的模块。`SEMANTIC_COLOR_ROLES` 基于 CSS 变量，自动适配亮/暗色主题。
...
| 品牌主色柔色 | `SEMANTIC_COLOR_ROLES.primary.soft` | `<div className={SEMANTIC_COLOR_ROLES.primary.soft} />` | 浅底深色字 |
| 品牌主色前景 | `SEMANTIC_COLOR_ROLES.primary.fg` | `<span className={SEMANTIC_COLOR_ROLES.primary.fg} />` | 用于主色背景上的文字 |
| 中性文字 | `SEMANTIC_COLOR_ROLES.neutral.text` | `<span className={SEMANTIC_COLOR_ROLES.neutral.text} />` | 对应 text-foreground |
| 中性背景 | `SEMANTIC_COLOR_ROLES.neutral.bg` | `<div className={SEMANTIC_COLOR_ROLES.neutral.bg} />` | 对应 bg-background |
| 中性柔色 | `SEMANTIC_COLOR_ROLES.neutral.soft` | `<div className={SEMANTIC_COLOR_ROLES.neutral.soft} />` | 对应 muted |
...
| 强边框 | `SEMANTIC_COLOR_ROLES.surface.borderStrong` | `<div className={SEMANTIC_COLOR_ROLES.surface.borderStrong} />` | 对应 border-divider |
| 成功柔色 | `SEMANTIC_COLOR_ROLES.success.soft` | `<div className={SEMANTIC_COLOR_ROLES.success.soft} />` | 浅底深色字 |
| 警告柔色 | `SEMANTIC_COLOR_ROLES.warning.soft` | `<div className={SEMANTIC_COLOR_ROLES.warning.soft} />` | 浅底深色字 |
| 危险柔色 | `SEMANTIC_COLOR_ROLES.danger.soft` | `<div className={SEMANTIC_COLOR_ROLES.danger.soft} />` | 浅底深色字 |
| 信息柔色 | `SEMANTIC_COLOR_ROLES.info.soft` | `<div className={SEMANTIC_COLOR_ROLES.info.soft} />` | 浅底深色字 |
| 主色 HEX | `SEMANTIC_COLOR_ROLES.primary.raw` | `style={{ color: SEMANTIC_COLOR_ROLES.primary.raw }}` | 亮色模式代表值 |
| 成功 HEX | `SEMANTIC_COLOR_ROLES.success.raw` | `style={{ color: SEMANTIC_COLOR_ROLES.success.raw }}` | 亮色模式代表值 |
...
| 内边距 xs | `THEME_TOKENS.spacing.xs` | `<div className={THEME_TOKENS.spacing.xs} />` | `p-1` |
| 内边距 md | `THEME_TOKENS.spacing.md` | `<div className={THEME_TOKENS.spacing.md} />` | `p-4` |
| 水平内边距 sm | `THEME_TOKENS.spacing.pxSm` | `<div className={THEME_TOKENS.spacing.pxSm} />` | `px-2` |
| 垂直内边距 md | `THEME_TOKENS.spacing.pyMd` | `<div className={THEME_TOKENS.spacing.pyMd} />` | `py-2` |
...
| 字间距 wide | `THEME_TOKENS.typography.letterSpacing.wide` | `<span className={THEME_TOKENS.typography.letterSpacing.wide} />` | `tracking-wide` |
| 焦点环宽度 | `THEME_TOKENS.focusVisible.ringWidth` | `<input className={THEME_TOKENS.focusVisible.ringWidth} />` | `ring-2` |
| 焦点环颜色 | `THEME_TOKENS.focusVisible.ringColor` | `<input className={THEME_TOKENS.focusVisible.ringColor} />` | `ring-blue-500` |
| 过渡动画 | `THEME_TOKENS.motion.fadeIn` | `<div className={THEME_TOKENS.motion.fadeIn} />` | `transition-opacity duration-300 ease-in-out` |
| 骨架屏脉冲 | `THEME_TOKENS.motion.skeletonPulse` | `<div className={THEME_TOKENS.motion.skeletonPulse} />` | `animate-pulse` |
| 加载旋转 | `THEME_TOKENS.motion.spin` | `<div className={THEME_TOKENS.motion.spin} />` | `animate-spin` |
| 优秀评分阈值 | `THEME_TOKENS.score.excellent` | `score >= THEME_TOKENS.score.excellent` | 魔法数字 4 |
| 良好评分阈值 | `THEME_TOKENS.score.good` | `score >= THEME_TOKENS.score.good` | 魔法数字 3 |
| 可接受阈值 | `THEME_TOKENS.score.ok` | `score >= THEME_TOKENS.score.ok` | 魔法数字 2 |
...
- 中国 A 股标准：**红涨绿跌**；
- 所有涉及个股/指数/ETF 涨跌幅的颜色，必须使用 `STOCK_COLOR_TOKENS`；
- 暗色模式下，股票涨跌颜色**不变**（豁免主题切换）；
- 禁止用 `COLOR_TOKENS.up/down` 或 `THEME_TOKENS.color.success/destructive` 替代。
...
`chartColors.ts` 中的 L4 图表调色板已经集中管理，允许使用其内部 HEX 值，但禁止在图表组件之外使用这些 HEX 值。如果图表业务场景与 `COLOR_TOKENS` 语义重合，优先引用 `COLOR_TOKENS.*.hex`。

### 4.3 令牌定义文件本身

`src/constants/theme/` 和 `src/config/chartColors.ts` 中允许出现字面量 Tailwind 类名与 HEX 值，因为它们是“源头”。其他所有文件通过 import 消费，不得在源码中裸写。

### 4.4 第三方库覆盖

当必须覆盖第三方组件库的样式（如 shadcn/ui、Recharts 默认主题）时，应在 `src/index.css` 或专门的覆盖文件中使用 CSS 变量，并通过设计令牌导出，而不是在业务组件中直接写 `!text-blue-500`。
...
| 错误写法 | 问题 | 正确写法 |
|----------|------|----------|
| `text-red-500` | 硬编码 Tailwind 颜色类 | 状态色用 `THEME_TOKENS.color.destructive`；股票用 `STOCK_COLOR_TOKENS.up.tailwind` |
...
| `rounded-lg` | 硬编码圆角 | `THEME_TOKENS.radius.lg` |
| `gap-3` | 硬编码间隙 | `THEME_TOKENS.gap.md` |
| 直接使用 `#ef4444` | 硬编码 HEX | 股票用 `STOCK_COLOR_TOKENS.up.hex`；图表用 `CHART_PALETTE.upColor` |
| `score >= 4` | 硬编码阈值 | `score >= THEME_TOKENS.score.excellent` |

---

## 6. AI 提示词注入模板

当使用 AI 生成 UI 代码时，必须在提示词中显式注入以下约束：

```markdown
## 颜色与令牌使用约束
- 禁止在组件文件中直接书写 HEX 颜色值或 Tailwind 颜色类名（如 `text-red-500`、`bg-blue-50`）。
...
新增或修改 UI 代码时，按以下清单自审：

- [ ] 源码中不存在 `text-*` / `bg-*` / `border-*` 等硬编码 Tailwind 颜色类（除 `theme/` 与 `chartColors.ts` 外）；

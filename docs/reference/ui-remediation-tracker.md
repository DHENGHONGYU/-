...
2. **新建 `src/components/cockpit/SignalSpectrum.test.tsx`**（RM-007 验收：锚点单测通过）—— 4 项：渐变含 `var(--muted)` 与 success raw / 越界 clamp / 带 label 显示档位 / compact 高度。
...
改动：
1. **新建 `src/components/widgets/WidgetAnchorBar.tsx`** —— 顶部签名谱条（纯视觉锚点，**无标题**避免双头）：
...
- 新建 **`src/components/ui/states/states.test.tsx`**（4 项）：Loading 渲染 label+`role=status` / Empty 渲染 message+description+action 回调 / ErrorState 渲染标题+重试回调 / Skeleton `aria-hidden` 装饰占位。
...
改动：
1. **新建 `src/cockpit/WidgetStateBoundary.tsx`** —— 数据级四态边界：
...
**RM-013 ResultCard 成品卡（`src/components/output/ResultCard.tsx` 新）**
...
- 新建 `.workbuddy/skills/ui-design-audit/SKILL.md`：编码十项经典设计原则打分卡（Nielsen/Rams，含 V9 当前水位与对接阶段）、五道机器门禁（lint:colors / token-scan / audit:layers / tsc:prod / audit:hardcode）及**环境陷阱**（系统 Node 24 + 项目 node_modules + 关沙箱）、RM-001~016 可追踪矩阵、审计报告模板。供后续 UI 合并前主动调用拦截设计令牌违规与裸色硬编码。
...
1. **tsc 预存错误（已于 P2 清除）**：
   - `src/components/cockpit/SignalSpectrum.tsx(45,59)`: `mutedRaw` 在 `COLOR_TOKENS` 中不存在 —— **P2 RM-007 已修复**（改用 `var(--muted)`），全仓 tsc 现 **0 错误** ✅。

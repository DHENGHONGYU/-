...
| V10 | V9 现状 | 适配建议 |
|-----|---------|----------|
| `src/core/stateBoard.ts` 作为跨模块唯一状态 truth source | V9 使用 `eventBus` + Zustand + DataBridge | 可借鉴「字段契约」思想，但不必完全替换现有机制。建议在 `./data-interaction-protocols.md` 中增加「共享字段契约」章节 |

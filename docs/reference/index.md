...
```
<用户目录>\AppData\Roaming\kimi-desktop\daimon-share\daimon\runtime\kimi-code\home\plugins\managed\<plugin>\skills\<plugin>\SKILL.md
```
此路径位于系统盘，与您的工作目录隔离，**无法在工作目录中直接打开**。

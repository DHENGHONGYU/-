---
title: 如何新增一�?Store（Zustand + withBroadcast�?
type: how-to
domain: data
phase: development
tier: standard
...
summary: "版本：v1.0.0 日期�?026-07-12 目标：在 15 分钟内完成新�?Store 的全流程，不踩注册遗漏、广播缺失、类型守卫等常见�?"
tags: [data, store, guide]
version: v1.0.0
last_updated: 2026-07-17
code_version: 2.0.0
doc_id: V9-DOC-DATA-055
referenced_by: [V9-DOC-META-000, V9-DOC-BACK-026, V9-DOC-PROJ-122, V9-DOC-PROJ-176, V9-DOC-DATA-061, V9-DOC-PROJ-157, V9-DOC-DATA-073, V9-DOC-PROJ-149]
change_log:
  - version: v1.0.0
...
# 如何新增一�?Store（Zustand + withBroadcast�?
> **版本**：v1.0.0  
> **日期**�?026-07-12  
> **目标**：在 15 分钟内完成新�?Store 的全流程，不踩注册遗漏、广播缺失、类型守卫等常见�?
---

## 前置检�?
在开始之前，确认以下事项�?
- [ ] 已阅�?`../tutorials/getting-started.md`（了解四步集成契约）
- [ ] 已确�?Store 的数据类型（�?`src/types/` �?`src/data/types.ts` 中定�?Interface�?- [ ] 已确定对应的 Service 层接口（Store 只能依赖 `services/` �?`core/`�?- [ ] 已检�?`src/constants/store-channels.constants.ts` 中是否有合适的事件名，或需要新�?
---

## 步骤 1：定义类型（3 分钟�?
�?`src/types/modules/` �?`src/data/types.ts` 中定�?Store 相关的类型�?
```typescript
// 示例：新�?sectorAnalysisStore 的类�?export interface SectorAnalysisData {
  sectorId: string
  sectorName: string
  score: number
  trend: 'up' | 'down' | 'flat'
...
> **规则**：复杂泛型必须有 `Expect<Equals>` 类型测试（位�?`tests/__tests__/types/`）�?
---

## 步骤 2：创�?Store�? 分钟�?
�?`src/store/` 下新建文件，命名规范：`{domain}Store.ts`（camelCase + `Store` 后缀）�?
### 2.1 标准模板
 ...
 * @see @/pages/analysis/SectorAnalysisPage.tsx - 消费�? */
...
export const useSectorAnalysisStore = create<SectorAnalysisState>((set, get) => ({
  ...
  loadSectors: async () => {
    logger.info('[sectorAnalysisStore] loadSectors 开�?)
  ...
  loadTrend: async (sectorId: string) => {
    logger.info(`[sectorAnalysisStore] loadTrend 开�? ${sectorId}`)
    set({ loading: true, error: null })
...
// ============================================================
// 派生查询（可选）
// 设计原则：派生查询独立函数模式，通过 getState() 访问状态，不存�?State
// ============================================================
...
| `initialState` | �?必须 | 提取为常量对�?| �?create 中内联定义，难以重置 |
| `withBroadcast()` | �?必须 | 写操作后广播�?Tab 事件 | 忘记广播，其�?Tab 数据不同�?|
| `logger.info/error` | �?必须 | 核心分支打印日志 | 无日志，线上难以调试 |
| 错误处理 `try/catch` | �?必须 | 所�?async Action 包裹 | 未捕获异常，导致 UI �?loading |
| `set({ loading: false })` | �?必须 | 无论成功失败都要关闭 loading | 成功时未关闭 loading |
| 派生查询 `.derived.ts` | 🟡 建议 | 复杂计算�?memoizeByRef 缓存 | �?State 中存派生数据，导致冗余渲�?|

---

## 步骤 3：注册广播事件（2 分钟�?
如果使用了新的事件名，必须在 `src/constants/store-channels.constants.ts` �?`EVENT_NAMES` 中注册：
...
�?`src/store/sectorAnalysisStore.test.ts` 中创建：
...
# 确认：sectorAnalysisStore.ts 无跨层调用违�?```
...
// �?正确：通过 Service 层间接访�?import { loadSectorAnalysis } from '@/services/analysis/sectorAnalysisService'
const result = await loadSectorAnalysis()
```
...
// �?正确：set 后广�?set({ sectors: result.data })
withBroadcast(EVENT_NAMES.SECTOR_ANALYSIS_CHANGED, { action: 'load' })
```

### �?3：在 create 中定义内联函数导致类型推断丢�?
```typescript
// �?错误：内联定义，类型推断不完�?export const useStore = create<State>((set) => ({
  loadData: async () => { /* ... */ }, // 类型推断可能不完�?}))
...
// �?正确：派生查询独立函�?export function getTopSector(): SectorData | null {
...
| ../../AGENTS.md 契约 | `../../AGENTS.md` | 分层规则与四步集�?|

---

> **验证完成�?*：更�?`docs/README.md` �?C 类索引，将新�?Store 链接回主索引�
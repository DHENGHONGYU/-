---
title: 如何新增一�?Widget（WidgetShell + 事件总线�?
type: how-to
domain: frontend
phase: development
tier: standard
...
summary: "版本：v1.0.0 日期�?026-07-12 目标：在 20 分钟内完成新�?Widget 的全流程，不�?ErrorBoundary..."
tags: [frontend, widget, guide]
version: v1.0.0
last_updated: 2026-07-17
code_version: 2.0.0
doc_id: V9-DOC-FRONT-026
referenced_by: [V9-DOC-META-000, V9-DOC-PROJ-122, V9-DOC-PROJ-176, V9-DOC-FRONT-034, V9-DOC-PROJ-157, V9-DOC-PROJ-149]
change_log:
  - version: v1.0.0
...
status: active
maintainer: V9 Architecture Team
summary: "如何新增一�?Widget（WidgetShell + 事件总线�?type: how-to document"
tags: [frontend, widget, guide]
phase: development
version: v1.0.0
last_updated: 2026-07-17
...
# 如何新增一�?Widget（WidgetShell + 事件总线�?
> **版本**：v1.0.0  
> **日期**�?026-07-12  
> **目标**：在 20 分钟内完成新�?Widget 的全流程，不�?ErrorBoundary 遗漏、事件命名冲突、状态提升等常见�?
---

## 前置检�?
- [ ] 已阅�?`../tutorials/getting-started.md`（了解四步集成契约）
- [ ] 已阅�?`../reference/widget-integration-checklist.md`（Widget 集成检查清单）
- [ ] 已阅�?`../reference/atomic-component-system.md`（原子组件系统）
- [ ] 已确�?Widget 的数据来源（Store �?Service�?- [ ] 已确�?Widget 的宿主位置（驾驶�?`/cockpit` 或页面内嵌）

---

## 步骤 1：定�?Widget 类型与配置（3 分钟�?
�?`src/types/widget.ts`（或新建 `src/types/widget.ts`）中定义 Widget 类型�?
```typescript
// 示例：新�?SectorHeatmapWidget 的类�?export interface SectorHeatmapWidgetConfig extends WidgetConfig {
  settings?: {
    title?: string
    showTrend?: boolean
...
> **基类**：所�?Widget 必须继承 `WidgetConfig`（位�?`src/types/widget.ts`）：
> ```typescript
...
├── WidgetShell.tsx          # 通用外壳（已存在�?├── SectorHeatmapWidget/
�?  ├── index.tsx            # 主入口（导出 Widget 组件�?�?  ├── SectorHeatmapView.tsx # 纯视图组件（无状态）
�?  ├── useSectorHeatmap.ts   # 自定�?Hook（数据逻辑�?�?  └── types.ts             # 局部类型（可选）
```
 ...
 * @description 板块热力�?Widget。展示板块评分热力分布，支持趋势切换�? *
 * @see @/components/widgets/WidgetShell.tsx - 外壳包装�? * @see @/store/sectorAnalysisStore.ts - 数据来源
 */
...
function SectorHeatmapView({ sectors, loading, error, onRetry }: SectorHeatmapViewProps) {
  if (loading) {
    return <div className={THEME_TOKENS.color.muted}>加载�?..</div>
  }
  if (error) {
    return (
...
export function SectorHeatmapWidget({ widgetId, config }: SectorHeatmapWidgetProps) {
  ...
  // 视觉状态映�?  const visualState = useMemo(() => {
...
| 纯视图组�?| �?必须 | 数据与视图分离，方便测试 | 逻辑与视图混合，难以测试 |
| 自定�?Hook | 🟡 建议 | 数据逻辑抽离�?`useXxxData` | 在视图组件中直接调用 Store |
| `visualState` | �?必须 | `ready`/`loading`/`empty`/`error` | 未映射到 WidgetShell �?state 属�?|
| `useEffect` 清理 | �?必须 | 订阅/定时器在 cleanup 中移�?| 内存泄漏 |
| 颜色令牌 | �?必须 | 使用 `THEME_TOKENS` / `COLOR_TOKENS` | 硬编�?HEX �?Tailwind �?|
...
在驾驶舱或页面的 Widget 注册文件中添加（�?`src/cockpit/core/widgetRegistry.ts`）：
...
export const DEFAULT_COCKPIT_LAYOUT = [
  // ... 现有 Widget ...
  {
    ...
    position: { x: 0, y: 6 },
    size: { cols: 4, rows: 3 },
    settings: { title: '板块热力�?, showTrend: true },
  },
]
```
...
describe('SectorHeatmapView', () => {
  it('renders sectors', () => {
    const sectors = [
      { sectorId: '1', sectorName: '半导�?, score: 4.5, trend: 'up' as const },
      { sectorId: '2', sectorName: '新能�?, score: 3.2, trend: 'flat' as const },
    ]
    render(<SectorHeatmapView sectors={sectors} loading={false} error={null} onRetry={() => {}} />)
    expect(screen.getByText('半导�?)).toBeInTheDocument()
...
2. 导航到驾驶舱 `/cockpit`
3. 添加 Widget：搜索「板块热力图�?4. 确认：数据加载、视觉状态切换、错误重试、设置按钮点�?
---

## 常见踩坑与规�?
### �?1：Widget 直接调用 dataLayer（跨层违规）

```tsx
// �?错误：Widget 直接访问 dataLayer（Widget 只能依赖 Store �?Service�?import { dataLayer } from '@/data/dataLayer'
...
### �?2：遗�?WidgetShell �?ErrorBoundary

```tsx
// �?错误：无 WidgetShell 包装，渲染错误导致整个页面崩�?export function MyWidget() {
  return <div>{data.map(...)}</div> // 如果 data 未定义，整个页面白屏
}

// �?正确：WidgetShell 提供 ErrorBoundary，局部错误隔�?<WidgetShell widgetId={id} config={config}>
  <MyWidgetContent />
</WidgetShell>
```
...
```tsx
// �?错误：使用硬编码事件名，与其�?Widget 冲突
widgetEventBus.publish('update', data)

// �?正确：使�?widget:{widgetId}:{event} 命名空间
widgetEventBus.publish(`widget:${widgetId}:update`, data)
// 或依�?WidgetShell 自动添加前缀
```

### �?4：useEffect 未清�?Store 订阅

```tsx
// �?错误：Store 订阅未清理，导致内存泄漏和重复回�?useEffect(() => {
  useSectorAnalysisStore.subscribe((state) => {
    setLocalData(state.sectors)
  })
}, [])

// �?正确：subscribe 返回 unsubscribe，在 cleanup 中调�?useEffect(() => {
...
### �?5：Widget 状态未提升�?Store

```tsx
// �?错误：Widget 内部使用 useState 管理数据，导致多�?Widget 实例数据不一�?const [data, setData] = useState([])

// �?正确：数据统一�?Store 中管理，Widget 只负责展�?const { data } = useMyStore()
```

> **原则**：Widget 是「展示层」，状态应提升�?Store。只�?UI 局部状态（如折�?展开）可保留�?Widget 内部�?
---
...
// 使用 react-window �?@tanstack/react-virtual 实现虚拟滚动
```
...
| 文档 | 路径 | 说明 |
|------|------|------|
| 新增 Store | `./how-to-add-store.md` | Widget 的数据来�?|
| 新增 Service | `./how-to-add-service.md` | Store 的下游依�?|
| Widget 集成检查清�?| `../reference/widget-integration-checklist.md` | 逐项核对 |
| Widget 错误处理 | `../reference/widget-error-handling.md` | 错误边界与降�?|
| 原子组件系统 | `../reference/atomic-component-system.md` | 组件分层 |
| WidgetShell 源码 | `src/components/widgets/WidgetShell.tsx` | 外壳实现 |

---

> **验证完成�?*：更�?`docs/README.md` �?C 类索引，将新�?Widget 链接回主索引�
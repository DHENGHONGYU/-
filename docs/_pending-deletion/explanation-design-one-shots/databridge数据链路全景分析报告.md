...
DataBridge 作为 V9 智能投研复盘系统?*统一数据桥接?*，承担四大核心职责：

| 职责 | 功能描述 | 关键文件 |
|------|---------|---------|
| **写操作入?* | `forward()` - ACL 校验 + 路由?DB/Manager/Strategy | [databridge.ts](../../../src/core/databridge.ts) |
| **读操作入?* | `query()` - 缓存 + ACL 校验 + 审计日志 | [databridge.ts](../../../src/core/databridge.ts) |
...
```mermaid
graph TB
    subgraph DataBridge 核心?        DB[DataBridge] -->|策略模式| DH[databridgeHandlers]
    ...
    subgraph Handler 策略?        DH --> Put[PutHandler]
...
```mermaid
graph LR
    Forward[DataBridge.forward] -->|策略?action| Strategy[routeToStrategy]
    Forward -->|管理?action| Manager[routeToManager]
    Forward -->|数据?action| DB[routeToDB]
...
sequenceDiagram
    ...
    participant Subscribers as DataBridge订阅?    
    ...
    alt 策略?action
        ...
        DataBridge->>Subscribers: 通知所有订阅?    end
```
...
```typescript
// stocks store ?keyPath ?'symbol'，缺失会导致 IndexedDB 报错
...
graph TD
    ...
    Step2a -->|按主?symbol| PK[db.delete(store, symbol)]
    Step2b -->|按索?by-symbol| Index[db.getAllByIndex + db.delete]
...
sequenceDiagram
    ...
    Engine->>Engine: 执行L-1~L8?1层计?    Engine-->>Service: CompositeScore
...
```typescript
// 数据完整度检?const totalLayers = Object.keys(composite.layers).length
...
// 低于100%时添加质量警?if (dataCompleteness < 100) {
  v6Score.qualityWarning = `数据完整?${dataCompleteness.toFixed(0)}%`
}
```
...
```mermaid
graph TB
    subgraph 策略路由?        Route[routeToStrategy] --> HS[handleHotSectorRefresh]
    ...
    subgraph 价值洼地策?        VP --> Catalyst[催化确定? 30%]
    ...
    subgraph 轮动信号检?        RS --> Volume[成交量突破]
        RS --> Capital[资金流入]
        RS --> GoldenCross[金叉信号]
...
| 维度 | 权重 | 输入指标 | 评分范围 |
|------|------|---------|---------|
| 动量强度 | 35% | 板块强度分、涨跌幅排名、量比、资金流入天数、RS?| 0-5 |
| 情绪热度 | 25% | 舆情排名、散户情绪、机构买入家数、涨停板数量 | 0-5 |
| 技术突?| 20% | 突破形态、RSI信号、均线排?| 0-5 |
| 估值风?| 15% | PE、PB分位、市值、股息率 | 0-5 |
| 大盘环境 | 5% | 大盘趋势、系统性风险等?| 0-5 |

**综合评分公式**?```
overallScore = momentum * 0.35 + sentiment * 0.25 + breakout * 0.20 + valuation * 0.15 + marketEnv * 0.05
```

**行动建议阈?*?- `immediate`：评??4.0（立即行动）
- `probe`：评??2.5（试探建仓）
- `ignore`：评?< 2.5（忽略）
...
| 维度 | 权重 | 输入指标 | 评分逻辑 |
|------|------|---------|---------|
| 催化确定?| 30% | 政策催化、周期拐点、技术突破、订单爆?| 取四类信号最大值，双确认加?|
| 估值安全垫 | 25% | PE/PB分位、股息率、PEG?| PE/PB分位映射，股息率/PEG调整 |
| 筹码结构 | 20% | 北向资金变化、基金持仓变化、股东户数变?| 资金增持加分，筹码集中加?|
| 轮动位置 | 15% | 板块成交量分位、资金流入强度、金叉信?| 底部区域高分，金叉加?|
| 流动?| 10% | 日均成交额、换手率、市值规?| 成交额映射，换手?市值调?|
...
sequenceDiagram
    ...
    loop 逐板块评?        Router->>Analyzer: analyzeHotSector(input)
    ...
    Router->>Router: 汇总统?avg/max/min/counts)
...
```mermaid
graph TB
    subgraph 信号生成?        Scan[scanWatchingSignals] --> Generate[generateSignalsForSymbol]
        Generate --> Signal[TradingSignal]
    end
    
    subgraph 决策分析?        Signal --> Pick[pickStrongestSignal]
        Pick -->|buy/sell| Sizing[calculatePosition]
        Pick -->|watch/hold| Hold[中性建议]
        Sizing --> Position[PositionSizingResult]
    end
    
    subgraph 风控?        Position --> Risk[checkOrderRisk]
        Risk -->|通过| Order[创建订单]
        Risk -->|拒绝| Block[风控阻断]
    end
    
    subgraph 执行?        Order --> Envelope[EnvelopeFactory.create]
...
sequenceDiagram
    ...
    alt 非交易信?watch/hold)
        Service-->>UI: 返回中性建?    else 交易信号(buy/sell)
...
**买入信号**?| 信号类型 | 触发条件 | 置信?|
|---------|---------|-------|
| `buy_dip` | 价格低于MA20 8% ?RSI < 30 | 0.55 |
| `buy_pivot` | 突破MA20 + 放量 + MACD红柱 | 0.65 |
| `buy_safety_margin` | PE/PB低于配置阈?| 0.50 |

**卖出信号**?| 信号类型 | 触发条件 | 置信?|
|---------|---------|-------|
| `sell_profit_taking` | 价格高于MA20 15% ?RSI > 70 | 0.55 |
| `sell_trailing_stop` | 从近期高点回?0% | 0.70 |

**综合共振**：同方向多个独立信号时生?`composite_buy`/`composite_sell`，置信度叠加

### 5.4 仓位计算详解

[positionSizer.ts](../../../src/services/trading/positionSizer.ts) 使用 **Kelly 公式半凯?* 计算仓位?
```
Kelly % = W - (1 - W) / R
其中：W = 胜率, R = 盈亏?```

**约束规则**?1. 单笔仓位上限（默?10%?2. 总仓位上限（默认 80%?3. 最小仓位阈值（低于则归零）
4. 按整手取?
### 5.5 风控引擎详解
...
| 同标的冷却期 | 同标的交易在冷却期内 | - |
| 当日交易次数 | 达到每日上限 | 接近上限 |
| 单笔仓位上限 | 买入后超出单笔上?| 接近上限 |
| 总仓位上?| 买入后超出总仓位上?| 接近上限 |
| 卖出持仓检?| 卖出数量超过当前持仓 | - |

### 5.6 筛选引擎详?
[screeningEngine.ts](../../../src/services/analysis/screeningEngine.ts) 实现两级晋升?
**candidate ?screened**?- 数据质量满足 basic/kline 要求
- V6 评分 ?阈?
**screened ?deepDive**?- 数据质量满足 basic/kline/finance 要求
- V6 评分 ?阈??智能评分 ?阈?
### 5.7 订单创建流程

[tradingService.ts](../../../src/services/trading/tradingService.ts) 实现完整的风?下单流程?
```typescript
async function createOrderWithRiskCheck(input: CreateOrderInput) {
  // 1. 参数校验
  if (price <= 0 || input.quantity <= 0) return { success: false }
  
  // 2. 风控检?  const risk = await checkOrderRisk({ symbol, direction, quantity, price, portfolioValue })
  if (!risk.ok) return { success: false, error: `风控未通过` }
  
  // 3. 构建订单
  const order = buildOrder(input)
  
  // 4. 通过 DataBridge 写入（ACL校验 + 审计日志?  const envelope = EnvelopeFactory.create({
...
分布式循环机制已?[pipelineScheduler.ts](../../../src/core/pipelineScheduler.ts) 中实现，包含?
- **PipelineScheduler**：循环调度器，管理多个独立循环的生命周期
- **PipelineCycle**：单个循环单元，包含状态机、指数退避重试、健康检?- **DataIntegrityGuard**：数据完整性保障，包含自动修复逻辑
...
stateDiagram-v2
    ...
    RETRY --> RUNNING: 重试成功
    RETRY --> FAILED: 超过最大重试次?    FAILED --> IDLE: reset()
    SUCCESS --> RUNNING: 等待下一个周?    RUNNING --> PAUSED: pause()
...
| **批处?* | 批量Envelope合并写入 | 降低DB事务开销 |
| **优先级调?* | 高频低延迟任务优?| 保障实时性要?|
...
graph LR
    ...
    subgraph 服务?        Scoring[Scoring Services]
    ...
    subgraph 核心?        DB[DataBridge]
        ACL[ACL Engine]
        Envelope[Envelope]
        EventBus[EventBus]
    end
    
    subgraph 数据?        DataLayer[dataLayer]
        DBIndexed[IndexedDB]
    end
...
```mermaid
sequenceDiagram
    participant Client as 调用?Store/Service)
    participant Envelope as EnvelopeFactory
    participant DB as DataBridge
    participant ACL as ACLEngine
    participant Handler as HandlerRegistry
    participant IndexedDB as IndexedDB
    participant Subscribers as 频道订阅?    participant EventBus as EventBus
    ...
    alt ACL拒绝
        ...
        alt 策略类action
            ...
            DB->>Subscribers: 通知所有订阅?            DB->>EventBus: emit(store+_CHANGED)
        end
...
sequenceDiagram
    ...
    else 缓存未命?        DB->>ACL: assert(module, store, SELECT)
        ACL-->>DB: OK
...
```mermaid
graph TB
    subgraph 数据采集?        F1[Fetcher-1] --> MQ[消息队列]
        F2[Fetcher-2] --> MQ
        F3[Fetcher-N] --> MQ
    end
    
    subgraph 数据处理?        MQ --> P1[Processor-1]
        MQ --> P2[Processor-2]
        MQ --> P3[Processor-N]
    end
    
    subgraph 存储?        P1 --> DB1[IndexedDB-1]
        P2 --> DB2[IndexedDB-2]
        P3 --> DB3[IndexedDB-N]
    end
    
    subgraph 查询?        DB1 --> Q[QueryService]
        DB2 --> Q
        DB3 --> Q
        Q --> API[统一查询API]
    end
...
| 优势 | 说明 |
|------|------|
| **统一入口** | DataBridge 作为唯一数据通道，确?ACL 校验与审计日志覆?|
| **策略模式** | HandlerRegistry 支持灵活扩展新的 action 处理?|
| **订阅机制** | 跨模块数据同步通过频道订阅实现，解耦模块依?|
| **降级队列** | 市场数据 ACL 拒绝时自动重试，保证数据完整?|
| **缓存?* | MemoryCache 减少重复查询，提升读取性能 |

### 11.2 改进建议

| 优先?| 建议 | 说明 |
|--------|------|------|
| P0 | 批量操作支持 | 当前所?forward 均为单条写入，增?BULK_* 系列 Action |
| P0 | 细粒度订?| 当前?Store 全量广播，增?`stocks:{symbol}` 级别订阅 |
| P1 | 事件型频道独?| 事件型动作混?DB 路由中，分离事件路由建立 `event:*` 频道体系 |
| P1 | 查询?forward | 评估是否需?QUERY_* 系列 Action 统一?DataBridge |
| P2 | 分布式调度器 | ?已实现：[pipelineScheduler.ts](../../../src/core/pipelineScheduler.ts) |

### 11.3 数据完整性保障措?
1. **写入前校?*：EnvelopeFactory.validate() 确保数据格式正确
2. **写入后广?*：所有写操作完成后广播变更通知，订阅者自动刷?3. **缓存失效**：写操作后调?invalidateCache()，保证读一致?4. **审计日志**：所有操作自动写?researchLogs，支持追?5. **级联删除**：DeleteStockHandler 确保关联数据完整清理

---

> **文档结束**
> 
> 本文档基?`src/core/databridge.ts`、`src/core/databridgeHandlers.ts`、`src/core/databridgeStrategyRouter.ts`、`src/services/scoring/`、`src/services/trading/`、`src/store/` 等核心模块综合梳理生成
...
受影响文档均已同步：`V9数据宪法.md`、`V9_IndexedDB_Store_Schema.md`、`03-architecture-standards.md`、`06-routing-specs.md`、`10-glossary.md`。
...
> **总体结论**：七维采集配置模块的代码实现（Store / Page / Config / Tests）内部一致性良好，但与项目级真相源（`ROUTE_REGISTRY`、`docs/06-routing-specs.md`、`docs/DATA_DICTIONARY_INDEX.md`、`CHANGELOG.md`）存在多处脱节。该问题已于 2026-07-01 修复，`/input/seven-dim` 已注册到 `ROUTE_REGISTRY`（原 `data-collection-route-ui-audit.md` P0 项 F-01 已闭环）。
...
| `docs/06-routing-specs.md` 第2.1节表格（第31-61行） | 32 |
| `docs/06-routing-specs.md` 第65行声明 | 32 |
| `docs/06-routing-specs.md` 第288行（第9节）声明 | 32 |

**不一致点**：
- 该问题已于 2026-07-01 修复：`docs/06-routing-specs.md` 第9节路由数已统一为 32 条，与 `routes.ts` 实际数量（32）一致，原 29 vs 31 文档内部矛盾已消除。

**修复建议**：已闭环（2026-07-01）。

#### 1b) InputHubPage 链接路径是否都有对应路由注册 — ✅ 已修复（2026-07-01）

`src/pages/input/InputHubPage.tsx` 中 `INPUT_MODULES`（第34-71行）链接路径检查：
...
| `src/pages/input/InputHubPage.tsx` (卡片链接) | ✅ 存在 | 68 |
| `docs/06-routing-specs.md` (第2.1节路由表) | ✅ 已补入 | — |
| `docs/06-routing-specs.md` (第3.2节输入舱子路由映射) | ✅ 已补入 | — |
| `docs/06-routing-specs.md` (第8节路由→组件映射) | ✅ 已补入 | — |
| `docs/implementation/data-collection-route-ui-audit.md` | ✅ 已记录并标记为已修复 | 16, 30, 142, 155, 226 |
...
**不一致点**：
- 该问题已于 2026-07-01 修复：`docs/06-routing-specs.md` 第8节已删除 `/analysis/news-v6` 残留行，与代码保持一致。
...
**不一致点**：
- 该问题已于 2026-07-01 修复：`/input/seven-dim` 已在 `docs/06-routing-specs.md` 第2.1节、第3.2节、第8节中补入。
...
**不一致点**：
- `docs/DATA_DICTIONARY_INDEX.md` 的"按模块索引"表格（第13-20行）未包含七维采集配置模块条目。
- "通用类型与常量"表格（第26-43行）未列出 `src/config/collectConfig.ts`。

**修复建议**：
1. 在 `docs/DATA_DICTIONARY_INDEX.md` "按模块索引"表新增一行：七维采集配置模块 → 新建 `docs/data-collection/COLLECT_CONFIG_DATA_DEFINITION.md`。
...
`SevenDimConfigPage.tsx` 使用的UI组件 vs `docs/implementation/component-library-guide.md` 记录：
...
**不一致点**：
- `docs/implementation/component-library-guide.md` 组件清单缺少 `Badge`、`Label`、`Progress`、`Breadcrumb` 四个组件的文档条目，而这些组件在 `src/components/ui/` 下已实现且被 SevenDimConfigPage 使用。
...
| `CHANGELOG.md:8` | `Unreleased` | 2026-07-01 | ⚠️ 见下 |
| `CHANGELOG.md:409` | 最新发布 `0.9.6` | 2026-06-29 | ❌ 落后 |
| `docs/06-routing-specs.md:4` | `v1.5.0` | 2026-07-01 | ⚠️ 见下 |
| `docs/06-routing-specs.md:281` | 第9节自称 `v1.2.0` | — | ❌ 内部矛盾 |
| `docs/DATA_DICTIONARY_INDEX.md:4` | `v2.2.0` | 2026-06-30 | 文档独立版本号 |
...
2. 统一 `docs/06-routing-specs.md` 版本号：将第9节的 `v1.2.0` 更新为 `v1.5.0`，并更新版本比对说明。
3. 待 `/input/seven-dim` 路由注册后，更新第9节路由数为 32。

#### 5b) 日期是否统一为 2026-07-01 — ⚠️ 部分一致

| 文件 | 日期 | 状态 |
|------|------|------|
| `docs/06-routing-specs.md:5` | 2026-07-01 | ✅ |
| `CHANGELOG.md` Unreleased | 2026-07-01 | ✅ |
| `docs/DATA_DICTIONARY_INDEX.md:6` | 2026-06-30 | ⚠️ 落后1天 |
| `docs/DATA_DICTIONARY_INDEX.md:7` (Scan Time) | 2026-06-30 23:07:32 | ⚠️ 落后 |

**部分一致点**：
- `DATA_DICTIONARY_INDEX.md` 日期为 2026-06-30，未同步至 2026-07-01，且该文档未纳入七维采集配置模块的字典条目（见 2a/2b）。

**修复建议**：在补入七维采集配置模块数据字典条目后，将 `DATA_DICTIONARY_INDEX.md` 的 `Last Updated` 更新为 2026-07-01。
...
| D-02 | 🔴 高 | `/input/seven-dim` 未在路由规格文档列出 | `docs/06-routing-specs.md` | 2.1/3.2/8节 | ✅ 已修复（2026-07-01） |
| D-03 | 🔴 高 | CHANGELOG 未记录七维采集模块新增 | `CHANGELOG.md` | Unreleased段 | 补入 Added 条目 |
| D-04 | 🔴 高 | 数据字典未登记 collectConfig / sevenDimConfigStore | `docs/DATA_DICTIONARY_INDEX.md` | 模块索引表 | 新增模块字典条目 |
| D-05 | 🟡 中 | types.ts 的 DataDimensionType 缺 `08_research` | `src/data/types.ts` | 694-702 | ✅ 已修复（2026-07-01） |
| D-06 | 🟡 中 | 06-routing-specs.md 第9节路由数 29 vs 31 矛盾 | `docs/06-routing-specs.md` | 288 | ✅ 已修复（2026-07-01） |
| D-07 | 🟡 中 | 06-routing-specs.md 版本号 v1.5.0 vs v1.2.0 矛盾 | `docs/06-routing-specs.md` | 4 vs 281 | ✅ 已修复（2026-07-01） |
| D-08 | 🟡 中 | 第8节残留 `/analysis/news-v6` 已删路由 | `docs/06-routing-specs.md` | 263 | ✅ 已修复（2026-07-01） |
| D-09 | 🟡 中 | 组件库指南缺 Badge/Label/Progress/Breadcrumb | `docs/implementation/component-library-guide.md` | 组件清单 | 补入组件文档 |
...
| D-12 | 🟢 低 | DATA_DICTIONARY_INDEX 日期 2026-06-30 | `docs/DATA_DICTIONARY_INDEX.md` | 6 | 更新为 2026-07-01 |
...
2. **D-02**：✅ 已完成（2026-07-01）— 在 `docs/06-routing-specs.md` 补入 `/input/seven-dim` 路由条目。
...
| `docs/06-routing-specs.md` | 路由规格文档 |
| `src/pages/input/InputHubPage.tsx` | Hub页面模块卡片 |
| `src/config/collectConfig.ts` | 七维采集配置常量 |
| `src/store/sevenDimConfigStore.ts` | 七维采集配置 Store |
| `src/data/types.ts` | 全局类型定义 |
| `docs/DATA_DICTIONARY_INDEX.md` | 数据字典索引 |

...
| B | P1 | 将蓝图校验纳入 CI / 质量门禁 | ✅ 已完成（B-1 质量门禁已注册；B-2 待 CI 条件触发） | V9 Team | `docs/09-quality-gates.md` 已新增「数据蓝图一致性」门禁；CI 工作流待 `.github/workflows/ci.yml` 启用 |
...
2. 在 `src/store/newsStore.ts` 中将 `NewsBookmarkRecord` 替换为 `import type { NewsBookmark }`
3. 更新 `src/blueprints/__tests__/dataRelationship.test.ts` 将 `NewsBookmark` 加入 entityStoreMap
...
**问题：** 早期 `docs/V9_数据血缘追踪.md` 标注 `DB_VERSION = 14`，而 `src/config/dbConfig.ts` 实际为 `14`（v15/v16 升级前）。
...
**执行步骤：**
1. 读取 `docs/V9_数据血缘追踪.md` 中 DB_VERSION 相关行
...
### B-1: 在 `docs/09-quality-gates.md` 中注册蓝图校验

**执行步骤：**
1. 在 `docs/09-quality-gates.md` 的「质量门禁清单」中新增一行：
   - 校验项：数据蓝图一致性
...
**执行步骤：**
1. 在 `docs/blueprints/v9-data-timeline.md` 第 3.2 节追加「运行时校验实现状态」列
2. 提交
...
- [x] freshness-verify 运行相关测试与校验，更新 `v9-data-timeline.md` 与 `CHANGELOG.md`
...
- [ ] 检查本周新增/修改的 IndexedDB Store：
  ...
  - 确认每个 Store 在 `docs/blueprints/v9-data-relationship-er.md` 中有定义
  - 确认每个 Store 在 `src/blueprints/__tests__/dataRelationship.test.ts` 的 `expectedStores` 列表中
...
- [ ] 检查时间戳字段变更：
  ...
  - 确认新增/重命名的时间戳字段已在 `v9-data-timeline.md` 3.2 节登记
...
- [ ] 检查 `docs/blueprints/v9-data-timeline.md` 3.2 节运行时校验状态表是否最新

### 8.5 质量门禁状态

- [ ] 确认 `docs/09-quality-gates.md` 中「数据蓝图一致性」门禁命令仍可运行

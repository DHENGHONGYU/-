...
- **风险评估**:
  ...
  - 替代方案：保留 `src/core/databridge.ts` 作为 core 层的子目录，在 AGENTS.md 中补充定义 `src/core/databridge.ts ← DataBridge 适配层（core 层扩展）`
...
- **迁移建议**:
  ...
  2. 或重命名 `src/lib/utils.ts` 为 `src/lib/common-utils.ts`，然后将 `src/lib/` 整体迁入 `src/lib/`

...
summary: "## 一、工作概述 本次文档化工作针对 `audit:docs` 检测到的 15 个未文档化文件进行全面处理，涵盖代码注释补充、文档引用添加、审计脚本修复三个维度。"
...
| 2 | `src/components/templates/PageContainer.tsx` | 模块级注释、设计原则、合规说明 | P1 |
| 3 | `src/components/templates/PageHeader.tsx` | 模块级注释、设计原则、推荐用法 | P1 |
...
| # | 文件路径 | 修改内容 |
|---|---------|---------|
| 6 | `../reference/data-dictionary-index_reference.md` | 添加 10 个模块索引条目：Store 派生计算、事件订阅、全局错误处理、UI 基础组件、派生缓存工具、本地存储加密、错误总线、韧性工具、确认对话框 Hook、板块常量、风控派生计算、V6 评分引擎 L3 辅助函数 |
| 7 | `../reference/03-architecture-standards_reference.md` | 新增 §3.1.10 Store 派生计算与事件订阅（含设计原则、缓存策略、导出模式、文件清单）、§3.5.1 V6 评分引擎 L3 层辅助函数（含评分规则说明） |
| 8 | `docs/RISK_DERIVED_data-definition.md` | 新增完整文档：核心类型定义、风控三态判定规则、熔断状态机、风险趋势分析、22 个派生查询函数详解、React Hook 形式派生、缓存策略、业务规则汇总、UI 层交互场景 |

### 2.3 工具层修改（审计脚本修复）

| # | 文件路径 | 修改内容 |
|---|---------|---------|
| 9 | `scripts/audit/audit-doc-sync.ts` | 修复逻辑缺陷：将完整路径检查移到噪音词检查之前，避免文件名是噪音词（如 `helpers`）的文件被误判为未文档化 |
...
|------|------|
| 文档文件总数 | 276 |
| 本次新增文档 | 1（RISK_DERIVED_data-definition.md） |
| 本次更新文档 | 3（data-dictionary-index_reference.md、03-architecture-standards_reference.md、CHANGELOG_reference.md） |
| 代码注释补充文件 | 5 |
...
### 5.1 风控派生计算文档（RISK_DERIVED_data-definition.md）
...
### 5.2 Store 派生计算架构说明（03-architecture-standards_reference.md §3.1.10）

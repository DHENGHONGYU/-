...
type: reference
domain: project
doc_id: V9-DOC-AUTO-415F33
code_version: 2.0.0
summary: 对 V9 项目 629 个文件进行全面扫描，识别 31 个未注册/未引用文件，建立三层集中注册体系（Store/Service/Component），完成 7 个 Cockpit Widget 注册和 12 个组件的页面集成。
...
| 文件 | 用途 | 行数 |
|------|------|------|
| `src/store/storeRegistry.ts` | Store 集中注册表（29 条目） | ~200 |
| `src/services/serviceRegistry.ts` | Service 集中注册表（52 条目） | ~350 |
...
| `src/services/useCase/submitOrder.useCase.ts` | +feedbackService 操作反馈 | 交易执行 |
...
| `docs/REGISTRY_INDEX_root.md` | 新建注册体系核心文档 | 文档新增 |
| `docs/03-architecture-standards.md` | v2.2.1→v2.3.0：§3.1.8 注册体系、Widget +7、D19 修复 | 架构文档 |
| `docs/widget-development-guide_root.md` | v1.0.0→v1.1.0：§7 Widget 清单（19 个）、§7.3 错误隔离 | Widget 指南 |
| `docs/testing-strategy_root.md` | v1.0.0→v1.1.0：§9 注册体系测试策略、基线更新 | 测试策略 |
| `docs/DATA_DICTIONARY_INDEX.md` | v1.2.0→v1.3.0：Registry 模块索引 | 数据字典 |
| `docs/10-glossary.md` | v2.2.1→v2.3.0：§10.11 注册体系术语（8 条） | 术语表 |

...
summary: "Goal: 确保 `docs/reference/blueprints/` 中的数据关系与时间关系蓝图与源码持续一致，并将其校验纳入日常开发流程。"
tags: [data, spec, report, checklist, data-definition, store]
version: v1.3.0
last_updated: 2026-06-30
code_version: 2.0.0
change_log: 
doc_id: V9-DOC-AUTO-79DA4F
---

# V9 数据关系蓝图任务跟踪计划

> **Goal:** 确保 `docs/reference/blueprints/` 中的数据关系与时间关系蓝图与源码持续一致，并将其校验纳入日常开发流程。
...
| B | P1 | 将蓝图校验纳入 CI / 质量门禁 | ? 已完成（B-1 质量门禁已注册；B-2 待 CI 条件触发） | V9 Team | `../../reference/09-quality-gates_reference.md` 已新增「数据蓝图一致性」门禁；CI 工作流待 `.github/workflows/ci.yml` 启用 |
...
### B-1: 在 `../../reference/09-quality-gates_reference.md` 中注册蓝图校验

**执行步骤：**
1. 在 `../../reference/09-quality-gates_reference.md` 的「质量门禁清单」中新增一行：
   - 校验项：数据蓝图一致性
...
**执行步骤：**
1. 在 `../../reference/v9-data-timeline_reference.md` 第 3.2 节追加「运行时校验实现状态」列
2. 提交
...
1. 如使用 TRAE Schedule，可创建每周一早 9 点的任务：
   - 消息：运行 `npm run validate:blueprint` 与蓝图测试，检查 Store/接口/文档一致性，输出结果到 `docs/reference/blueprints/weekly-check-YYYY-MM-DD.md`
...
**执行步骤：**
1. `../../blueprints/v9-data-relationship-er.md` 已在 §1 Store 清单追加 4 行；§2 实体关系补 4 条；
   §3 ER 图新增 4 条边；DB_VERSION 14→16
2. `../../reference/v9-data-timeline_reference.md` 已在 §1 追加 P12/P13/P14/P15 四个管线阶段；§3.1 刷新频率表补 4 行
3. `scripts/other/validate-data-blueprint.ts` 的 Store 数量预期从 20→41（当前 expectedStores=41）
...
- [x] freshness-verify 运行相关测试与校验，更新 `../../reference/v9-data-timeline_reference.md` 与 `../../../CHANGELOG.md`
...
> **输出**：将结果记录到 `docs/reference/blueprints/weekly-check-YYYY-MM-DD.md`（可直接复制本检查单作为模板）。
...
- [ ] 检查本周新增/修改的 IndexedDB Store：
  ...
  - 确认每个 Store 在 `../../blueprints/v9-data-relationship-er.md` 中有定义
...
- [ ] 检查时间戳字段变更：
  ...
  - 确认新增/重命名的时间戳字段已在 `../../reference/v9-data-timeline_reference.md` 3.2 节登记
...
- [ ] 检查 `../../reference/v9-data-timeline_reference.md` 3.2 节运行时校验状态表是否最新

### 8.5 质量门禁状态

- [ ] 确认 `../../reference/09-quality-gates_reference.md` 中「数据蓝图一致性」门禁命令仍可运行

...
本方案建立一套可持续的 **“差异扫描 → 文档补齐 → 质量验证”** 闭环，确保新增模块的架构描述和数据定义完整、准确、可追溯。
...
| 文档类型 | 代表文件 | 同步时机 |
|----------|----------|----------|
| 数据字典 | `DATA_DEFINITION.md`、`docs/NEWS_DATA_DEFINITION.md`、`docs/AI_CENTER_DATA_DEFINITION.md` | 新增/修改数据模型、Store、Envelope Action、Service API 时 |
| 架构说明 | `ARCHITECTURE.md`、`docs/03-architecture-standards.md` | 新增模块、调整分层、新增路由/注册表时 |
| 实施计划 | `docs/08-implementation-plan.md` | 任务状态变更、新增/拆分任务时 |
| 路由规格 | `docs/06-routing-specs.md` | 新增/修改路由、页面组件时 |
| 引擎规格 | `docs/05-engine-specs.md` | 新增/修改引擎、算法、数据流时 |
| 质量门禁 | `docs/09-quality-gates.md` | 测试基线、审计基线变化时 |
| 更新日志 | `CHANGELOG.md` | 每次代码-文档同步完成后 |
| 验收确认书 | `NewsPage-迁移验收确认书.md` 等 | PoC/迁移任务完成后 |
...
**输入**：
- `docs/implementation/v9-issue-resolution-schedule.md` 中待执行的整改项；
...
| 路由 | 新增路由是否在 `docs/06-routing-specs.md` 中注册 | 比对 `src/config/routes.ts` 与文档 |

**输出**：`doc-sync-gap-list.md`（差异清单），每条包含：
- 模块名
- 涉及文件
- 缺失文档类型
- 优先级（P0/P1/P2）
...
| # | 任务 | 优先级 | 输入 | 输出 | 验收标准 |
|---|------|--------|------|------|----------|
| 1 | 建立文档同步工作流 | P0 | 本方案 | `docs/implementation/doc-sync-execution-plan.md` | 方案通过评审，团队知悉 |
| 2 | 补齐 NewsPage 数据字典 | P0 | PoC 报告、代码 | `docs/NEWS_DATA_DEFINITION.md` | 已输出并通过验证 |
| 3 | 补齐金融业务 Widget 架构与字典 | P0 | Widget 代码 | `ARCHITECTURE.md`、`DATA_DEFINITION.md` | 已输出并通过验证 |
| 4 | 补齐 AI 中心板块字典与 Vue3 示例 | P0 | AI Center 代码 | `docs/AI_CENTER_DATA_DEFINITION.md`、`docs/AI_CENTER_VUE3_EXAMPLES.md` | 已输出并通过验证 |
| 5 | 更新 `CHANGELOG.md` | P0 | 上述文档 | `CHANGELOG.md` `[Unreleased]` 新增条目 | 条目完整、引用路径准确 |
| 6 | 建立差异清单模板 | P1 | 本方案 | `docs/implementation/doc-sync-gap-list.md` | 模板可复用 |
...
| 10 | 更新 `docs/08-implementation-plan.md` | P1 | 任务 1, 4 | 在实施计划中新增“文档同步”任务列，状态可追踪 |
| 11 | 数据字典索引页 | P1 | 任务 2, 3, 4 | `docs/DATA_DICTIONARY_INDEX.md`：汇总所有模块数据字典入口 |
...
- **Phase 1 扫描**：将 `v9-issue-resolution-schedule.md` 中所有“待执行”项涉及的文件纳入差异扫描范围。
...
- 在 `docs/08-implementation-plan.md` Phase 2 中新增任务 **“2.22 代码-文档同步机制”**，归属 P1，依赖 2.1/2.3/2.13。
...
- 已落地：`docs/NEWS_DATA_DEFINITION.md`。
...
| 角色 | 职责 |
|------|------|
| **架构师 / Tech Lead** | 审批文档同步方案；评审架构说明与数据字典；维护 `docs/03-architecture-standards.md` |
...
| 模块 | 代码文件 | 同步文档 | 验证结果 |
|------|----------|----------|----------|
| 金融业务 Widget | `src/cockpit/widgets/*`、`src/services/stock-analysis/*` | `ARCHITECTURE.md`、`DATA_DEFINITION.md` | tsc/lint/test/build 通过 |
| AI 中心板块 | `src/constants/ai-center.constants.ts`、`src/services/ai-center/*` | `docs/AI_CENTER_DATA_DEFINITION.md`、`docs/AI_CENTER_VUE3_EXAMPLES.md` | tsc/lint 通过 |
| NewsPage PoC | `src/pages/news-v6/*`、`src/services/news/newsService.ts` | `docs/NEWS_DATA_DEFINITION.md` | tsc/lint/test 通过 |

...
| # | 文档 | 当前版本 | 问题 | 需更新内容 |
|---|------|---------|------|-----------|
| 1 | `docs/CHANGELOG.md` | v1.2.0 (2026-06-27) | **完全缺失**批次 A~E 的全部变更记录（6次版本迭代） | 新增 v1.3.0~v1.7.0 条目；后续需新增"校正审计"条目 |
| 2 | `docs/implementation/v6-v9-architecture-audit-action-list.md` | v1.7.0 (2026-07-02) | 声称 26/26 已修复，但 **5+ 项与实际代码不符** | 回退偏差项状态为"未修复"；新增校正审计记录 |
| 3 | `docs/03-architecture-standards.md` | v1.1.0 (2026-06-26) | 行动清单声称已更新到 v1.4.0，**实际仍为 v1.1.0** | 版本号升级；补充 Store 广播、usePageGuard 等架构说明 |
| 4 | `docs/06-routing-specs.md` | v1.2.0 | **三处版本号冲突**（v1.2.0/v0.9.0-migration/v1.4.0）；仍含 `/analysis/news-v6` 和 `/input/prototype` 条目 | 统一版本号；删除/更新死路由条目；补充 14 条缺失路由 |
| 5 | `docs/implementation/completeness-profile.md` | v6.0.0 (2026-06-27) | 批次命名与行动清单冲突；25 项待修复状态未同步 | 区分"完成度审计批次"与"架构整改批次"；同步修复状态 |
...
|------|---------|-------------|-----------|
| B: P1-03 | "news-v6 已删除" | `src/pages/news-v6/` 含 8 个文件仍存在 | **P0 严重** |
| B: P1-04 | "input/prototype 已删除" | `src/apps/input/prototype/` 含 6 个文件仍存在 | **P0 严重** |
| C: P1-08 | "newsStore.ts 已不存在" | `src/store/newsStore.ts`（225 行）仍存在 | **P0 严重** |
...
- **`src/pages/news-v6/`**: 8 个文件（NewsPage.tsx、types.ts、6 个组件）
- **`src/store/newsStore.ts`**: 225 行，与 news-v6 循环依赖（3 处 import）
...
| 2 | 删除 | `src/pages/news-v6/` 整个目录（8 个文件） | 低 |
| 3 | 删除 | `src/store/newsStore.ts` | 低 |
...
- **`src/apps/input/prototype/`**: 6 个文件（BulkImportProto.tsx、DashboardProto.tsx、DataTestProto.tsx、HotSectorProto.tsx、InputPrototype.tsx、mockData.ts）
- **引用关系**: 仅内部互相引用，无外部消费者
...
| 步骤 | 文件 | 修改内容 | 复杂度 |
|------|------|---------|--------|
| 1 | 删除 | `src/apps/input/prototype/` 整个目录（6 个文件） | 低 |
...
| 步骤 | 文件 | 修改内容 | 复杂度 |
|------|------|---------|--------|
| 1 | `action-list.md` | 回退 5 项偏差的状态为"未修复"；新增校正审计记录 | 中 |
...
| 5 | `completeness-profile.md` | 区分批次命名；同步修复状态 | 中 |
...
**执行步骤**:
1. 删除 `src/pages/news-v6/` 目录（8 个文件）
2. 删除 `src/store/newsStore.ts`
3. 移除 `routes.ts` 中 `/analysis/news-v6` 路由
4. 删除 `src/apps/input/prototype/` 目录（6 个文件）

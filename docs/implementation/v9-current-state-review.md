...
| 文件 | 关键导出 | 职责 |
|------|----------|------|
| `src/services/stockpool/stockpoolService.ts` | `transitionStock`、`getStocksByStatus`、`getAllPoolGroups` | 股票池状态流转、分组加载 |
...
| `tradeErrorClassifier.ts` | 🚧 未建 | `docs/05-engine-specs.md` §3.2 | Phase 2/3 待建 |
| `tradeReviewAI.ts` | 🚧 未建 | `docs/05-engine-specs.md` §3.2/§3.8 | Phase 2/3 待建 |
| E2E 测试 | 🚧 未建立 | `docs/09-quality-gates.md` 门禁 #9 | Phase 3 引入 Playwright |
| CI 工作流 | 🚧 未建立 | `docs/09-quality-gates.md` §6.1 | Phase 3 建立 |
| PWA 离线验证 | 🚧 未建立 | `docs/09-quality-gates.md` 门禁 #11 | Phase 3 配置 |
...
| 2 | `docs/03-architecture-standards.md` | DB 版本写为 `3`；`inputConfig.ts` 标为待建；L2 数据层未提及 `signals` | DB 版本更新为 `4`；`inputConfig.ts` 标为已建；偏差清单 D07 改为已修复；L2 映射补充 `signals` |
| 3 | `docs/05-engine-specs.md` | `EnvelopeAction` 遗漏 `INSERT_SIGNAL`；ACL 矩阵遗漏 tradinghub 读 `signals`；偏差清单仍写输入舱缺少 `inputConfig.ts` | 补充 `INSERT_SIGNAL`；ACL 矩阵更新；偏差清单更新 |
| 4 | `docs/implementation/implementation-governance.md` | ADR 表只到 ADR-006；ADR 目录状态为「待建立」 | 补充 ADR-007；ADR 目录状态改为已建立 7 个 ADR |
| 5 | `docs/implementation/data-interaction-protocols.md` | `orders.signalId` 字段在代码中不存在 | 标注为「规划中」，说明将在复盘引擎阶段补齐 |
| 6 | `docs/02-functional-specs.md` | 导出示例仅含 stocks/v6_scores/orders/watchlists | 补充全部 Store |
| 7 | `docs/README.md` | 专项文档导航表遗漏 `input-cabin-ui-reshaping.md` 与 `investment-pipeline-stage-analysis.md` | 补录两项文档 |
| 8 | `docs/09-quality-gates.md` | 当前扫描基线仍写 107/107 passed；偏差收敛计划仍写输入舱缺少 `inputConfig.ts` | 更新为 291/291 passed；E2E 基线更新为 5/5 passed；偏差项改为已修复 |
...
4. **架构版本比对文档**：同步更新 `docs/implementation/architecture-version-comparison.md`，反映 v0.9.0-docs-review 的最新变化。
5. **输入舱原型清理**：`src/apps/input/prototype/` 在正式功能稳定后可考虑移除或归档，减少维护面。

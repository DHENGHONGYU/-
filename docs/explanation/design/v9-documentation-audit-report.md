---
title: v9-documentation-audit-report
tier: reference
code_version: 2.0.0
---
...
| `../../reference/01-vision-and-goals.md` | `v0.9.0-docs-review` | 🟢 基本有效 | 低 |
| `../../reference/02-functional-specs.md` | `v0.9.0-docs-review` | 🟡 流程图需补充新模块 | 中 |
| `../03-architecture-standards.md` | `v0.9.0-docs-review` | 🔴 多处严重滞后 | 最高 |
| `04-ui-ux-specs.md` | `v0.9.0-docs-review` | 🟡 图表/Widget 状态需更新 | 中 |
| `..` | `v0.9.0-docs-review` | 🟡 需核对（本次未全文审计） | 中 |
...
| `../../reference/architecture-version-comparison.md` | 未标注 | 🟡 部分状态已过时 | 中 |
| `../../reference/data-interaction-protocols.md` | 未标注 | 🟢 基本有效 | 低 |
| `./implementation-governance.md` | 未标注 | 🟡 ADR 数量不一致 | 中 |
...
- `../../reference/01-vision-and-goals.md`：纯前端、IndexedDB、离线可用、SMART 目标。
- `../03-architecture-standards.md` §3.2「调用方向铁律」、§3.3「数据访问规范」、§3.8「信封结构」：与 `DataBridge` + `ACL` 实现一致。
- `../../reference/data-interaction-protocols.md` §2.1~§2.5：跨模块写操作走 `DataBridge.forward()`、调用方向矩阵、事件总线命名规范。

### 4.2 数据协议与输入舱规格

- `input-cabin-spec.md` §4.1~§4.4：输入舱职责、端到端流程、数据协议、事件名。
- `../../reference/v6-to-v9-migration-spec.md`：V6 Pro 导出结构、12 个 store 映射、转换规则、导入顺序，与 `v6MigrationService.ts` 实现一致。
...
| # | 新增内容 | 目标路径 | 优先级 | 说明 |
|---|----------|----------|--------|------|
| N1 | 数据流引擎规格 | `../../reference/dataflow-engine-spec.md` | 高 | 补充 `src/core/dataflow/dataflowEngine.ts` 的通道模型、缓存策略、SSE/轮询、优先级、慢订阅者检测 |
...
| C2 | Store 清单 | `../03-architecture-standards.md` §3.7 / `../../reference/v9-system-blueprint.md` §5.4 | 8 个当前 store | 实际 16 个 store | 🔴 高 |
| C3 | `agents/` 状态 | `../03-architecture-standards.md` §3.1.1 / `../../reference/v9-system-blueprint.md` | 未创建 | `src/agents/agentRuntime.ts` 已存在 | 🟡 中 |
...
| C8 | 跨层调用基线 | `09-quality-gates.md` / `../../reference/v9-system-blueprint.md` | 0 违规 / 0 警告 | 0 违规 / **2 警告** | 🔴 高 |
| C9 | 硬编码基线 | `09-quality-gates.md` / `../../reference/v9-system-blueprint.md` | 71 处 | **389 处** | 🔴 高 |
| C10 | 死代码基线 | `09-quality-gates.md` / `../../reference/v9-system-blueprint.md` | 4 处 | **11 处** | 🔴 高 |
...
| C16 | ADR 数量不一致 | `implementation-governance.md` / `../../reference/v9-system-blueprint.md` | 7 个 / 6 个 | 实际 8 个 | 🟡 中 |
...
1. 新增 `../db-migration-v4-to-v6.md`。
2. 新增 `../quality-gates-baseline.md`。
3. 修正 `06-routing-specs.md`：补全路由表、修正组件映射、解释 HubPage 设计。
4. 修正 `../10-glossary.md`：补全模块 ID、`group` 字段、`watchlists`、迁移术语。
5. 修正 `../../reference/v9-system-blueprint.md`：ADR 列表、模块状态、质量基线。

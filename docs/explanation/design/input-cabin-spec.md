...
summary: "输入舱是 V9 研究工作流的起点，承担以下职责："
...
>
> 本文档是输入舱的专项业务蓝图，定义输入舱端到端流程、子页面职责、数据协议、服务层契约�?UI 组件映射�? 
> 目标读者：前端开发者、产品经理、架构师�?
---
...
1. **股票录入**：手动输入、搜索录入、批量文本导入�?2. **候选池管理**：五态股票池（candidate �?screened �?deepDive �?watching �?archived）的看板展示与流转�?3. **热门板块发现**：基�?V4/V6 评分�?mock 数据推荐板块，关联股票一键入库�?4. **数据采集测试**：验�?AKShare/本地数据源健康，拉取基础/K线数据�?5. **采集配置（P2�?*：运行时配置采集维度、频率、数据源优先级、限流�?
---

## 2. 端到端流�?
### 流程 A：单条搜索录�?
```
用户�?StockSearch 输入 "茅台"
  �?inputService.searchStocks("茅台")
  �?返回匹配列表（代�?名称/行业/PE/PB/市值）
  �?用户点击录入
...
```
用户�?PoolCard 点击「推送到观察池�?  �?poolTransitionEngine.validateTransition('candidate', 'watching', stock)
...
用户进入 /input/data-test
  ...
  �?返回基础/K线数据快�?  �?清洗检查标识缺失字�?```

---

## 3. 子页面职�?
| 子页�?| 页面路由 | 舱内组件 | 核心服务 | 主要职责 |
|--------|----------|----------|----------|----------|
| 录入看板 | `/input` �?`PortalShell` | `InputDashboard`（含 `StockSearch`, `PoolBoard`, `QualityIndicator`�?| `inputService`, `stockpoolService` | 单条/搜索录入、五态池看板、快捷入�?|
| 模块首页 | `/input/hub` �?`PortalShell` | `InputHubPage` | �?| 输入舱模块首页、功能导�?|
| 批量导入 | `/input/bulk-import` �?`PortalShell` | `BulkImportPanel` | `batchImportService` | 文本解析、行级状态、导入进度、错误明�?|
| 热门板块 | `/input/hot-sectors` �?`PortalShell` | `HotSectorPanel` | `hotSectorService` | 板块排名、因子进度条、轮动建议、关联股票入�?|
| 采集测试 | `/input/data-test` �?`PortalShell` | `DataTestPanel` | `fetcherService` | 数据源健康、单/批量探测、清洗检�?|
| 本地知识�?| `/input/local-knowledge` | `LocalKnowledgePage` | `localKnowledgeService` | 本地知识录入、分类、检索与股票关联 |
| 交互原型 | `/input/prototype` �?`PortalShell` | `InputPrototype` | mock | 临时 UI/UX 校对，确认后删除 |

> 路由映射规则：舱室入口（`/input`、`/input/hub`）及子页面统一�?`PortalShell` 渲染外壳，舱内具体组件由 `PortalShell` 根据当前 pathname 注入；`LocalKnowledgePage` 因作为独立全页，直接挂载�?`/input/local-knowledge`。新增子页面必须同步更新 `src/config/routes.ts` 与本表�?
---
...
| 事件�?| 触发时机 | 订阅�?|
|--------|----------|--------|
| `input:poolChanged` | 股票录入/导入/流转/删除�?| `InputDashboard`, `PoolBoard` |
| `input:fetcherStatusChanged` | 采集服务健康状态变�?| `DataTestPanel`, 顶部状态栏 |
| `input:importProgress` | 批量导入进度更新 | `BulkImportPanel` |

### 4.3 数据质量字段

输入舱负责维�?`Stock.dataQuality`�?
```ts
...
质量指示组件 `QualityIndicator` 据此展示�?�?绿点或进度条�?
---

## 5. 服务层契�?
### 5.1 `inputService.ts`（新�?增强�?
```ts
...
| `QualityIndicator` | `src/components/organisms/input/QualityIndicator.tsx` | 数据质量�?进度�?| `stock.dataQuality` |
...
| `FetcherConfigPanel` | `src/pages/input/FetcherConfigPage.tsx`（P2�?| 采集配置抽屉 | `fetcherConfigService` |
...
- [ ] 用户可通过搜索组件录入股票到候选池�?- [ ] 批量导入展示行级状态与导入进度�?- [ ] 股票池看板支持列表视图、复选批量操作、导�?导出�?- [ ] 热门板块展示因子进度条与轮动建议�?- [ ] 采集测试展示多数据源健康度与实时探测结果�?- [ ] 所有写操作�?`DataBridge.forward()` 并携�?`source: 'input-cabin'`�?- [ ] 新增功能均补充单元测试，整体测试通过�?100%�?
---
...
| 版本 | 时间 | 变化 |
|------|------|------|
| v0.9.0-docs-base | 2026-06-24 �?| 输入舱作为单页巨石组件，无搜索、无质量指示、无子页面拆�?|

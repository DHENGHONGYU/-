...
**Goal:** 治理 V9 数据架构的五个核心问题：Store 碎片化、刷新风暴、Python 依赖瓶颈、IndexedDB �?JOIN、硬编码残留，并逐项验证�?
**Architecture:** 采用渐进式治理策略——P0 删除死代码（零风险），P1 �?DataBridge broadcast 中引入优先级队列和重入锁保护（低侵入），P2 抽象 DataSourceProvider 接口解�?Python 依赖（中侵入），P3 引入 queryBuilder 封装�?Store 聚合查询（低侵入），P4 硬编码提取到常量（持续治理）�?
...
### 约束
- 所�?Page 组件必须处理 isLoading 状�?- 所�?engine 计算必须零硬编码数字/字符�?- L3/L4/L7/L8 是确定性层，L0/L1/L2/L5/L6 �?LLM 增强�?- 新模块必须遵循四步集成合同（类型定义 �?Store �?Builder �?核心集成�?- 测试必须在修改代码之前编写（TDD Red-Green-Refactor�?
### 关键发现
- **hotSectorStore / valuePitStore / rotationSignalStore** 三个 Store 已标�?`@deprecated`�?*零页�?组件消费�?*，是可安全删除的死代�?- **DataFlowEngine ChannelMeta 已定�?`priority: 'high'|'normal'|'low'`** 但未用于调度排序
- **6 �?Store 中仅 3 个有 isRefreshing 防重入锁**（signalStore/dualStrategyStore/disciplineStore），orderStore/holdingsStore/poolStore �?positionStore 无保�?- **DataSourceProvider 接口不存�?*，需新建
- **�?Store JOIN 核心场景�?dataFusionEngine.ts**�? Store 并行查询后内存聚合）
- **现有测试 80+ 文件**，覆�?fetcher/scoring/trading/strategy 等模块，盲区�?fetcherClient、dataFusionEngine、unifiedStockService

---

## Batch A �?Store 碎片化治理（P0，零风险删除�?
### Task 1: 删除 3 个废�?Store 文件
...
- Modify: `src/store/dualStrategyStore.ts` �?移除文件�?`@deprecated` 相关注释（如有引用旧 Store�?- Test: 运行全量测试确认无编译错�?
- [ ] **Step 1: 验证无消费者引�?*
...
```
Expected: 无任�?src/ 下的非自身文件引用（�?db.ts �?objectStore schema 定义除外，不影响运行�?
- [ ] **Step 2: 删除 6 个废弃文�?*
...
### Task 2: �?orderStore/holdingsStore/poolStore 添加 isRefreshing 防重入锁

**Files:**
- Modify: `src/store/orderStore.ts` �?initOrderStoreSubscriptions �?debounce callback 中添�?isRefreshing 检�?- Modify: `src/store/holdingsStore.ts` �?同上
- Modify: `src/store/poolStore.test.ts` �?同上
- Test: `tests/storeSubscriptions.test.ts`

- [ ] **Step 1: 编写失败测试 �?orderStore 去抖后无重入保护**

�?`tests/storeSubscriptions.test.ts` 中添加：
...
- [ ] **Step 3: �?orderStore.ts �?debounce callback 中添�?isRefreshing 检�?*

```typescript
// �?debounce 回调中，调用 refresh() 之前添加�?if (getState().isRefreshing) return
```

同时确保 state 中添�?`isRefreshing: boolean` 字段（如果不存在），并在 refresh 开始时设为 true、完成时设为 false�?
- [ ] **Step 4: �?holdingsStore.ts �?poolStore.ts 重复 Step 3**
...
### Task 3: �?positionStore 添加去抖 + isRefreshing 保护

**Files:**
- Modify: `src/store/positionStore.ts` �?initPositionStoreSubscriptions 中添�?200ms 去抖 + isRefreshing �?- Test: `tests/storeSubscriptions.test.ts`
...
describe('[Batch B] positionStore debounce + isRefreshing guard', () => {
  it('should debounce recomputeFromOrders and skip when isRefreshing', async () => {
    // 验证高频 orders 变化不会每次都触�?recompute
...
### Task 4: DataBridge broadcast 引入优先级调度队�?
**Files:**
- Create: `src/lib/eventBus.ts`
- Modify: `src/core/databridge.ts` �?broadcast() 方法中引�?queueMicrotask 优先级排�?- Test: `tests/priorityBroadcastQueue.test.ts`
...
export class PriorityBroadcastQueue {
  ...
  subscribe(channel: string, cb: (envelope: StandardEnvelope) => void, priority: 'high' | 'normal' | 'low' = 'normal'): () => void {
    // 注册 subscriber，记�?priority �?order
    // 返回 unsubscribe 函数
  }
  ...
  broadcast(channel: string, envelope: StandardEnvelope): void {
    ...
    if (!subs) return
    // �?priority 排序: high > normal > low，同优先级按 order
...
- [ ] **Step 4: �?databridge.ts �?broadcast() 中使�?PriorityBroadcastQueue**

向后兼容改造：
1. 保持现有 `subscribers` Map 结构不变
2. �?`subscribers` �?callback 类型扩展为带可�?priority
3. �?broadcast forEach 前按 priority 排序
4. 不改�?API 签名，默�?priority='normal'
...
### Task 5: 创建 DataSourceProvider 接口�?AkshareProvider 实现

**Files:**
- Create: `src/services/fetcher/types.ts` �?DataSourceProvider 接口定义
- Create: `src/services/fetcher/akshareProvider.ts` �?基于 fetcherClient �?AkShare 适配
...
describe('DataSourceProvider interface contract', () => {
  it('AkshareProvider implements DataSourceProvider', async () => {
    ...
    // 测试 healthCheck() 在服务不可用时返�?unhealthy
  ...
  it('DataSourceRegistry supports fallback chain', async () => {
    ...
    // 第一�?healthy �?provider 应被选中
    const active = await registry.getActiveProvider()
    expect(active.name).toBe('mock') // akshare 不可用时降级�?mock
  })
})
```
...
- [ ] **Step 4: 实现 AkshareProvider（包装现�?fetcherClient�?*

�?`fetcherClient` �?`collectBasic`/`collectKline`/`healthCheck` 适配�?`DataSourceProvider` 接口。不修改 fetcherClient 本身，只在外层包装�?
- [ ] **Step 5: 实现 MockProvider（内存数据，离线降级�?*

提供预置�?A 股基础数据�?0 只股票的模拟行情），不需要网络连接�?
- [ ] **Step 6: 实现 DataSourceRegistry（自动健康检�?+ 降级�?*
...
export class DataSourceRegistry {
  ...
  // 遍历 providers，返回第一�?healthCheck() === 'healthy' �?  // 如果全部 unhealthy，返回最后一个注册的（降级策略）
}
```

- [ ] **Step 7: 修改 fetcherService 注入 Provider**

�?fetcherService 的采集方法中，通过 Registry 获取 active provider，而非直接调用 fetcherClient。保�?fetcherClient 作为 AkshareProvider 的底层实现�?
- [ ] **Step 8: 运行测试**
...
## Batch D �?IndexedDB �?Store JOIN 治理（P1，低侵入封装�?
### Task 6: 创建 QueryBuilder 封装�?Store 聚合查询

**Files:**
- Create: `src/data/queryBuilder.ts` �?QueryBuilder 工具�?- Modify: `src/services/analysis` �?使用 QueryBuilder 替代手动 Promise.all
...
- [ ] **Step 4: �?dataFusionEngine.ts 中使�?QueryBuilder**

替换手动 6 �?dataLayer 调用�?`queryBuilder.queryStock({ symbol, includeBasic: true, includeV6Score: true, ... })`
...
### Task 7: 提取组件 HEX 颜色�?chartColors 配置
...
- [ ] **Step 2: 将高频出现的 HEX 颜色提取�?chartColors.ts**

将重�?>=3 次的 HEX 值提取为语义化命名常量（�?`CHART_UP_COLOR`, `CHART_DOWN_COLOR`, `CHART_NEUTRAL_COLOR`）�?
- [ ] **Step 3: 编写测试验证颜色常量完整�?*
...
### Task 8: 提取高频魔法数字�?thresholds 配置

**Files:**
- Modify: `src/config/thresholds.ts` �?添加新阈�?- Test: `tests/thresholds.test.ts`
...
- [ ] **Step 2: 将业务含义明确的数字提取�?thresholds.ts**
...
| 批次 | Task | 风险 | 预估时间 | 依赖 |
|------|------|------|---------|------|
| **Batch A** | Task 1: 删除废弃 Store | 零风�?| 10 min | �?|
| **Batch B** | Task 2: Store isRefreshing �?| �?| 20 min | �?|
| **Batch B** | Task 3: positionStore 去抖 | �?| 15 min | �?|
| **Batch B** | Task 4: 优先级广播队�?| �?| 30 min | �?|
| **Batch C** | Task 5: DataSourceProvider | �?| 45 min | �?|
| **Batch D** | Task 6: QueryBuilder | �?| 30 min | �?|
| **Batch E** | Task 7: 颜色常量提取 | �?| 15 min | �?|
| **Batch E** | Task 8: 魔法数字提取 | �?| 15 min | �?|

**每个 Batch 完成后需运行 `npx vitest run` 全量测试确认无回归�?*

**最终验收：**
- [ ] Store 文件数从 30+ 降至 ~27（删�?3 个废�?Store�?- [ ] 全部 6 个订�?Store 均有 isRefreshing 防重入锁
- [ ] DataBridge broadcast 支持优先级排�?- [ ] fetcherService 通过 DataSourceProvider 接口调用数据源，支持 Mock 降级
- [ ] dataFusionEngine 通过 QueryBuilder 执行�?Store 查询
- [ ] 全量测试 80+ 文件全部 PASS

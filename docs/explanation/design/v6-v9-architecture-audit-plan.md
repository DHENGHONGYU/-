---
title: v6-v9-architecture-audit-plan
tier: reference
code_version: 2.0.0
---
...
| DataBridge 是否按 V6 抽象实现 | 阅读 `src/core/databridge.ts`、`../../../src/showcase/index.ts` | 动作协议、缓存、ACL 完整性 | 🟡 骨架完成，动作枚举与广播有缺口 |
...
| 是否存在原型与正式面板重复 | 检查 `src/apps/input/` | 职责与正式面板重叠 | 🔴 存在：5 个原型面板 |
| 是否存在未引用孤儿页面 | 检查 `src/apps/command/ConfigApp.tsx` | 是否有路由引用 | 🔴 存在：ConfigPage 无路由引用 |

### 1.3 路由与地址一致性（Routing & Path）

| 检查项 | 检查方法 | 关注点 | 结论 |
|:---|:---|:---|:---|
| 路由表与规格文档是否一致 | 对比 `src/config/routes.ts` 与 `../../reference/06-routing-specs.md` | 路由数量、path、component | 🟡 文档记录 29 条，实际 35 条 |
...
1. `../../reference/completeness-profile.md` — 第十一章新增 V6→V9 架构一致性专项审计。
2. `../v6-v9-architecture-audit-action-list.md` — 26 项问题清单与批次规划。
3. 本文档 `./v6-v9-architecture-audit-plan.md` — 审计方法与关键证据。

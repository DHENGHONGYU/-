...
summary: "审查时间: 2026-07-04 审查范围: 系统解耦架构、模块间依赖关系、数据传递流�? 审查依据: MCP (Minimum Coupling..."
...
# V9 智能投研复盘系统 - MCP 最小耦合原则合规性分析报�?
**审查时间**: 2026-07-04  
**审查范围**: 系统解耦架构、模块间依赖关系、数据传递流�? 
...
- **数据持久�?*: IndexedDB、dataLayer、dbConfig
- **配置与常量层**: dbConfig、store-channels.constants
- **审计工具**: audit-layer-calls.ts

### 总体结论
🟡 **需修正** - 系统整体架构设计优秀，符�?MCP 原则的核心思想，但在部分实现细节上存在耦合度偏高、职责边界模糊的问题�?
---
...
- �?**元数据追�?*: traceId、timestamp、source 提供完整的审计链�?- �?**类型安全**: 通过 TypeScript 泛型和类型守卫保证编译期检�?- �?**零硬编码**: EnvelopeAction、EnvelopeTarget 全部从配置注�?
**评分**: ⭐⭐⭐⭐�?(5/5)
...
**文件**: `src/core/databridge.ts` (760 �?

**核心职责**:
- 接收所有数据写入请求（Envelope�?- ACL 权限校验
- 路由分发（routeToDB / routeToStrategy / routeToManager�?- 广播通知（EventBus + Subscriber�?- 审计日志记录

**符合 MCP 的设�?*:
- �?**单一职责**: DataBridge 只负责路由和分发，不处理业务逻辑
- �?**依赖倒置**: 服务层通过 DataBridge 访问数据，不直接依赖 IndexedDB
- �?**开闭原�?*: 新增 action 只需�?routeToDB 中添�?case，不修改核心逻辑
...
**文件**: `src/lib/eventBus.ts` (113 �?

**符合 MCP 的设�?*:
- �?**松耦合**: 发布者与订阅者无需知道对方存在
- �?**动态订�?*: 运行时注�?注销监听�?- �?**错误隔离**: 单个订阅者失败不影响其他订阅�?- �?**内存安全**: 提供 off() 方法防止内存泄漏
...
**符合 MCP 的设�?*:
- �?**声明式权�?*: ACL_MATRIX 集中定义所有模块权�?- �?**最小权限原�?*: 每个模块只能访问必需�?store 和操�?- �?**运行时校�?*: DataBridge.forward() 强制执行 ACL 检�?- �?**零硬编码**: ModuleId、StoreName、DbOperation 全部从配置注�?
...
**符合 MCP 的设�?*:
- �?**职责分离**: Store 只负责状态管理，广播逻辑�?withBroadcast 处理
- �?**代码复用**: 避免每个 Store 手写 EventBus.emit()
- �?**错误隔离**: 广播失败不影�?Store 写操�?
**关键代码** (L42-49):
```typescript
...
**文件**: `src/core/memoryCache.ts` (325 �?

**符合 MCP 的设�?*:
- �?**独立模块**: 缓存逻辑与业务逻辑完全解�?- �?**可配�?*: TTL、maxSize、cleanupInterval 支持自定�?- �?**自动清理**: 定时�?+ LRU 淘汰防止内存溢出
...
### �?7. 分层审计工具自动化检�?
**文件**: `scripts/audit-layer-calls.ts` (299 �?

**符合 MCP 的设�?*:
- �?**静态分�?*: 通过正则匹配检测跨层调用违�?- �?**CI/CD 集成**: 退出码 1 表示发现违规，阻断构�?- �?**分级告警**: violation（阻断）vs warning（过渡期允许�?
**检查规�?* (L46-50):
```typescript
// L5/L4 禁止直接�?dataLayer
...
## 🔴 需要改进的问题�?
### �?问题 1: DataBridge.routeToDB() 违反开闭原�?
**文件**: `src/core/databridge.ts` L218-444  
**严重程度**: 🟡 一�? 
**违反规范**: 开闭原则（OCP�?
**问题描述**:
`routeToDB()` 方法包含 30+ �?case 分支，每新增一�?action 都需要修改此方法。这导致�?- 代码膨胀�?26 �?switch-case�?- 测试困难（需要覆盖所有分支）
- 合并冲突频繁（多人同时新�?action�?
**代码示例**:
```typescript
...
**修复建议**:
采用**策略模式**将每�?action 的处理逻辑封装为独立的 Handler 类：
...
// 3. 使用 Handler 注册�?class DataBridge {
...
**收益**:
- �?新增 action 只需添加�?Handler，不修改 DataBridge
- �?每个 Handler 可独立测�?- �?减少合并冲突
...
**问题分析**:
- 服务层可以直接调�?`dataLayer.stocks.get()`，绕�?DataBridge �?ACL 检�?- 无法统一拦截读操作（如缓存、审计、权限校验）
- 违反"所有数据访问必须通过 DataBridge"的架构约�?
**修复建议**:
为读操作也引�?DataBridge 的查询信封（Query Envelope）：
...
async query<T>(envelope: QueryEnvelope): Promise<T | undefined> {
  // ACL 检�?  aclEngine.assert({ module: meta.source, store: targetStore, operation: 'SELECT' })
...
**收益**:
- �?统一读写操作的权限校验和审计
- �?可在 DataBridge 层添加缓存、限流等横切关注�?- �?完全符合依赖倒置原则

---

### �?问题 3: EventBus 事件名称硬编码分�?
...
**问题分析**:
- 同一事件有多个定义，容易拼写错误
- 重构时需要修改多�?- 无法全局搜索事件的所有使用位�?
**修复建议**:
统一事件名称定义�?`src/lib/eventBus.ts`�?
```typescript
...
**收益**:
- �?单一事实来源（Single Source of Truth�?- �?IDE 自动补全和重构支�?- �?编译期检查拼写错�?
---

### �?问题 4: DataBridge 缺少读操作缓存集�?
...
// �?缺少：写操作后更新缓�?// �?缺少：读操作时先查缓�?```

**问题分析**:
- 高频读取（如股票列表、评分数据）每次都访�?IndexedDB，性能较差
- 写操作后缓存未失效，可能导致数据不一�?
**修复建议**:
�?DataBridge 中集�?MemoryCache�?
```typescript
...
class DataBridge {
  ...
  async forward(envelope: StandardEnvelope): Promise<void> {
    // 写操�?    await this.routeToDB(envelope, targetStore)
...
**收益**:
- �?读性能提升 10-100 倍（内存 vs IndexedDB�?- �?自动缓存失效保证数据一致�?- �?减少 IndexedDB 事务冲突
...
**问题描述**:
部分 dataLayer 方法直接调用 `db.put()` / `db.delete()`，绕�?DataBridge�?
```typescript
...
async update(id: string, updates: Partial<ExecutionPlan>): Promise<DataLayerResult<ExecutionPlan>> {
  ...
  await db.put('execution_plans', updated)  // �?直接�?db
...
function inferStore(action: string): StoreName {
  ...
  // ...
  return STORE_NAME.stocks  // �?默认返回 stocks，可能掩盖错�?}
```

**问题分析**:
- 字符串包含判断不够精确（�?`UPDATE_STOCK` �?`DELETE_STOCK` 都会匹配 `stocks`�?- 默认返回 `stocks` 可能掩盖未处理的 action
...
## �?通过项（优秀实践�?
### 1. 分层架构清晰
- �?配置层（config）：零硬编码，纯声明�?- �?核心层（core）：DataBridge、Envelope、ACL 职责明确
- �?数据层（data）：IndexedDB 封装，提供统一接口
- �?服务层（services）：业务逻辑，通过 DataBridge 访问数据
- �?状态层（store）：Zustand 状态管理，withBroadcast 广播
- �?页面层（pages）：UI 组件，只依赖 store �?services

### 2. 数据流闭环完�?- �?写操作：Service �?DataBridge �?ACL �?DB �?EventBus �?Store �?UI
- �?读操作：UI �?Store �?Service �?DataLayer �?DB（需改进为通过 DataBridge�?- �?广播机制：所有写操作自动触发 EventBus 通知

### 3. 错误处理健壮
- �?DataBridge.forward() 包含完整 try-catch
- �?EventBus.emit() 单个订阅者失败不影响其他
- �?withBroadcast 广播失败不影�?Store 写操�?- �?FallbackQueue 降级队列处理市场数据写入失败

### 4. 可观测性强
- �?所有关键操作都�?logger.info/debug 日志
- �?traceId 贯穿整个数据流链�?- �?writeAuditLog() 记录所有写操作�?research_logs
- �?MemoryCache 提供命中率统�?
### 5. 类型安全保证
- �?所有数据结构都�?TypeScript Interface 定义
- �?EnvelopeAction、EnvelopeTarget、ModuleId 等使�?const + type
- �?禁止使用 any（ESLint 规则强制�?- �?类型守卫（type guards）保证运行时类型安全
...
**优势**:
1. �?**信封模式**实现了数据传递的标准化和可追踪�?2. �?**DataBridge**作为统一路由中枢，职责清晰、扩展性强
3. �?**EventBus**发布-订阅模式有效解耦了模块间通信
4. �?**ACL 权限矩阵**实现了细粒度的访问控�?5. �?**分层审计工具**自动化检查跨层调用违�?
**劣势**:
1. �?**routeToDB()**包含 30+ case 分支，违反开闭原�?2. �?**dataLayer**部分方法直接访问 db，绕�?DataBridge
3. �?**事件名称**硬编码分散在多个文件�?4. �?**读操�?*未集成缓存，性能有待优化
5. �?**部分写操�?*（update、incrementRetry）未通过 DataBridge

### 优先处理建议

#### P0（阻断性）- 立即修复
1. **修复 dataLayer 直接�?db 的方�?*（问�?5�?   - 涉及文件：`src/data/dataLayer.ts` L474-485, L538-544, L562-564
   - 预计工作量：2 小时
   - 风险：低（只是改为通过 DataBridge�?
#### P1（严重）- 本周内完�?2. **重构 routeToDB() 为策略模�?*（问�?1�?   - 涉及文件：`src/core/databridge.ts` L218-444
   - 预计工作量：1 �?   - 风险：中（需要测试所�?action�?
3. **读操作也通过 DataBridge**（问�?2�?   - 涉及文件：`src/core/databridge.ts`、`src/data/dataLayer.ts`
   - 预计工作量：1 �?   - 风险：中（需要添�?QueryEnvelope�?
#### P2（优化）- 本月内完�?4. **统一事件名称常量**（问�?3�?   - 涉及文件：多�?   - 预计工作量：4 小时
   - 风险：低

5. **集成 MemoryCache �?DataBridge**（问�?4�?   - 涉及文件：`src/core/databridge.ts`
   - 预计工作量：4 小时
   - 风险：低

6. **inferStore() 改为显式映射**（问�?7�?   - 涉及文件：`src/core/databridge.ts` L58-79
   - 预计工作量：2 小时
   - 风险：低

### 下一步建�?
1. **立即执行 P0 修复**：将 dataLayer 中直接写 db 的方法改为通过 DataBridge
2. **运行分层审计**：`npm run audit:layers` 确认无新增违�?3. **补充单元测试**：为 DataBridge 的所�?action 添加测试用例
4. **性能基准测试**：对比集�?MemoryCache 前后的读操作性能
5. **代码审查**：组织团队审查本报告的改进建议，确认优先�?
---
...
- [x] 开闭原则（新增 action 只需添加 case�?- [x] 依赖倒置（服务层依赖 DataBridge 抽象，不依赖 IndexedDB�?- [x] 最小权限原则（ACL 矩阵限制模块访问范围�?- [x] 零硬编码（所有配置从 dbConfig 注入�?
### �?需改进
- [ ] 开闭原则（routeToDB �?switch-case 需改为策略模式�?- [ ] 依赖倒置（dataLayer 部分方法直接访问 db�?- [ ] 单一事实来源（事件名称分散在多个文件�?- [ ] 性能优化（读操作未集成缓存）

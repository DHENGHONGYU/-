---
title: V6 Pro �?V9 源码比对与二次开发重点模块梳�?code_version: 2.0.0
type: explanation
domain: project
phase: planning
tier: important
...
summary: "┌─────────────────────────────────────────────────────────────�?
tags: [project, migration, analysis, plan, explanation]
version: v0.9.0
...
1. **禁止引入 Node 后端**：V9 为纯前端架构，V6 �?tRPC 调用必须替换为本�?`services/` 层调用�?2. **写操作必�?DataBridge**：所�?IndexedDB 写入必须通过 `DataBridge.forward(StandardEnvelope)`，禁�?L5/L4 直接调用 `dataLayer`�?3. **路由注册规范**：新增页面必须在 `src/config/routes.ts` 注册，并同步更新 `../reference/06-routing-specs.md`�?4. **配置优先**：阈值、权重、解析规则必须从 `src/config/*.ts` 读取�?5. **API 兼容�?*：对外暴露的服务方法签名尽量保持不变，避免影响上层调用�?6. **不新增大型依�?*：除非经�?ADR 评审�?
### 9.2 数据层风�?
...
### 9.4 测试与文档风�?
1. **测试覆盖**：V9 当前要求 `npm test` 全部通过，新增模块需同步补充单元测试�?2. **文档同步**：每新增一个模块，需更新 `../reference/02-functional-specs.md`、`../reference/05-engine-specs.md`、`../reference/06-routing-specs.md`、`../reference/08-implementation-plan.md`�?3. **审计基线**：新增代码可能增�?`audit:hardcode` 的静默回退/魔法数字计数，需保持 Fatal �?0，并尽量控制 Critical/Major 增长�?
---

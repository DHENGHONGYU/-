...
### 1.4 2026-07-11/12 测试失败继续收敛（新增 6 项）

| 文件 | 问题 | 修复动作 |
|------|------|----------|
| `tests/InputApp.test.tsx` | poolStore 经 DataBridge.query 读取，mock 的 `dataLayer.stocks.list` 未生效；子路由测试用 `MemoryRouter` 与 `HashRouter` 行为不一致 | mock `dataBridge.query` 返回股票列表；`renderApp` 改用 `HashRouter` 并设置 `window.location.hash`；批量导入 mock 对齐 `importStocksWithProgress`/`detectDuplicates`；skip 已移除的列表视图切换用例 |
| `tests/TradingApp.test.tsx` | `loadPortfolio` 先调用 `portfolioService.loadPortfolioInput` 走 DataBridge，未 mock 导致挂起 | 用 `vi.hoisted` + `vi.mock` 拦截 `loadPortfolioInput`，使其直接返回 `{ stocks: [], orders: [] }` |
| `src/cockpit/widgets/HotSectorWidget.test.tsx` | 空态断言写死旧文本「暂无热门板块策略」，与组件当前 `UI_TEXT.analysis.hotSector.noData` 不一致 | 更新为「暂无热门板块策略数据」 |
| `src/cockpit/widgets/ValuePitWidget.test.tsx` | 空态断言写死旧文本「暂无价值洼地策略」，与组件当前 `UI_TEXT.analysis.valuePit.noData` 不一致 | 更新为「暂无价值洼地策略数据」 |
| `src/store/tradingStore.ts` | `loadPortfolio` 中 `portfolio` 可能为 `undefined` 导致 `portfolio.holdings.length` 报错 | 增加可选链 `portfolio?.holdings.length ?? 0` |
| `src/cockpit/CockpitShell.tsx` | `SafeWrapper` 在组件内部定义 `useMemo` 后仍出现条件返回前的 hooks 问题 | 调整 `SafeWrapper` 定义位置，确保 hooks 顺序稳定 |

> 以上修复后，29 个历史失败测试文件已全部独立通过。
### 1.1 剩余 P0 核心层嵌套（3 项）
...
| `npm run test -- --run` | ✅ 已通过 | 29 个历史失败测试文件已修复；`test:clean` 后台最终确认 **317 files / 4610 tests passed / 15 skipped / exit 0** |
...
1. **全量测试已最终确认**：后台 `npm run test:clean -- --run` 已完成，结果 **317 test files passed / 4610 tests passed / 15 skipped / Duration 1011.16s / exit 0**。日志见 `test-clean-output.log`。
2. **保持门禁**：后续新增代码继续通过 `tsc:prod` + `npm run audit` + `complexity-scan` + `test:clean` 回归。
...
- **P0 核心层复核**：`src/mcp/core/server.ts::readResource`、`src/mcp/core/client.ts::readResource` 经此前轮次已抽取 `checkResourcePermission`/`resolveResource`/`findResourceMatch` 辅助函数，本次复核确认 0 深层嵌套。
- **P0/P1 深层嵌套清零**：对剩余 7 处深层嵌套（≥4 层）全部抽取辅助函数扁平化：
  - `src/core/databridgeHandlers.ts`：`deleteIndexedRecords`/`deleteScannedRecords` → 抽取 `deleteBySymbolIndex`/`deleteBySymbolScan`（嵌套 4→≤3）。
  - `src/core/dataflow/dataflowEngine.ts`：`connect` 内 SSE 消息解析 → 抽取 `_handleSseMessage`（消除 2 处 depth=4）。
  - `src/data/db-schema.ts`：`createSchema` 内 group 字段 backfill 游标 → 抽取模块级 `backfillGroupField`（消除 depth=4）。
  - `src/services/resilience.ts`：`withRetry` 重试循环 try/catch → 抽取 `safeCall`（嵌套 4→≤2）。
  - `src/services/data-collector/TaskScheduler.ts`：错误监听器广播 → 抽取 `notifyErrorListeners`（消除 depth=4）。
- **结果**：`complexity-scan` 深层嵌套 **7 → 0**，长链条件 0、重复条件 0；基线 `.complexity-baseline.json` 已刷新为全 0。

### 7.2 颜色硬编码收尾（基线刷新）

- 复核 `eslint --config eslint.colors.config.js`（lint:colors）全局 **0 违规**；`audit:hardcode` 仅 32 条「静默回退」Warning（非颜色硬编码），0 Critical。
- 原清单 `docs/reports/hardcoded-colors-inventory.json` 记录的 6 处（FundFlowWidget/InputDashboard/MarketIndicesWidget/PortfolioOverviewWidget）经历史轮次已迁移完毕，本次将清单刷新为 `totalViolations: 0`、remediationPlan 全 `completed`、`lastUpdated: 2026-07-12`，并补充 `baseline` 字段说明 lint:colors 与 audit:hardcode 的真实状态。

### 7.3 性能监控（关键路径耗时日志）

新增性能监控基础设施，建立真实性能数据：
- `src/lib/perf.ts`：`measureAsync` / `measureSync` / `recordPerf` / `getPerfStats`（count/avg/p50/p95/max 聚合）/ `clearPerf`，标签常量 `PERF.DATA_FETCH_REQUEST`、`PERF.SCORING_CALCULATE_ALL`。
- `src/hooks/usePerfTrace.ts`：组件渲染耗时 Hook（`render:<Name>` 标签）。
- 埋点三关键路径：
  - 数据获取：`src/services/fetcher/fetcherClient.ts::request`（标签 `data-fetch:request`）。
  - 评分计算：`src/services/scoring/v6-engine/engine.ts::calculateAll`（标签 `scoring:calculateAll`）。
  - 图表渲染：`src/components/chart/{LineChart,AreaChart,BarChart,ScoreRadar}.tsx` 接入 `usePerfTrace`（标签 `render:LineChart` 等）。
- 所有 `[PERF]` 日志经 `getLogger()` 输出，运行时可经 `getPerfStats()` 实时聚合热点。
...
- **Tests**：4610 passed | 15 skipped (4625)
- **Duration**：1011.16s（约 16m 51s）
- **Exit Code**：0

全部门禁（类型/审计/复杂度/颜色/测试子集）均已绿，本轮优化无新增回归。

---

*附：原始嵌套评审数据已更新至 `nested-code-review-report.json`；复杂度基线见 `.complexity-baseline.json`。*

---

## 九、优化继续（2026-07-12 续）：计划失准核查 + 真实债务清理

> 用户指令「按任务继续优化」。本轮先对 `optimization-plan.md`（185 项）做**当前代码实测核对**，发现计划快照严重过时，遂以真实债务为准推进。

### 9.1 关键发现：计划已失准（185 → 95 真实项）

- 计划生成于 **2026/7/10 08:47**，但两个旗舰项在 **7/11** 已被重构：
...
- 实测结果固化为 `complexity-baseline-current.json`（93 项，见 9.3 修复后），作为后续「债务只降不升」对照锚点。
- 说明：项目 canonical `.complexity-baseline.json`（0/0/0）未动；其阈值偏保守是治理层面的独立议题，本轮不改其策略。

### 9.3 本轮已修复的真实项（2 项）

| 文件 | 函数 | 问题 | 修复方式 |
|------|------|------|----------|
| `src/services/scoring/valuePitAnalyzer.ts` | `analyze` | 唯一真实长链（4 分支 `if-else-if` 赋 `action`） | 提取 `resolveAction()` 早返回守卫，行为等价 |
| `src/core/databridge.ts` | `extractSymbolFromPayload` | D4（数组项 symbol 判断） | 卫语句（早返回）拍平至深度 2，行为不变 |

- 修复后实测：长链 **1→0**、深度嵌套 **65→64**，合计 **95→93**。

### 9.4 重要风险发现：重复条件多不可安全提取

- 计划与实测均列出大量「重复条件」项（29 处）。逐条甄别后发现：多数重复是**状态相关判断**，提取为函数入口局部变量会**改变并发/时序语义**。
- 例：`src/services/resilience.ts::guard` 中 `state === 'half-open'` 在 `Promise.then` 的两个回调各出现一次——该状态在异步执行期可能被并发修改，必须在回调内即时求值，不可提前提取。
- 结论：重复条件项**不盲目批量提取**，仅对纯函数式、无副作用、无状态依赖的判定做提取。本轮未做此类提取（避免引入回归）。

### 9.5 既存门禁阻断项修复

- `tsc:prod` 报 `src/store/marketDataStore.ts(462)` 类型比较无重叠（`key: MarketDataSourceKey | undefined` 与 `''` 无重叠）。该文件在本轮优化前即被修改未提交，属**既存问题、非本次引入**。
- `key !== ''` 为死判断（`key` 类型本就排除 `''`），修复为 `key ?? 'unknown'`，行为等价，解除 `tsc:prod` 阻断。
...
|------|------|
| `tsc:prod` | ✅ exit 0 |
| `audit:layers` | ✅ 0 违规 / 0 警告（868 文件） |
| `audit:atomic` | ✅ 0 违规（133 文件） |
| `lint:colors` | ✅ 0 违规 |
| 定向单测 | ✅ 49 passed（valuePitAnalyzer ×2 + databridge） |

### 9.7 剩余真实债务与建议

- 真实剩余：**深度嵌套 64**（多为 D4，最低严重度档，结构来自 try/switch/case/if）、**重复条件 29**（多数不可安全提取）、**长链 0**。
- 建议下步：
  1. 以 `complexity-baseline-current.json` 为锚，逐文件安全提取 D4（优先核心/基础设施文件，如 `databridge.query`、`localStorageManager` 三处）；

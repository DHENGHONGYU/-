...
summary: "版本: v1.0.0 | 日期: 2026-07-13 适用范围: 所�?`src/` 下的 TypeScript / TSX 文件 强制等级: 必须遵守 关联文档:..."
 ...
 * @param userId - 用户唯一标识
 * @param options - 可选配�? * @returns 用户信息对象
 * @throws {Error} �?userId 为空时抛�? *
 * @example
 * ```typescript
...
| `@returns` | 返回值说�?| �?|
| `@throws` | 异常说明 | 推荐 |
| `@example` | 用法示例 | 推荐（公�?API�?|
| `@template` | 泛型约束说明 | 复杂泛型时强�?|
| `@deprecated` | 标记废弃 | 废弃函数时强�?|

## 四、禁止事�?
- �?只写 `@param userId` 而无参数用途说�?- �?复制函数签名作为 JSDoc 内容
- �?在简�?getter/setter 上写冗余 JSDoc
...
<!-- merge-source: docs/reference/jsdoc-convention.md (2026-07-14 内容融合，避免去重丢失有效信�? -->
## 补充内容（合并自 `docs/reference/jsdoc-convention.md`�?
# JSDoc 与文档门禁规�?## 1. 必须使用 JSDoc 的实�?| 实体类型 | 最小要�?| 示例位置 |
...
## 2. JSDoc 模板
 * 一句话说明函数做什么�? * @param symbol - 股票代码（如 600519�? * @param startDate - 起始日期 ISO 字符�? * @returns 计算后的评分对象
 * @throws �?symbol 为空时抛�?ValidationError
export function calculateScore(symbol: string, startDate: string): ScoreResult { ... }
 * 自选股 Widget
 * @description
 * 展示用户自选股的实时价格与涨跌幅。数据通过 `useMarketData()` 注入�? * @param props.config - Widget 实例配置
...
- `src/components/`（导出组件）
- `src/pages/`（页面组件）
输出缺失 JSDoc 的函�?组件列表，并给出文件路径与行号�?## 5. �?AGENTS.md 的关�?本文档是 `../../AGENTS.md` 中「日志规范」与「零硬编码」的延伸，AI 在生成代码时应按本规范补�?JSDoc�?
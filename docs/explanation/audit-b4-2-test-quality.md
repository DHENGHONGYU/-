...
| 模块类别 | 未覆盖模块 | 风险等级 | 说明 |
|---------|-----------|---------|------|
| **数据层** | `src/data/db.ts` (IndexedDB 封装) | ⚠️ 中高 | 核心数据持久化层，虽通过 service 间接测试，但缺少直接单元测试 |
...
| 内存边界 | 大数据量下的内存/性能测试缺失 | ⚠️ 中高 | 增加大数据量（1000+ 股票）场景测试 |
| 组件边界 | UI 组件的边界 props 测试不足 | ⚠️ 中高 | 为核心 UI 组件补充边界 props 测试 |

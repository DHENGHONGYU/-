...
summary: "生成时间: 2026-07-13 演练类型: 文档推演 + 命令验证（非实际生产回滚） 回滚目标: 前端静态资源（Vite SPA）+ IndexedDB 数据兼容"
...
降级兼容性: 
  - 前端静态资源: ? 版本号切换可行
  - IndexedDB: ?? 需验证旧代码对新 schema 的兼容性
...
| 数据一致性 | 100% | 依赖 IndexedDB 迁移脚本 | ?? 需测试降级兼容性 |

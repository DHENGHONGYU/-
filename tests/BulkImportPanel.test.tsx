...
// sevenDimConfigStore — zustand store mock（BulkImportPanel 通过 getState() 调用 getCollectionConfig）
vi.mock('@/store/sevenDimConfigStore', () => ({
  useSevenDimConfigStore: {
    getState: () => ({
      getCollectionConfig: () => ({ dimensions: [], timeout: 30000 }),
    }),
  },
}))

// collectPoolSymbols — 采集服务 mock（仅在点击采集按钮时调用）
vi.mock('@/services/pool/collectionService', () => ({
  collectPoolSymbols: vi.fn().mockResolvedValue({ totalDimensions: 0, failedDimensions: 0 }),
}))

// react-router — BulkImportPanel 内部使用 <Link>，需 mock 避免缺少 Router 上下文报错

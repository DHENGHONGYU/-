...
- Name: functional\data-flow.spec.ts >> 数据流测试 >> 股票导入 → 意向池更新 >> 输入 000001 平安银行后应出现在意向候选池中
- Location: functional\data-flow.spec.ts:20:5
...
Locator: locator('text=平安银行').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('text=平安银行').first()
...
```ts
  ...
  17  |       await expect(page.getByRole('textbox', { name: '股票代码' })).toBeVisible({ timeout: 10000 })
  18  |     })
  19  | 

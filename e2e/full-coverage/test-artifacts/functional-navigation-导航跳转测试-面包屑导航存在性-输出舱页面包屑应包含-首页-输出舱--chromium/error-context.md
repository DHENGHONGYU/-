...
- Name: functional\navigation.spec.ts >> 导航跳转测试 >> 面包屑导航存在性 >> 输出舱页面包屑应包含"首页 > 输出舱"
- Location: functional\navigation.spec.ts:161:5
...
Locator: getByRole('heading', { name: '输出舱' }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: '输出舱' }).first()

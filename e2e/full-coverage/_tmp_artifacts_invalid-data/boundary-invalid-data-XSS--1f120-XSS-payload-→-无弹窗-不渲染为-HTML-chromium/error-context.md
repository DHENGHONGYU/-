...
- Name: boundary\invalid-data.spec.ts >> XSS payload >> 输入 XSS payload → 无弹窗/不渲染为 HTML
- Location: boundary\invalid-data.spec.ts:77:3
  ...
  - table:
    - rowgroup:
      - row "# 代码 名称 标准化 状态":
        ...
        - columnheader "状态"
    - rowgroup:
      - row "1 <script>alert(\"xss\")</script> 无效 代码格式不合规（需为 6 位数字）":
        - cell "1"
        - cell "<script>alert(\"xss\")</script>"
        - cell
        - cell
        - cell "无效 代码格式不合规（需为 6 位数字）"
  - button "无可导入行" [disabled]
```

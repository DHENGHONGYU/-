...
test.describe('输出舱功能测试', () => {
  test.describe('输出舱首页', () => {
    ...
    test('应显示三个进入链接', async ({ page }) => {
      const links = page.getByRole('link', { name: '进入' })
      await expect(links).toHaveCount(3)
    })

    test('侧边栏应显示输出模块导航', async ({ page }) => {
      await expect(page.getByRole('button', { name: '输出舱首页' })).toBeVisible()
      await expect(page.getByRole('button', { name: '研究报告' })).toBeVisible()

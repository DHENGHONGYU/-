...
import { Settings, Plus, Target, Activity, Bot, Lightbulb } from 'lucide-react'
...
function CockpitContent(): React.JSX.Element {
  ...
  const handleAddWidget = (): void => {
    const priority = [
      'catalystEvent', 'keyDataCalendar', 'marketSentiment', 'signalQuality',
      'kaiScore', 'poolBoard', 'hotSector', 'valuePit',
      'pnlAnalysis', 'riskMonitor', 'modelCompare', 'aiTradeReview',
    ]
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground hidden sm:block">投资总览 · 策略与时机 · 智能研判</p>
            </div>
            <div className="hidden items-center gap-1.5 ml-2 text-xs text-muted-foreground sm:flex">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-muted/60 px-2.5 py-0.5">
                <span className="font-medium text-foreground">{stats.running}/{stats.total}</span> 数据采集
              </span>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-2">
        <div className="flex items-center justify-between rounded-lg border bg-card/50 px-4 py-2">
          ...
          <Link to="/command/system-health" className="text-xs text-primary hover:underline">
            校对总控舱 →
          </Link>
        </div>
      </section>

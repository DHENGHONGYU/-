      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className={`${COLOR_SHADES.gray[50]} rounded-lg p-3 text-center`}>
            ...
            <Badge
              variant="outline"
              className={`text-xs mt-1 ${data.varLevel === 'high' ? `${COLOR_TOKENS.danger.tailwind} ${twBorder('red', 300)}` : data.varLevel === 'medium' ? `${twText('yellow', 500)} ${twBorder('yellow', 300)}` : `${COLOR_TOKENS.success.tailwind} ${twBorder('green', 300)}`}`}
            >
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="text-center">
            <div className={`text-lg font-bold ${COLOR_SHADES.gray[700]}`}>{data.betaEstimate}</div>

 ...
 * @example
 * // 调用评分 Server 对当前标的打分
 * { id: 's1', type: 'mcp_tool', server: 'scoring:v6', tool: 'score_stock', args: { symbol: '{{target}}' } }
 * // 等待 1 秒
 * { id: 's2', type: 'delay', ms: 1000 }
 */
export interface WorkflowStep {
  /** 步骤唯一 ID（同工作流内唯一，用于变量引用 {{steps.<id>.output}}） */
  id: string
  /** 步骤类型 */
  type: WorkflowStepType
  /** 步骤名称（可选，用于可读性） */
  name?: string
  /** type=mcp_tool 时的目标 Server 名（info.name 或 registry 名，引擎模糊匹配） */
  server?: string
  /** type=mcp_tool 时的 Tool 名 */
  tool?: string
  /** type=mcp_tool 时的参数（支持变量替换，见 resolveArgs） */
  args?: Record<string, unknown>
  /** type=delay 时的等待毫秒数 */
  ms?: number
  /** 出错时策略：stop（中断整个运行，默认）/ continue（标记失败并继续） */
  onError?: StepOnError
  /** 失败重试次数（默认 0） */
  retry?: number
}
    ...
    return [
      ...
      },
      {
        ...
        resolver: async (uri) => {
          ...
          if (!run) return { uri, mimeType: 'application/json', text: JSON.stringify({ error: `run not found: ${runId}` }) }
      ...
      if (typeof val === 'string') {
        return val
          ...
          .replace(/\{\{steps\.([\w-]+)\.output(?:\.([\w.]+))?\}\}/g, (_m, id: string, path?: string) => {
            ...
            const nested = path.split('.').reduce<unknown>((o, k) => ((o && typeof o === 'object') ? (o as Record<string, unknown>)[k] : undefined), out)

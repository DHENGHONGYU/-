...
const AgentTaskPanel = React.lazy(() => import('@/pages/command/agent/AgentTaskPanel'))
const ModelConfigPanel = React.lazy(() => import('@/pages/command/agent/ModelConfigPanel'))
const OptimizationPanel = React.lazy(() => import('@/pages/command/agent/OptimizationPanel'))
...
const AGENT_ROUTE_MAP: Record<string, { component: React.LazyExoticComponent<React.ComponentType<{ agentId?: string }>>; label: string }> = {
  ...
  '/command/agents/task-panel': { component: AgentTaskPanel, label: '任务管理' },
  '/command/agents/model-config': { component: ModelConfigPanel, label: '模型配置' },
  '/command/agents/optimization-panel': { component: OptimizationPanel, label: '优化建议' },
}

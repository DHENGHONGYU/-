...
const {
  ...
  mockGenerateBuySellPointInsight,
  mockAnalyzeBuySellPoints,
} = vi.hoisted(() => ({
  mockLogger: {
    info: vi.fn(),
    warn: vi.fn(),
  ...
  mockGenerateBuySellPointInsight: vi.fn(),
  mockAnalyzeBuySellPoints: vi.fn(),
}))
...
vi.mock('@/services/trading/tradeReviewAI.reportGenerator', () => ({
  ...
  generateBuySellPointInsight: mockGenerateBuySellPointInsight,
}))
...
vi.mock('@/services/trading/buySellPointAnalyzer', () => ({
  analyzeBuySellPoints: mockAnalyzeBuySellPoints,
}))
...
function setupDefaultMocks() {
  ...
  mockGenerateBuySellPointInsight.mockReturnValue({
    pnlAttribution: [],
    dataPatterns: [],
    personalizedAdvice: [],
  })

  mockAnalyzeBuySellPoints.mockReturnValue({
    analyzedAt: Date.now(),
    entryTiming: {
      buyPointStats: [],
      bestBuyPointType: null,
      worstBuyPointType: null,
      timingScore: 0,
      summary: '',
    },
    exitTiming: {
      sellPointStats: [],
      bestSellPointType: null,
      worstSellPointType: null,
      timingScore: 0,
      summary: '',
    },
...
describe('generateTradeReviewUseCase（同步版）', () => {
  ...
  describe('正常生成复盘成功流程', () => {
    ...
    it('应当调用所有分析服务', () => {
      ...
      expect(mockAnalyzeBuySellPoints).toHaveBeenCalledWith(orders)
      expect(mockGenerateBuySellPointInsight).toHaveBeenCalledWith(expect.any(Object))
    })
  ...
  describe('复盘报告结构完整性验证', () => {
    it('报告应当包含所有必需字段', () => {
      ...
      expect(result).toHaveProperty('buySellPointReview')
  ...
  describe('并发调用互不干扰', () => {
    ...
    it('并发调用中部分失败不应影响其他调用', async () => {
      ...
      mockGenerateTradeSummary.mockImplementation(() => {
        ...
        return { totalTrades: 1, profitableTrades: 0, losingTrades: 0, winRate: 0, profitLossRatio: 0, avgProfit: 0, avgLoss: 0, totalPnL: 0, totalPnLPercent: 0, disciplineScore: 0, totalErrors: 0 }
      })

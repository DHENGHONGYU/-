...
function normalizeResearchStatus(isFavorite?: boolean): ResearchStatus {
  return (isFavorite ?? false) ? RESEARCH_STATUS.watching : RESEARCH_STATUS.candidate
}
...
export function normalizeStockThemes(v6: V6Stock): {
  ...
  if (tags.length > 0) {
    ...
    if ((groupCandidate ?? '').length <= 20) {
      group = groupCandidate
    }
    for (const tag of tags) {
      if (group === undefined || tag === group) continue
      themes.add(tag)
    }
  }
...
export function transformV6Score(v6: V6ScoreRecord): V6Score {
  ...
  if (v6.layers !== null && v6.layers !== undefined) {
    for (const [key, layer] of Object.entries(v6.layers)) {
      if (layer !== null && layer !== undefined && typeof layer.score === 'number' && Number.isFinite(layer.score)) factors[key] = layer.score
    }
  }
...
export function transformV6ScoreToDoc(v6: V6ScoreRecord): ScoreDocVersion {
  ...
  if (v6.layers !== null && v6.layers !== undefined) {
    for (const [key, layer] of Object.entries(v6.layers)) {
      if (layer !== null && layer !== undefined && typeof layer.score === 'number') layers[key] = {
        score: layer.score,
...
export function transformV6NewsArticle(v6: V6NewsArticle): NewsArticle {
  return {
    ...
    source: v6.source,
    category: v6.category,
    publishTime: v6.publishTime,
    fetchTime: v6.fetchTime,

 ...
 */
async function handleQuoteMode(ctx: SingleTraceContext): Promise<TraceResult> {
 ...
 */
async function handleKlineMode(ctx: SingleTraceContext): Promise<TraceResult> {
 ...
 */
async function handleFinancialMode(ctx: SingleTraceContext): Promise<TraceResult> {
 ...
 * AI 增强失败不阻塞主流程。
 */
async function runKimiaiEnhancement(
  ...
  if (dimensionCode === '05') {
    ...
    await writeMockDimensionData(symbol, dimensionCode, { _kimiSummary: aiResult }, DIMENSION_TO_ACTION[dimensionCode]!)
  ...
  if (dimensionCode === '08') {
    ...
    await writeMockDimensionData(symbol, dimensionCode, { _kimiDigest: aiResult }, DIMENSION_TO_ACTION[dimensionCode]!)
 ...
 */
async function handleNonQuoteMode(ctx: SingleTraceContext): Promise<TraceResult> {
  ...
  try {
    ...
    await writeMockDimensionData(symbol, dimensionCode, dimData, storeAction)
    getQualityMetrics().recordWrite(true)

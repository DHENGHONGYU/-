...
async function importOneRow(
  ...
  try {
    ...
    if (addResult.success && addResult.data) return { ok: true, row: globalIdx, raw: '', error: '', stock: addResult.data }
    return { ok: false, row: globalIdx, raw: `${row.code} ${row.name}`, error: addResult.error ?? '导入失败', stock: null }
  } catch (err) {
    return { ok: false, row: globalIdx, raw: `${row.code} ${row.name}`, error: err instanceof Error ? err.message : String(err), stock: null }
  }
}

function applyOutcome(result: BulkImportResult, o: RowImportOutcome): void {
  if (o.ok && o.stock) {
    result.success++
...
export async function importStocksWithProgress(
  ...
  for (let i = 0; i < importableRows.length; i += batchSize) {
    ...
    const outcomes = await Promise.all(
      batch.map((row, batchIdx) => importOneRow(row, i + batchIdx, options, seen)),
    )

    for (const o of outcomes) {
      applyOutcome(result, o)
    }
    ...
    // 批次间隔（最后一批不需要）
    if (i + batchSize < importableRows.length) await new Promise((resolve) => setTimeout(resolve, intervalMs))
  }

...
export const WIDGET_DEFAULT_DATA_SOURCE: Record<string, DataSourceConfig> = {
  ...
  catalystEvent: {
    ...
    mode: COLLECTION_MODE.POLLING,
    interval: 60000,
    endpoint: '/strategy/catalyst-events',
    enabled: true,
  },
  keyDataCalendar: {
    type: activeDs,
    mode: COLLECTION_MODE.POLLING,
    interval: 60000,
    endpoint: '/market/key-data-calendar',
    enabled: true,
  },
}
...
export const WIDGET_CROSS_LAYOUT: Record<string, { domain: WidgetDomain; perspective: WidgetPerspective }> = {
  ...
  // 策略与时机（投资者视角：催化剂+核心数据 → 何时行动）
  catalystEvent:      { domain: 'market',    perspective: 'signal' },
  keyDataCalendar:    { domain: 'market',    perspective: 'signal' },
}
...
export const COCKPIT_CROSS_DOMAINS: readonly { id: WidgetDomain; label: string; icon: string }[] = [
  { id: 'research', label: '智能研判', icon: '🔬' },
  { id: 'market', label: '市场总览', icon: '📈' },
  { id: 'ai', label: 'AI 辅助', icon: '🤖' },
  { id: 'portfolio', label: '持仓与风控', icon: '💼' },
]

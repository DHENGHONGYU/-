...
export const DATA_DICTIONARY: Record<StoreName, StoreDef> = {
  ...
  [STORE_NAME.sectorCollectData]: {
    name: STORE_NAME.sectorCollectData,
    keyPath: 'id',
    autoIncrement: false,
    introducedAt: 'v36',
    domain: '采集',
    description: '维度 10（热门板块）采集数据结构化存储（v36 新增，2026-08-22 采集能力缺口补齐；脱离 local_docs 过渡方案，与 hot_sector_scores 双策略评分存储严格隔离）',
    tsType: 'SectorCollectDataRecord',
    tsTypeFile: 'src/data/dataLayerContentStores.ts',
    fields: [
      { name: 'id', type: 'string', required: true, description: '记录唯一 ID（主键，管线合成 dim-10-{symbol}-{date}）' },
      { name: 'symbol', type: 'string', required: true, description: '关联股票代码', foreignKey: { store: STORE_NAME.stocks, field: 'symbol' } },
      { name: 'dimensionCode', type: 'string', required: true, description: '采集维度码，固定 10' },
      { name: 'collectedAt', type: 'number', required: true, description: '采集时间戳' },
      { name: 'source', type: 'string', required: false, description: '数据来源标签（ifind_mcp/tencent_mcp/...）' },
      { name: 'sectorCode', type: 'string', required: false, description: '板块代码（申万二级）' },
      { name: 'sectorName', type: 'string', required: false, description: '板块名称' },
      { name: 'score', type: 'number', required: false, description: '综合评分（0-5）' },
      { name: 'signal', type: 'string', required: false, description: '板块轮动信号' },
      { name: 'alertLevel', type: 'string', required: false, description: '预警等级' },
      { name: 'total', type: 'number', required: false, description: '五因子加权综合得分（0-100）' },
    ],
    indexes: [
      { name: 'by-symbol', keyPath: 'symbol', unique: false, description: '按股票代码查询' },
      { name: 'by-collected-at', keyPath: 'collectedAt', unique: false, description: '按采集时间排序' },
    ],
    dependsOn: [
      { store: STORE_NAME.stocks, field: 'symbol', description: 'N:1 采集记录↔股票' },
    ],
  },

  [STORE_NAME.qualityMetricsHistory]: {
    name: STORE_NAME.qualityMetricsHistory,
    keyPath: 'id',
    autoIncrement: false,
    introducedAt: 'v37',
    domain: '采集',
    description: '采集质量指标历史快照（v37 新增，P0-2 整改；解决质量指标纯内存刷新即丢问题，支撑 KPI 历史趋势与质量回归对比）',
    tsType: 'QualityMetricsHistoryRecord',
    tsTypeFile: 'src/data/types/types.qualityMetricsHistory.ts',
    fields: [
      { name: 'id', type: 'string', required: true, description: '记录唯一 ID（主键，形如 qmh-{capturedAt}）' },
      { name: 'capturedAt', type: 'number', required: true, description: '快照捕获时间戳（ms）' },
      { name: 'since', type: 'number', required: true, description: '指标时间窗口起始时间戳' },
      { name: 'totalCollects', type: 'number', required: true, description: '采集总次数' },
      { name: 'successCollects', type: 'number', required: true, description: '采集成功次数（含 mock）' },
      { name: 'mockCollects', type: 'number', required: true, description: 'mock 采集次数' },
      { name: 'mockSuccesses', type: 'number', required: true, description: 'mock 采集成功次数' },
      { name: 'successRate', type: 'number', required: true, description: '采集成功率（含 mock，0-100）' },
      { name: 'realSuccessRate', type: 'number', required: true, description: '真实数据源采集成功率（排除 mock，0-100）' },
      { name: 'completeness', type: 'number', required: true, description: '数据完整率（0-100）' },
      { name: 'sourceCounts', type: 'object', required: true, description: '各数据源使用次数（报价链源 + MCP 多维源）' },
      { name: 'fallbackCount', type: 'number', required: true, description: '降级次数' },
      { name: 'writeSuccess', type: 'number', required: true, description: '写入成功次数' },
      { name: 'writeTotal', type: 'number', required: true, description: '写入总次数' },
      { name: 'writeRate', type: 'number', required: true, description: '写入成功率（0-100）' },
      { name: 'mockWrites', type: 'number', required: true, description: 'mock 写入次数' },
      { name: 'avgLatency', type: 'number', required: true, description: '平均延迟（ms）' },
      { name: 'totalLatency', type: 'number', required: true, description: '总延迟（ms）' },
      { name: 'lastError', type: 'string', required: false, description: '最后一次错误信息' },
    ],
    indexes: [
      { name: 'by-captured-at', keyPath: 'capturedAt', unique: false, description: '按捕获时间排序（趋势查询）' },
    ],
    dependsOn: [],
  },

  [STORE_NAME.dimensionCollectData]: {
    name: STORE_NAME.dimensionCollectData,
    keyPath: 'id',
    autoIncrement: false,
    introducedAt: 'v38',
    domain: '采集',
    description: '维度 11-14（技术指标/资金流向/机构持仓/估值分析）采集数据通用专用存储（v38 新增，2026-08-23 遗留问题整改 P2；脱离 local_docs 过渡方案，沿用 sector_collect_data v36 先例）',
    tsType: 'DimensionCollectDataRecord',
    tsTypeFile: 'src/data/dataLayerContentStores.ts',
    fields: [
      { name: 'id', type: 'string', required: true, description: '记录唯一 ID（主键，管线合成 dim-{dimensionCode}-{symbol}-{date}）' },
      { name: 'symbol', type: 'string', required: true, description: '关联股票代码', foreignKey: { store: STORE_NAME.stocks, field: 'symbol' } },
      { name: 'dimensionCode', type: 'string', required: true, description: '采集维度码（11/12/13/14）' },
      { name: 'collectedAt', type: 'number', required: true, description: '采集时间戳' },
      { name: 'source', type: 'string', required: false, description: '数据来源标签（ifind_mcp/tencent_mcp/...）' },
      { name: 'payload', type: 'object', required: false, description: '维度数据载荷（各维度结构化字段按需透传）' },
    ],
    indexes: [
      { name: 'by-symbol', keyPath: 'symbol', unique: false, description: '按股票代码查询' },
      { name: 'by-dimension', keyPath: 'dimensionCode', unique: false, description: '按维度码查询' },
      { name: 'by-collected-at', keyPath: 'collectedAt', unique: false, description: '按采集时间排序' },
    ],
    dependsOn: [
      { store: STORE_NAME.stocks, field: 'symbol', description: 'N:1 采集记录↔股票' },

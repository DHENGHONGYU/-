...
export const ACL_MATRIX: Readonly<Record<ModuleId, AclPermission>> = {
  ...
  [MODULE_ID.pool]: {
    read: [STORE_NAME.stocks, STORE_NAME.v6Scores, STORE_NAME.traceRecords, STORE_NAME.rotationScores],
  ...
  [MODULE_ID.tradeReviews]: {
    ...
    // 2026-08-11 修复：补上 select 权限。disciplineStore.refresh() 以 tradeReviews 模块
    // 发起 queryGet 读 trade_reviews（恢复已持久化复盘记录），原 actions 缺 select 触发

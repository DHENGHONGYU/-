/**
 * 外部行情数据源端点配置（Vite proxy 路径 — 浏览器环境专用）
 *
 * @description
 * 直连腾讯/新浪/网易等公开行情 API 的 Vite proxy 路径与 Referer 集中管理。
 * 浏览器代码从此文件导入，禁止在 services/ 层硬编码外部 URL。
 *
 * ⚠️ **配置源分工说明（消除混淆）**：
 *   - 本文件（marketDataEndpoints.ts）：**Vite proxy 路径**（/api/proxy/*），用于浏览器 fetch
 *     （经 Vite dev server 代理转发至真实 URL，解决 CORS 限制）
 *   - `@/config/dataSourceUrls`：**绝对 URL**，用于 Node 环境 / Electron 主进程 fetch 直连

...
const {
  ...
  const capturedEventBusCallbacks = new Map<string, ((payload?: Record<string, unknown>) => void)>()
  const unsubscribeFns: Array<ReturnType<typeof vi.fn>> = []

  const mockOn = vi.fn().mockImplementation((event: string, callback: (payload?: Record<string, unknown>) => void) => {

...
export default function TradingFlowPage(): React.JSX.Element {
  ...
  // 从信号创建订单
  const handleCreateOrderFromSignal = async (signal: { symbol: string; direction: string }): Promise<void> => {
    const timestamp = new Date().toISOString()
        ...
        title={
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge className={cn(
              ...
              {USE_MOCK_DATA ? '数据源：Mock · 开发演示' : '数据源：真实采集 · DataBridge'}
            </Badge>
          </div>
        }
        description={
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              待执行 / 总订单 {displayOrders.length} 个
            </p>
          </CardContent>
        </Card>

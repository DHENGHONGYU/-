...
export function generateDemoKlineData(
  ...
  for (let i = days; i >= 0; i--) {
    ...
    const rand2 = ((seed * 3591 + 12917) % 233280) / 233280

    const drift = (rand1 - 0.5) * safeBase * 0.04
    ...
    const volume = Math.round(500000 + rand1 * 2000000)
...
export function generateDemoChipData(
  ...
  const peakOffset = isLow ? -0.15 : 0.15
      ...
      .then(async (resp) => {
        ...
        if (json.success && json.data) {
          ...
          logger.info(`${LOG_TAG} [数据剖面] 真实筹码数据 (real)`, {
            symbol,
            binsCount: realChip.priceBins?.length ?? 0,
            chipPercent: {
              sum: pctSum,
              max: pctMax,
              min: pctMin,

/**
 * @fileoverview 贵州茅台实时行情展示组件
 * 使用 useRealtimeQuote 和 useKline hooks 展示实时行情与K线数据
 *
 * @module showcase/MoutaiRealtimeShowcase
 */

import { TrendingUp, TrendingDown, Minus, RefreshCw, Clock, BarChart3, LineChart } from 'lucide-react'
 ...
 * 贵州茅台实时行情展示组件
 *
 * 使用 useRealtimeQuote hook 自动获取并刷新实时行情数据，
 * 使用 useKline hook 获取K线数据，支持周期切换。
 */
export function MoutaiRealtimeShowcase(): React.JSX.Element {
  const {
    data: quoteData,
  ...
  const klineStats = (() => {
    ...
    return {
      ...
      high: Math.max(...highs),
      low: Math.min(...lows),
      avgVolume: klineBars.reduce((sum, b) => sum + b.volume, 0) / klineBars.length,
    }
  })()
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className={cn('text-lg font-bold', 'text-foreground')}>{STOCK_NAME}</span>
              <span className={cn('text-sm', 'text-muted-foreground')}>{SYMBOL}</span>
              {isStale && (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {klineLoading && klineBars.length === 0 ? (
                ...
                klineBars.slice(-RECENT_KLINE_COUNT).reverse().map((bar, idx) => {
                  ...
                  return (
                    <tr key={bar.date} className={cn('border-t', 'border-border')}>
                      ...
                      <td className="px-4 py-2 text-right tabular-nums text-foreground">{bar.open.toFixed(2)}</td>
                      <td className="px-4 py-2 text-right tabular-nums" style={{ color: getStockColorHex(1) }}>
                        {bar.high.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums" style={{ color: getStockColorHex(-1) }}>
                        {bar.low.toFixed(2)}
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums font-medium" style={{ color: barColor }}>
                        {bar.close.toFixed(2)}
                      </td>
